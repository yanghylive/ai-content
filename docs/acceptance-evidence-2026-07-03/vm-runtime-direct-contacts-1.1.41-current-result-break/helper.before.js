...
function decryptedSnapshotCandidates(originalDbPath, details = {}) {
  ...
  const candidates = [];
  const addCandidate = (filePath, reason) => {
    ...
    if (originalSize > 0) {
      ...
      const tolerance = Math.max(1024 * 1024, Math.floor(originalSize * 0.15));
      if (delta > tolerance) return;
    }
    if (!isPlainSqliteDatabase(filePath)) return;
    candidates.push({
      path: filePath,
      reason,
      size,
  ...
  for (const [dbIndex, dbPath] of dbPaths.entries()) {
    ...
    if (!isPlainSqliteDatabase(dbPath)) {
      ...
      const earlySnapshots = isFirstRankedContactDb &&
        ...
        ? decryptedSnapshotCandidates(dbPath, details)
        : [];
