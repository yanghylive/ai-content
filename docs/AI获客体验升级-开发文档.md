...
> - ✅ **账号**：kaypal 平台用户 `cmo9p6i5x000a58uckbcyv45u`（phone=18230326666 / name=大壮 / FLAGSHIP）已在用户表，已设置登录密码（=ai-content 本地密码 sn198456 的 bcrypt $2b$10$），`phoneVerifiedAt` 已置位
> - ✅ **登录**：`POST /api/auth/login`（3000 内网或 https://kaypal.cn 同构）`{"phone":"18230326666","password":"sn198456"}` → 200 + `Set-Cookie: kaypal_auth=<JWT>`（HttpOnly，**Domain=.kaypal.cn，Secure，7 天**）；`email` 字段亦可。注意：`identifier`/`username` 字段不被认，必须 `phone` 或 `email`
...
> - **ai-content 后端落地方式**：后端启动时用 18230326666 登一次拿 JWT（或维护 refresh），后续请求带 `Cookie: kaypal_auth=<JWT>`；JWT 7 天过期需重登。**敏感信息**：密码/凭据放后端 env（不入库不入前端）
