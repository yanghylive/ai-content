# Growth Commercial Live Gate

Status: **BLOCKED**
Generated: 2026-06-26T19:03:13.856Z

## Checks

- **PASS** backend-process: Backend listening on 3011, pid=87611.
- **PASS** auth-cookie: Auth cookie/session token was provided to the gate.
- **PASS** auth-me: Authenticated as 大壮.
- **PASS** commercial-permission: commercialExecutionAllowed=true, planMode=commercial, kaypalPlan=ADVANCED, expired=false.
- **BLOCKER** growth-execution-switch: runtimeStatus.executionEnabled=false; processEnv=false.
- **BLOCKER** growth-scheduler-daemon-armed: configured=false; armed=false; envDaemon=false; envRealAllowed=false.
- **BLOCKER** verified-growth-account: visibleAccounts=1, onlineNormalAccounts=0.
- **PASS** schedule-plan-api: items=2, ready=0, blocked=0, waiting=0.
- **BLOCKER** ready-auto-task: readyCount=0; a commercial live execution test needs at least one ready auto task bound to a verified account.
- **PASS** overview-api: activeConfigCount=1, todayLeadCount=1, contacted=1.
- **PASS** report-diagnosis-honesty: bottlenecks=3; visibleAccounts=1; no-blocker-copy=false.
- **PASS** database-readable: Read SQLite database at /Users/yanghy/Library/Application Support/ai-content-desktop/kaypal-ai.sqlite.
- **PASS** database-account-health: growth_account_health rows=4.
- **PASS** database-configs: growth_acquisition_configs rows=4.
- **PASS** database-runs: growth_acquisition_runs rows=7.

## Evidence Files

- `summary.json`
