...
| 优先级 | 差距 | 理由 | 参考工作量 |
|---|---|---|---|
| P0 | **TikTok 授权 + 自动加友 + 号码提取** | 获客纵深最大空白，炼刀核心卖点；抖音链路可复用（同技术栈） | 中 |
| P1 | **POI 门店数据层** | 本地生活赛道入口；video-workshop 已有探店模板，补数据层闭环 | 中 |
| P1 | **Token 用量追踪 + RPA 额度预检** | 计费/风控基础设施，AI 员工商用必备 | 小 |
| P2 | **悬浮球 + AI 网页代操作** | 通用 RPA 产品形态；底层 LocalBrowserEngine 已有，做 AI 驱动层 | 大 |
| P2 | **商品视频自动剪辑** | 视频赛道纵深；studio_core 已有一键成片，补商品模板 | 中 |
...
2. **真实差距 = 获客纵深 + 通用 RPA 形态**：TikTok（17 端点全空白）、POI（0）、手机操控/悬浮球/AI 网页代操作（0）——都是炼刀在"AI 数字员工"叙事下的差异化卖点
3. **技术可行性高**：TikTok 可复用抖音浏览器自动化栈；AI 网页代操作可基于已有 LocalBrowserEngine 加 AI 意图层；无需新建基础设施
4. 建议按 P0→P2 排期，**先做 TikTok 授权闭环**（对标价值最大），再做 POI 数据层
