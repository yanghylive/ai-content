...
- 账号：__REDACTED_TEST_USER__（User.id = cmo9p6i5x000a58uckbcyv45u）
...
## 验证
`SELECT phone, "subscriptionPlan", balance FROM "User" u JOIN "EntropyBalance" eb ON eb."userId"=u.id WHERE phone='__REDACTED_TEST_USER__'`
→ `__REDACTED_TEST_USER__ | FLAGSHIP | 16037.01`
