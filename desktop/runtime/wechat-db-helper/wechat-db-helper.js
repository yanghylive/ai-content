...
const MIN_STABLE_DECRYPTED_CACHE_BYTES = 4096;
const MIN_SCANNED_DECRYPTED_CACHE_BYTES = 1024 * 1024;
...
function decryptWithMemoryKey(dbPath, input, diagnostics, originalDbPath = dbPath) {
  ...
  const skipAfterExternalFailure =
...
function isHealthySqliteSnapshot(sqlitePath, filePath) {
  if (!isPlainSqliteDatabase(filePath)) return false;
  if (!sqlitePath) return true;
  const result = run(sqlitePath, ['-batch', '-noheader', filePath, 'PRAGMA quick_check;'], {
    timeout: 10000,
  });
  return result.status === 0 && String(result.stdout || '').trim().toLowerCase() === 'ok';
}

function decryptedSnapshotCandidates(originalDbPath, details = {}, sqlitePath = '') {
  const originalStat = safeStat(originalDbPath);
  const originalSize = Number(originalStat?.size) || 0;
  const originalMtime = Number(originalStat?.mtimeMs) || 0;
  const maxAgeMs = Math.max(
    5 * 60 * 1000,
    Math.min(Number(process.env.AI_CONTENT_WECHAT_DECRYPTED_CACHE_MAX_AGE_MS) || 6 * 60 * 60 * 1000, 7 * 24 * 60 * 60 * 1000),
  );
  const candidates = [];
  const addCandidate = (filePath, reason, options = {}) => {
    if (!filePath || !pathExists(filePath)) return;
    const stat = safeStat(filePath);
    const size = Number(stat?.size) || 0;
    const minimumBytes = Number(options.minimumBytes) || MIN_SCANNED_DECRYPTED_CACHE_BYTES;
    if (size < minimumBytes) return;
    if (originalSize > 0) {
      const delta = Math.abs(size - originalSize);
      const tolerance = Math.max(
        Number(options.toleranceBytes) || 1024 * 1024,
        Math.floor(originalSize * 0.15),
      );
      if (delta > tolerance) return;
    }
    if (!isHealthySqliteSnapshot(sqlitePath, filePath)) return;
    candidates.push({
      path: filePath,
      reason,
      size,
      mtimeMs: Number(stat?.mtimeMs) || 0,
      sizeDelta: originalSize > 0 ? Math.abs(size - originalSize) : 0,
      ageMs: originalMtime > 0 && Number(stat?.mtimeMs) ? originalMtime - Number(stat.mtimeMs) : 0,
      accountFolder: details.accountFolder || '',
      baseWxid: details.baseWxid || '',
    });
  };

  addCandidate(stableDecryptedDbPath(originalDbPath), 'stable-path', {
    minimumBytes: MIN_STABLE_DECRYPTED_CACHE_BYTES,
    toleranceBytes: 64 * 1024,
  });
  try {
    const entries = fs.readdirSync(DECRYPTED_DB_ROOT, { withFileTypes: true });
    for (const entry of entries) {
      if (!entry.isFile()) continue;
      if (!/^contact-[a-f0-9]{32}\.db$/i.test(entry.name)) continue;
      addCandidate(path.join(DECRYPTED_DB_ROOT, entry.name), 'decrypted-root-scan');
    }
  } catch {
    // Missing temp cache is normal on a fresh machine.
  }

  return candidates
    .filter((item) => item.ageMs <= maxAgeMs)
    .sort((left, right) => {
      if (left.sizeDelta !== right.sizeDelta) return left.sizeDelta - right.sizeDelta;
...
function queryContacts(sqlitePath, dbPaths, mode, limits, input = {}) {
  ...
  for (const [dbIndex, dbPath] of dbPaths.entries()) {
    ...
    if (!isPlainSqliteDatabase(dbPath)) {
      ...
      const earlySnapshots = isFirstRankedContactDb &&
        process.env.AI_CONTENT_WECHAT_DECRYPTED_CACHE_FIRST !== '0'
        ? decryptedSnapshotCandidates(dbPath, details, sqlitePath)
        : [];
      if (earlySnapshots.length) {
        diagnostics.decryptedSnapshotCandidates = earlySnapshots.map((item) => ({
          path: item.path,
          reason: item.reason,
          size: item.size,
          ...
          ageMs: item.ageMs,
          accountFolder: item.accountFolder,
          baseWxid: item.baseWxid,
        }));
        candidatePath = earlySnapshots[0].path;
        candidateSource = 'windows-wechat-db-decrypted-cache';
        sawPlaintext = true;
        diagnostics.dbErrors.push({
          path: dbPath,
          status: 'warn',
          reason: 'current-account-db-key-missing-used-decrypted-cache',
          error: 'using a recent matching plaintext decrypted snapshot before slow key extraction',
          snapshotPath: candidatePath,
        });
        diagnostics.warnings.push('current-account-db-key-missing-used-decrypted-cache');
      }
      const encryptedTarget = candidateSource === 'windows-wechat-db-decrypted-cache'
        ? null
        : prepareReadTarget(dbPath, diagnostics);
      ...
      if (!decryptedSucceeded && isFirstRankedContactDb) {
        const snapshots = decryptedSnapshotCandidates(dbPath, details, sqlitePath);
        diagnostics.decryptedSnapshotCandidates = snapshots.map((item) => ({
          path: item.path,
          reason: item.reason,
          size: item.size,
          mtimeMs: item.mtimeMs,
        ...
        if (snapshots.length) {
          candidatePath = snapshots[0].path;
          candidateSource = 'windows-wechat-db-decrypted-cache';
          sawPlaintext = true;
          diagnostics.dbErrors.push({
            path: dbPath,
            status: 'warn',
            reason: 'current-account-db-key-missing-used-decrypted-cache',
            error: 'current account contact.db could not be decrypted; using a matching plaintext decrypted snapshot instead of stale-account fallback',
            snapshotPath: candidatePath,
          });
          diagnostics.warnings.push('current-account-db-key-missing-used-decrypted-cache');
        } else {
    ...
    const currentDbHasResult = isFirstRankedContactDb &&
      candidateResults.some((item) => item.dbPath === dbPath);
    if (currentDbHasResult) {
      break;
    }
    if (isFirstRankedContactDb && !currentDbHasResult) {
      diagnostics.currentAccountDbBlocked = true;
      diagnostics.selectedDbPath = dbPath;
      diagnostics.selectedDbAccountFolder = details.accountFolder;
      diagnostics.selectedDbBaseWxid = details.baseWxid;
      ...
      diagnostics.dbErrors.push({
        ...
        error: 'current account contact.db returned no readable contacts; refused stale-account fallback',
      });
      break;
    }
  }
