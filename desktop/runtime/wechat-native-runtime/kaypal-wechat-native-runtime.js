...
// Key discovery scans the same WeChat process memory regardless of result mode.
const DB_HELPER_TIMEOUT_MS = 240000;
...
function isUiaContactFallbackEnabled() {
  return process.env.AI_CONTENT_WECHAT_DISABLE_UIA_CONTACT_FALLBACK !== '1';
}

function contractSummary(mode) {
  const uiaFallbackEnabled = isUiaContactFallbackEnabled();
  return {
...
function collectByHelper(helperInfo, dbPaths, mode, processInfo) {
  const diagnostics = {
    ...
    helperError: '',
    helperTimeoutMs: DB_HELPER_TIMEOUT_MS,
  };
  if (!helperInfo) {
  ...
  diagnostics.helperTimedOut = result.error && result.error.code === 'ETIMEDOUT';

  const jsonLine = findLastJsonLine(result.stdout);
  if (!jsonLine) {
    diagnostics.helperStatus = diagnostics.helperTimedOut
      ? 'timeout'
      : result.status === 0
        ? 'failed-no-json'
        : 'failed-exit';
    diagnostics.helperError = diagnostics.helperTimedOut
      ? `db helper timed out after ${DB_HELPER_TIMEOUT_MS}ms`
      : compactText(result.stderr || result.stdout || result.error?.message || `helper exit ${result.status}`);
    return { items: [], diagnostics };
  }
...
function runContacts() {
  ...
  const now = new Date().toISOString();
  const allowUiaContactFallback = isUiaContactFallbackEnabled();
  ...
  if (!allowUiaContactFallback) {
    ...
    emit({
      ...
      error: mode === 'all'
        ? '微信全部好友暂时没同步成功：本机没有读到可用联系人'
        : '微信通讯录暂时没同步成功：本机没有读到可用联系人',
      diagnostics: {
        ...diagnostics,
  ...
  emit({
    ...
    output: {
      ...
      inputValidation: {
        ...
        'friend-accept': ['auto-send mode', 'explicit test-only matchKeywords[]', 'dailyLimit 1-100'],
    ...
    responseShape: {
      ...
      command: 'contacts|group-broadcast|contact-add|friend-accept|moments-publish|moments-marketing|chat-history',
    ...
    commandMatrix: [
      ...
      },
      {
        ...
        implementation: 'bundled-windows-runner-or-controlled-preflight',
        supportsAutoSend: 'only for explicitly matched test requests with commercial authorization and readback evidence',
        realSendSupported: 'runner-dependent',
        platforms: ['win32'],
        notes: 'Approval mode never clicks Accept. Auto-send requires non-empty matchKeywords and returns one result/evidence set per request.',
      },
      {
    ...
    commercialLimitations: [
      'Write-like WeChat tasks run only through a packaged or configured self-developed runner, and success is rejected unless real action/readback evidence is returned.',
      'Friend acceptance is test-scoped: approval mode is blocked and auto-send requires explicit match keywords before the runner can touch a request.',
    ...
    externalRunner: {
      ...
      stdin: {
        contractVersion: UNIFIED_COMMAND_CONTRACT_VERSION,
        command: 'group-broadcast|contact-add|friend-accept|moments-publish|moments-marketing|chat-history',
...
function plannedBatchTargets(command, input) {
  ...
  if (command === 'friend-accept') {
    return [
      {
        id: 'friend-request-scan',
        displayName: '明确测试好友申请扫描',
        searchText: '新的朋友',
        synthetic: true,
      },
    ];
...
function actionForCommand(command, input) {
  ...
  if (command === 'friend-accept') return 'accept-friend';
  if (command === 'moments-marketing') {
...
function controlledRunnerBlockedReason(command, context) {
  ...
  if (command === 'friend-accept' && sendMode !== 'auto-send') {
    return {
      ...
      message: '通过好友是不可草稿化的真实写入动作，确认前不会点击任何好友申请。',
      nextAction: '仅对明确的测试好友申请配置筛选关键词，并确认自动执行后重试。',
    };
  }
...
function validateFriendAccept(rawInput) {
  const input = asRecord(rawInput);
  const keywords = Array.isArray(input.matchKeywords)
    ? input.matchKeywords.map(compactText).filter(Boolean)
    : [];
  if (!keywords.length) {
    return {
      errorCode: 'content_invalid',
      message: '自动通过好友必须配置明确的测试申请筛选关键词，禁止无条件扫描并通过真实联系人。',
    };
  }
  const dailyLimit = Number(input.dailyLimit === undefined ? 20 : input.dailyLimit);
  if (!Number.isFinite(dailyLimit) || dailyLimit < 1 || dailyLimit > 100) {
    return {
      errorCode: 'content_invalid',
      message: '自动通过好友 dailyLimit 必须是 1-100 之间的数字。',
    };
  }
  const remark = asRecord(input.remark);
  const strategy = compactText(remark.strategy || 'request_name');
  if (!['request_name', 'phone_wechat', 'manual'].includes(strategy)) {
    return {
      errorCode: 'content_invalid',
      message: '自动通过好友备注策略不受支持。',
    };
  }
  return { expectedText: compactText(input.welcomeMessage) || keywords.join('、') };
}
...
function controlledDiagnosticsForCommand(command, input, requestDiagnostics, context, validation) {
  ...
  if (command === 'group-broadcast' || command === 'contact-add' || command === 'friend-accept' || command === 'moments-marketing') {
...
function runControlledCommand(command) {
  ...
  if (command === 'friend-accept' && sendModeFromContext(request.context) !== 'auto-send') {
    const approvalBlocker = controlledRunnerBlockedReason(command, request.context);
    fail(command, {
      status: 'blocked',
      errorCode: approvalBlocker.errorCode,
      blocker: 'manual_approval_required',
      error: approvalBlocker.message,
      nextAction: approvalBlocker.nextAction,
      output: controlledOutputForCommand(command, request.input, validation, request.context),
      diagnostics: controlledDiagnosticsForCommand(command, request.input, request.diagnostics, request.context, validation),
      retryable: true,
      manualActionRequired: true,
