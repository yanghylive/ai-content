...
def generate_schedule_time_next_day(total_videos, videos_per_day = 1, daily_times=None, timestamps=False, start_days=0, jitter_minutes=0):
    """
    ...
    - jitter_minutes: Random minute offset applied around each base time.
    ...
    if jitter_minutes < 0:
        raise ValueError("jitter_minutes should not be negative")
    ...
    for video in range(total_videos):
        ...
        if jitter_minutes:
            timestamp += timedelta(minutes=random.randint(-jitter_minutes, jitter_minutes))
            day_start = datetime.combine(target_date, _time.min)
            day_end = datetime.combine(target_date, _time.max).replace(second=0, microsecond=0)
            timestamp = max(day_start, min(timestamp, day_end))
