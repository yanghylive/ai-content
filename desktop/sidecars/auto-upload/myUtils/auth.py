        ...
        try:
            ...
            for indicator in login_indicators:
                if await page.locator(indicator).count():
                    tencent_logger.error(f"[tencent] cookie 失效，命中登录提示: {indicator}")
                    if preview:
                        await page.wait_for_timeout(1500)
                    return False
            ...
            tencent_logger.warning(f"[tencent] cookie 校验临时超时: {e}")
            raise
        except PlaywrightError as e:
            tencent_logger.warning(f"[tencent] cookie 校验浏览器异常: {e}")
            raise
...
async def cookie_auth_ks(account_file, preview: bool = False):
    async with async_playwright() as playwright:
        ...
        try:
            ...
            kuaishou_logger.warning(f"[kuaishou] cookie 校验临时超时: {e}")
            raise
        except PlaywrightError as e:
            kuaishou_logger.warning(f"[kuaishou] cookie 校验浏览器异常: {e}")
            raise
        except Exception as e:
...
async def cookie_auth_xhs(account_file, preview: bool = False):
    async with async_playwright() as playwright:
        ...
        try:
            ...
            try:
                await page.wait_for_url("**/creator-micro/content/upload**", timeout=8000)
            except PlaywrightTimeoutError:
                current_url = page.url or ""
                if "creator.xiaohongshu.com" not in current_url:
                    xiaohongshu_logger.warning("[xhs] cookie 失效，未进入创作后台")
                    if preview:
                        await page.wait_for_timeout(1500)
                    return False
            try:
            ...
            xiaohongshu_logger.warning(f"[xhs] cookie 校验临时超时: {e}")
            raise
        except PlaywrightError as e:
            xiaohongshu_logger.warning(f"[xhs] cookie 校验浏览器异常: {e}")
            raise
        finally:
            await context.close()
            await browser.close()
