...
const apiBase = stripTrailingSlash(
  process.env.API_BASE || "http://localhost:3011/api",
);
const frontendUrl = stripTrailingSlash(
  process.env.FRONTEND_URL || "http://localhost:3010",
);
const localIntegrationEnvPath =
  process.env.COMMERCIAL_LOCAL_ENV_PATH ||
  join(process.cwd(), ".local-logs", "local-integration.env");
let localIntegrationEnv = {};
if (existsSync(localIntegrationEnvPath)) {
  try {
    localIntegrationEnv = parseEnvFile(
      readFileSync(localIntegrationEnvPath, "utf8"),
    );
  } catch {
    localIntegrationEnv = {};
  }
}
const explicitDatabaseMode =
  localIntegrationEnv.COMMERCIAL_DATABASE_MODE ||
  process.env.COMMERCIAL_DATABASE_MODE ||
  process.env.KAYPAL_DESKTOP_DATABASE_MODE ||
  "";
const explicitDatabasePath =
  localIntegrationEnv.COMMERCIAL_DATABASE_PATH ||
  process.env.COMMERCIAL_DATABASE_PATH ||
  "";
const backendEnvPath =
  process.env.COMMERCIAL_BACKEND_ENV_PATH ||
  join(process.cwd(), "backend", ".env");
const backendDatabaseUrl = readBackendEnvValue("DATABASE_URL");
const databaseUrl =
  process.env.COMMERCIAL_DATABASE_URL ||
  localIntegrationEnv.COMMERCIAL_DATABASE_URL ||
  (explicitDatabasePath ? `file:${explicitDatabasePath}` : backendDatabaseUrl);
const databaseMode =
  normalizeDatabaseMode(explicitDatabaseMode) ||
  (/^postgres(?:ql)?:\/\//i.test(databaseUrl || "") ? "postgres" : "sqlite");
const databasePath =
  explicitDatabasePath ||
  localIntegrationEnv.COMMERCIAL_DATABASE_PATH ||
  resolveSqliteDatabasePath();
const databaseTarget =
  databaseMode === "postgres" ? redactDatabaseUrl(databaseUrl) : databasePath;
const authCookieName =
  process.env.AUTH_COOKIE_NAME ||
  readBackendEnvValue("AUTH_COOKIE_NAME") ||
  "ai_content_session";
const providedCookieHeader =
  process.env.COMMERCIAL_COOKIE_HEADER || process.env.SMOKE_COOKIE_HEADER || "";
const providedCookieFile =
  process.env.COMMERCIAL_COOKIE_FILE || process.env.SMOKE_COOKIE_FILE || "";
const providedSessionToken =
  process.env.COMMERCIAL_SESSION_TOKEN || process.env.SMOKE_SESSION_TOKEN || "";
const cliRealAcceptanceEnabled =
  process.argv.includes("--real") || process.argv.includes("--real-acceptance");
...
const interactiveLoginHeadless =
  process.env.COMMERCIAL_INTERACTIVE_LOGIN_HEADLESS === "1";
const realAcceptanceEnabled =
  cliRealAcceptanceEnabled || process.env.COMMERCIAL_REAL_ACCEPTANCE === "1";
const interactiveLoginTimeoutMs = Number(
  process.env.COMMERCIAL_INTERACTIVE_LOGIN_TIMEOUT_MS || 180000,
);
const username =
  process.env.COMMERCIAL_USERNAME || process.env.SMOKE_USERNAME || "";
const password =
  process.env.COMMERCIAL_PASSWORD || process.env.SMOKE_PASSWORD || "";
const timeoutMs = Number(process.env.COMMERCIAL_TIMEOUT_MS || 30000);
const publishTimeoutMs = Number(
  process.env.COMMERCIAL_PUBLISH_TIMEOUT_MS || 180000,
);
const pollMs = Number(process.env.COMMERCIAL_POLL_MS || 1000);
const pollAttempts = Number(process.env.COMMERCIAL_POLL_ATTEMPTS || 45);
const realExecutionPollAttempts = Math.max(
  pollAttempts,
  Number(process.env.COMMERCIAL_REAL_EXECUTION_POLL_ATTEMPTS || 90),
);
const contentGenerateTimeoutMs = Number(
  process.env.COMMERCIAL_CONTENT_GENERATE_TIMEOUT_MS || 25 * 60 * 1000,
);
const realExecutionEnabled =
  realAcceptanceEnabled || process.env.COMMERCIAL_REAL_EXECUTION === "1";
const approveDraftsEnabled =
  realAcceptanceEnabled || process.env.COMMERCIAL_APPROVE_DRAFTS === "1";
const realAutoSendEnabled =
  realAcceptanceEnabled || process.env.COMMERCIAL_REAL_AUTO_SEND === "1";
const approveAutoSendEnabled =
  realAcceptanceEnabled || process.env.COMMERCIAL_APPROVE_AUTO_SEND === "1";
const realPublishEnabled =
  realAcceptanceEnabled || process.env.COMMERCIAL_REAL_PUBLISH === "1";
const approvePublishEnabled =
  realAcceptanceEnabled || process.env.COMMERCIAL_APPROVE_PUBLISH === "1";
const realContentPipelineEnabled =
  realAcceptanceEnabled || process.env.COMMERCIAL_REAL_CONTENT_PIPELINE === "1";
const approveContentPipelineEnabled =
  realAcceptanceEnabled ||
  process.env.COMMERCIAL_APPROVE_CONTENT_PIPELINE === "1";
...
const pressureCycles = Math.max(
  1,
  Number(process.env.COMMERCIAL_PRESSURE_CYCLES || 5),
);
const realAutoSendCycles = Math.max(
  1,
  Number(process.env.COMMERCIAL_REAL_AUTO_SEND_CYCLES || 5),
);
const contentPipelineCycles = Math.max(
  1,
  Number(process.env.COMMERCIAL_CONTENT_PIPELINE_CYCLES || 5),
);
...
const reportDir =
  process.env.COMMERCIAL_REPORT_DIR || join(process.cwd(), ".local-logs");
...
let latestPublishableArticle = null;
let latestReadyAccount = null;
...
async function checkLocalDatabasePreconditions() {
  if (databaseMode === "sqlite" && !existsSync(databasePath)) {
    record(
      "BLOCKED",
      "local database missing",
      databaseTarget,
      "先运行后端迁移/初始化，确认 DATABASE_URL 指向真实验收库。",
      {
        area: "readiness",
        requirement:
          "3010 项目数据库必须可读，模型、账号、任务状态不能靠前端假数据。",
      },
    );
    return;
  }
  ...
  try {
    const counts = {
      ...
      legacyInteractionTasks: await safeTableCount(
    ...
    record(
      "PASS",
      "interaction_tasks table present",
      `interaction_tasks=${counts.interactionTasks}`,
      "",
      {
        area: "readiness",
        requirement: "真实任务表应使用 interaction_tasks。",
      },
    );
    if (counts.legacyInteractionTasks === null) {
      record(
        "WARN",
        "legacy local_engine_interaction_tasks table absent",
        "兼容旧表已不作为硬门槛；当前代码路径使用 interaction_tasks。",
        "",
        {
          area: "readiness",
          requirement: "旧兼容表仅用于历史数据兼容，不应阻塞发布验收。",
        },
      );
    } else {
      record(
        "PASS",
        "legacy local_engine_interaction_tasks table detected",
        `local_engine_interaction_tasks=${counts.legacyInteractionTasks}`,
        "",
        {
          area: "readiness",
          requirement: "旧兼容表存在时仅作为历史数据兼容信号。",
        },
      );
    }
    record(
      counts.aiPlatforms > 0 && counts.aiModels > 0 && counts.defaultModels > 0
        ? "PASS"
        : "BLOCKED",
    ...
    if (counts.publishAccounts === 0) {
      ...
      record(
        "PASS",
        "3010 publish accounts present",
        `publish_accounts=${counts.publishAccounts}`,
        "",
        {
          area: "publishing-center",
...
async function checkDefaultTextModels() {
  ...
  record(
    ...
    usable
      .map((row) => `${row.purpose}:${row.modelName}/${row.platformName}`)
      .join(" | "),
    "",
    {
      area: "readiness",
...
async function checkRuntimePreconditions() {
  const health = await request("GET", "/auto-upload/health", undefined, {
    ...
    record(
      "BLOCKED",
      "in-process runtime health failed",
      error.message,
      "启动 3011 后端 Runtime 后重试。",
      {
        area: "readiness",
  ...
  if (health) {
    record(
      "PASS",
      "in-process runtime online",
      `${apiBase}/auto-upload/health`,
      "",
      {
        area: "readiness",
  ...
  record(
    "PASS",
    "runtime protected checks deferred",
    "capabilities / cdp-sessions / accounts will be checked after authentication",
    "",
    {
      area: "readiness",
      requirement:
        "受保护的 Runtime 账号与浏览器状态必须在登录后验收，不能在登录前误报。",
    },
  );
}
...
function normalizeDatabaseMode(value) {
  const text = String(value || "")
    .trim()
    .toLowerCase();
  if (text === "postgres" || text === "postgresql") return "postgres";
...
function resolveSqliteDatabasePath() {
  const userDataDir =
    localIntegrationEnv.KAYPAL_DESKTOP_USER_DATA_DIR ||
    process.env.KAYPAL_DESKTOP_USER_DATA_DIR ||
    inferDesktopUserDataDir();
  if (userDataDir) {
    return join(userDataDir, "kaypal-ai.sqlite");
  }
  return join(process.cwd(), "backend", "prisma", "dev.db");
}

async function loadCanonicalArticle(articleId) {
  ...
  try {
    const rows = await dbJson(`
      select
        ...
        title,
        content,
        final_html as "finalHtml",
        content_type as "contentType",
        content_format as "contentFormat",
        ${canonicalUpdatedAtSelectExpression()}
      from articles
      where id = ${sqlQuote(articleId)}
      limit 1;
    `);
    return rows[0] || null;
  } catch {
...
function canonicalUpdatedAtSelectExpression() {
  if (databaseMode === "postgres") {
    return `to_char(updated_at, 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"') as "updatedAt"`;
  }
  return `updated_at as "updatedAt"`;
}

function extractArticleBody(article) {
  return (
    (typeof article?.finalHtml === "string" && article.finalHtml) ||
...
function hasPublishableArticleBody(article) {
  return Boolean(extractArticleBody(article).trim());
}
...
function inferDesktopUserDataDir() {
  ...
  if (xdg) return `${xdg.replace(/[\\/]$/, "")}/ai-content-desktop`;
  const home = process.env.HOME?.trim();
  return home ? `${home.replace(/[\\/]$/, "")}/.config/ai-content-desktop` : "";
}
...
async function checkAuthentication() {
  ...
  const setup = await request("GET", "/auth/setup-status", undefined, {
    public: true,
  }).catch((error) => {
    record(
      "BLOCKED",
      "backend setup status unreachable",
      error.message,
      "启动后端服务，确认 API_BASE 指向 /api。",
      {
        area: "authentication",
  ...
  if (setup) {
    record(
      "PASS",
      "backend setup status reachable",
      `authMode=${setup.authMode || "unknown"}, hasUsers=${setup.hasUsers}`,
      "",
      {
        area: "authentication",
        requirement: "后台鉴权 setup 状态必须可达。",
  ...
  if (existingSession) {
    const me = await request("GET", "/auth/me").catch((error) => {
      record(
        "WARN",
        "reused local session rejected by backend",
        error.message,
        "页面登录态已失效时，请重新在 3010 完成 Kaypal 授权登录，或提供账号密码兜底。",
        {
          area: "authentication",
    ...
    if (me?.id) {
      record(
        "PASS",
        "reused existing Kaypal-backed 3010 session",
  ...
  if (derivedSession) {
    const me = await request("GET", "/auth/me").catch((error) => {
      record(
        "WARN",
        "derived Kaypal session rejected by backend",
        error.message,
        "确认 API_BASE 和 DATABASE_URL 指向同一个 3010 后端数据库。",
        {
          area: "authentication",
          requirement:
            "验收脚本可从已存在的真实 Kaypal 登录态派生短期脚本 session。",
        },
      );
      return null;
    });
    if (me?.id) {
      record(
        ...
        "",
        {
          area: "authentication",
          requirement:
            "脚本验收登录态必须来自已存在的真实 Kaypal 登录 session。",
        },
      );
      return true;
    }
  ...
  if (interactiveLoginEnabled) {
    ...
    if (interactiveSession) {
      const me = await request("GET", "/auth/me").catch((error) => {
        record(
          "WARN",
          "interactive Kaypal session rejected by backend",
          error.message,
          "重新在 3010 完成 Kaypal 授权登录，确认 API_BASE 与前端指向同一个后端。",
          {
            area: "authentication",
            requirement:
              "交互式 Kaypal 登录拿到的页面 cookie 必须能通过后台鉴权。",
          },
        );
        return null;
      });
      if (me?.id) {
        record(
          "PASS",
          "interactive Kaypal login succeeded",
          `${me.username || me.email || me.id}`,
          "",
          {
            area: "authentication",
      ...
      if (me?.id) {
        record(
          ...
          "",
          {
            area: "authentication",
            requirement:
              "本地验收可显式创建短期 3010 会话，不能默认绕过生产鉴权。",
          },
        );
        return true;
      }
  ...
  if (!username || !password) {
    record(
      ...
      "先设置 COMMERCIAL_INTERACTIVE_LOGIN=1 让脚本打开 Kaypal 授权登录；或传入 COMMERCIAL_COOKIE_HEADER/COMMERCIAL_SESSION_TOKEN；或设置 Kaypal 账号密码兜底。",
      {
        area: "authentication",
        requirement:
          "必须使用真实 Kaypal 登录态进入后台，不能把未登录当作通过。",
      },
    );
    return false;
  }
  ...
  const login = await request(
    ...
    record(
      "FAILED",
      "commercial account login failed",
      error.message,
      "确认账号密码、Kaypal 鉴权或本地管理员账号可用。",
      {
        area: "authentication",
  ...
  record(
    "PASS",
    "commercial account login succeeded",
    `${login.user.username || login.user.email || login.user.id}`,
    "",
    {
      area: "authentication",
      requirement: "真实后台账号必须能登录。",
  ...
  const me = await request("GET", "/auth/me").catch((error) => {
    record(
      "FAILED",
      "authenticated /auth/me failed",
      error.message,
      "检查登录 cookie、鉴权 guard 和 API_BASE 同源配置。",
      {
        area: "authentication",
...
async function createSessionFromExistingKaypalLogin() {
  ...
  const expiresAt = new Date(
    Math.min(
      sourceExpiresAt?.getTime() || defaultExpiresAt.getTime(),
      defaultExpiresAt.getTime(),
    ),
  ).toISOString();
...
function isReusableKaypalSession(row) {
  ...
  if (!metadata || metadata.source === "commercial-acceptance-gate")
    return false;
...
function reuseProvidedLoginState() {
  const reusableCookieHeader =
    providedCookieHeader || readCookieHeaderFromFile(providedCookieFile);
  if (reusableCookieHeader) {
    ...
    record(
      "PASS",
      "using provided browser cookie header",
      `cookieNames=${[...cookieJar.keys()].join(", ")}`,
      "",
      {
        area: "authentication",
  ...
  if (providedSessionToken) {
    ...
    record(
      "PASS",
      "using provided 3010 session token",
  ...
  record(
    "WARN",
    "no reusable browser login cookie supplied",
    `expected ${authCookieName} via COMMERCIAL_COOKIE_HEADER, COMMERCIAL_COOKIE_FILE, or COMMERCIAL_SESSION_TOKEN`,
    "如已在页面 Kaypal 登录，请把浏览器 cookie 传给脚本；否则脚本只能用账号密码兜底。",
    {
      area: "authentication",
      requirement:
        "页面登录态复用需要拿到浏览器 cookie，数据库 hash 不能反推出 cookie。",
    },
  );
  return false;
}
...
function readCookieHeaderFromFile(filePath) {
  ...
  if (!existsSync(filePath)) {
    record(
      "WARN",
      "provided cookie file missing",
      filePath,
      "确认 COMMERCIAL_COOKIE_FILE 指向当前 3010 登录态 cookie 文件。",
      {
        area: "authentication",
        requirement: "验收脚本可复用页面 Kaypal 登录态。",
      },
...
async function acquireInteractiveKaypalLogin() {
  ...
  try {
    ...
    console.log(
      `Opening ${frontendUrl}/login . Complete Kaypal authorization in the browser window.`,
    );
    browser = await loaded.chromium.launch({
      headless: interactiveLoginHeadless,
    });
    ...
    record(
      "PASS",
      "interactive Kaypal login cookie captured",
      `cookie=${authCookie.name}, playwright=${loaded.loadedFrom}`,
      "",
      {
        area: "authentication",
...
async function syncKaypalModels() {
  ...
  if (defaultAiReplyModelUsable) {
    record(
      "PASS",
      "default AI model already usable; Kaypal sync not forced",
      "已存在可用默认文本模型，本轮不强制覆盖。",
      "",
      {
        area: "readiness",
        requirement:
          "客户互动真实闭环必须能调用可用默认文本模型；Kaypal 同步失败不能覆盖可用本地模型。",
      },
    );
    return;
  }

  const status = await request("GET", "/ai-models/kaypal/status").catch(
    (error) => {
      record(
        "WARN",
        "Kaypal model sync status unavailable",
        error.message,
        "确认后端已部署 Kaypal 模型台同步接口。",
        {
          area: "readiness",
  ...
  const synced = await request("POST", "/ai-models/kaypal/sync", {}).catch(
    (error) => {
      record(
        "BLOCKED",
        "Kaypal model sync blocked",
        error.message,
        "完成 Kaypal 页面授权登录，并配置 KAYPAL_AI_PROXY_API_KEY/KAYPAL_API_KEY 后重试。",
        {
          area: "readiness",
          requirement:
            "客户互动真实闭环必须能调用 Kaypal 模型台提供的默认文本模型。",
        },
      );
      return null;
    },
  ...
  record(
    "PASS",
    "Kaypal model synced into 3010 defaults",
    synced.message || JSON.stringify(synced),
    "",
    {
      area: "readiness",
      requirement:
...
async function checkCdpBrowser() {
  ...
  const autoUploadHealth = await request("GET", "/auto-upload/health").catch(
    (error) => {
      record(
        "BLOCKED",
        "3011 Runtime offline",
        error.message,
        "启动 3011 后端 Runtime 后重试。",
        {
          area: "readiness",
...
async function readCdpSessions() {
  const backendSessions = await request(
    "GET",
    "/auto-upload/cdp-sessions",
  ).catch(() => null);
  if (backendSessions) {
    return {
  ...
  return Object.values(record)
    ...
    .filter(
      (entry) =>
        entry &&
        typeof entry === "object" &&
        ("status" in entry || "debuggingPort" in entry),
    );
}

async function checkReadOnlyCommercialPrerequisites() {
  section("Readiness, Accounts, Executors, Desktop");
  const shouldConfirmWechatSession =
    Boolean(wechatTargetContact) ||
    requiredTaskTypes.includes("wechat-reply-draft");
  if (shouldConfirmWechatSession) {
    await confirmWechatSessionForGate();
  }

  const runtime = await request("GET", "/local-engine/runtime/status").catch(
    (error) => {
      record(
        "FAILED",
        "runtime status unreachable",
        error.message,
        "运行 scripts/start-local-integration.sh 后重试。",
      );
      return null;
    },
  );
  if (runtime?.services) {
    ...
    if (offline.length) {
      record(
        ...
        offline
          .map((service) => `${service.key}:${service.message}`)
          .join(" | "),
        runtime.startScript ||
          "运行 scripts/start-local-integration.sh，并查看 .local-logs。",
      );
    } else {
      record(
        "PASS",
        "local runtime services online",
        runtime.services.map((service) => service.key).join(", "),
      );
    }
  }
  ...
  const readiness = await request(
    ...
    { timeoutMs: Math.max(timeoutMs, 60000) },
      ...
      record(
        ...
        error.message,
        "检查 /local-engine/readiness 返回结构。",
      );
      return null;
    },
  );
  if (readiness) {
    if (readiness.ready) {
      record(
        "PASS",
        "local engine readiness is green",
        JSON.stringify(readiness.summary || {}),
      );
    } else {
      record(
        "BLOCKED",
        ...
        firstNextAction(readiness.blockers) ||
          "按 readiness.blockers 逐项修复后重试。",
      );
    }
    if (Array.isArray(readiness.warnings) && readiness.warnings.length) {
      record(
        "WARN",
        "local engine readiness warnings",
        formatBlockers(readiness.warnings),
        firstNextAction(readiness.warnings),
      );
    }
  }
  ...
  const health = await request("GET", "/local-engine/health").catch((error) => {
    record(
      "FAILED",
      "local engine health unreachable",
      error.message,
      "确认本地引擎模块已注册。",
    );
    return null;
  ...
  if (health?.capabilities) {
    const commercial = health.capabilities.find(
      (item) => item.key === "permission-check",
    );
    const missing = health.capabilities.filter(
      (item) => item.status === "missing",
    );
    if (commercial) {
      record(
        commercial.status === "ready" ? "PASS" : "BLOCKED",
        "commercial permission capability",
        commercial.summary,
  ...
  const browserStatus = await request(
    "GET",
    "/local-engine/browser/status",
  ).catch((error) => {
    record(
      "FAILED",
      "browser account status unreachable",
      error.message,
      "确认 3011 Runtime 本地浏览器引擎在线。",
    );
    return null;
  });
  if (browserStatus) {
    if (!browserStatus.engineOnline) {
      record(
        ...
        browserStatus.engineMessage,
        browserStatus.recovery?.nextAction,
      );
      ...
      record(
        "PASS",
        "ready real platform accounts found",
        describeAccounts(browserStatus.accounts),
      );
    }
  }

  const executors = await request(
    "GET",
  ...
  if (executors?.executors) {
    for (const type of requiredTaskTypes) {
      ...
      if (!executor) {
        record(
          "BLOCKED",
          `executor missing from matrix: ${type}`,
          "接口未返回该执行器。",
          "补齐执行器状态定义后重试。",
        );
        ...
        record(
          "BLOCKED",
          `executor not ready: ${executor.name}`,
          executor.message,
          executor.nextAction,
        );
      } else {
  ...
  const desktop = await request("GET", "/local-engine/desktop/status").catch(
    (error) => {
      record(
        "FAILED",
        "desktop status unreachable",
        error.message,
        "确认桌面控制接口可用。",
      );
      return null;
  ...
  if (desktop) {
    const blockedPermissions = (desktop.permissionChecks || []).filter(
      (item) => item.status === "blocked",
    );
    if (!desktop.running || !desktop.available) {
      record(
        "BLOCKED",
        "desktop WeChat unavailable",
        desktop.message,
        desktop.nextAction || "打开桌面微信并授予辅助功能/屏幕录制权限。",
      );
      ...
      record(
        "PASS",
        "desktop WeChat control preflight passed",
        desktop.message,
      );
    }
  }

  const desktopPreflight = await request(
    "GET",
    "/local-engine/desktop/preflight",
  ).catch((error) => {
    record(
      "FAILED",
      "desktop commercial preflight unreachable",
      error.message,
      "确认 /local-engine/desktop/preflight 接口已注册。",
    );
    return null;
  });
  if (desktopPreflight) {
    ...
    const checks = Array.isArray(desktopPreflight.checks)
      ? desktopPreflight.checks
      : [];
    ...
    if (missingKeys.length) {
      ...
      record(
        ...
        [
          ...(desktopPreflight.blockers || []),
          ...blockedChecks.map((check) => `${check.label}:${check.message}`),
        ].join(" | ") || desktopPreflight.message,
        desktopPreflight.nextAction ||
          "修复桌面权限、窗口、截图或输入/点击能力后重试。",
      );
    } else if (
      !desktopPreflight.screenshot ||
      !desktopPreflight.takeoverReady ||
      !desktopPreflight.stopReady
    ) {
      record(
        "FAILED",
      ...
      record(
        "PASS",
        "desktop commercial preflight passed",
        desktopPreflight.message,
      );
    }
  }

  const fileAccess = await request("GET", "/local-engine/files/status").catch(
    (error) => {
      record(
        "FAILED",
        "file-selection status unreachable",
        error.message,
        "确认 /local-engine/files/status 接口可用。",
        {
...
async function confirmWechatSessionForGate() {
  ...
  await request("POST", "/local-engine/wechat/session/confirm", {
    ...
    record(
      "FAILED",
      "WeChat session confirmation failed",
      error.message,
      "检查微信会话确认接口和桌面状态。",
    );
    return null;
  ...
  if (!status.canDraft) {
    record(
      ...
      [...(status.blockers || []), ...(status.warnings || [])].join(" | ") ||
        "canDraft=false",
  ...
  record(
    "PASS",
    "WeChat session locked for draft-only execution",
    status.lock?.message || status.targetContact || wechatTargetContact,
  );
}
...
async function runBatchTaskMatrixCheck() {
  ...
  const task = await request("POST", "/local-engine/customers/tasks", {
    ...
    record(
      "FAILED",
      "batch task creation failed",
      error.message,
      "检查批量任务创建接口和 batchTargets 结构。",
      {
        area: "batch-tasks",
  ...
  const latest = await waitForTask(task.id, [
    "completed",
    "failed",
    "skipped",
    "no_target",
  ...
  const targetCount = Array.isArray(latest.batchTargets)
    ? latest.batchTargets.length
    : 0;
  if (
    targetCount !== batchTargets.length ||
    summary.total !== batchTargets.length
  ) {
    record(
      "FAILED",
    ...
    recordClassifiedTaskFailure(
      "Batch customer follow-up",
      latest,
      "batch-tasks",
    );
  } else {
  ...
  if (diagnostics) {
    if (
      diagnosticTargets.length !== batchTargets.length ||
      diagnosticSummary.total !== batchTargets.length
    ) {
      record(
        "FAILED",
      ...
      record(
        "PASS",
        "batch diagnostics export includes target matrix",
        `task=${task.id}, targets=${diagnosticTargets.length}`,
        "",
        {
          area: "batch-tasks",
...
async function runPublishingCenterCheck() {
  ...
  const unifiedAccounts = await request(
    "GET",
    "/publishing/accounts?validate=1&force=1",
    undefined,
    {
      timeoutMs: Math.max(timeoutMs, 45000),
    },
  ).catch((error) => {
    recordClassifiedAccessIssue(
      "unified publishing account validation",
      error,
      "修复 /publishing/accounts，确保能从 3011 Runtime 同步本地账号进 3010。",
      {
        area: "publishing-center",
  ...
  const health = await request("GET", "/auto-upload/health").catch((error) => {
    recordClassifiedAccessIssue(
      "publishing center health",
      error,
      "启动 3011 Runtime 并确认 /auto-upload/health 可达。",
      {
        area: "publishing-center",
  ...
  if (health) {
    record(
      "PASS",
      "publishing center engine health reachable",
      `${health.service || "auto-upload"} ${health.version || ""}`.trim(),
      "",
      {
        area: "publishing-center",
  ...
  const accounts = await request(
    "GET",
    "/auto-upload/accounts?validate=1",
    undefined,
    {
    ...
    recordClassifiedAccessIssue(
      ...
      error,
      "登录真实发布测试账号或修复 3011 Runtime 账号接口。",
      {
        area: "publishing-center",
  ...
  if (Array.isArray(accounts)) {
    ...
    if (!ready.length) {
      ...
      record(
        "PASS",
        "publishing center ready accounts found",
        `ready=${ready.length}, total=${accounts.length}`,
        "",
        {
          area: "publishing-center",
  ...
  const materials = await request("GET", "/auto-upload/materials").catch(
    (error) => {
      recordClassifiedAccessIssue(
        "publishing material list",
        error,
        "上传或选择真实测试素材，或修复素材接口。",
        {
          area: "publishing-center",
  ...
  if (Array.isArray(materials)) {
    if (!materials.length && !publishMaterialFile) {
      ...
      record(
        "PASS",
        "publishing center materials visible",
        `materials=${materials.length}, override=${Boolean(publishMaterialFile)}`,
        "",
        {
          area: "publishing-center",
  ...
  await ensurePublishingSourceArticle();
...
async function ensurePublishingSourceArticle() {
  if (hasPublishableArticleBody(latestPublishableArticle)) {
    const canonical = await loadCanonicalArticle(latestPublishableArticle.id);
    if (hasPublishableArticleBody(canonical)) {
      latestPublishableArticle = canonical;
    }
    return latestPublishableArticle;
  }

  const topics = unwrapItems(
    await request("GET", "/topics?limit=5&sortBy=date-desc").catch(() => null),
  );
  const topic = topics.find((item) => item?.id);
  if (!topic?.id) {
    return latestPublishableArticle;
  }
  ...
  const generated = await request(
    "POST",
    `/articles/${encodeURIComponent(topic.id)}/generate?force=true&contentType=article`,
    undefined,
    { timeoutMs: contentGenerateTimeoutMs },
  ).catch(() => null);
  if (generated?.id) {
    latestPublishableArticle =
      (await loadCanonicalArticle(generated.id)) || generated;
    record(
      "PASS",
      "publishing source article seeded from latest topic",
      `topic=${topic.id}, article=${generated.id}`,
      "",
      {
        area: "publishing-center",
        requirement: "发布中心校验需要至少一篇可发布的真实正文。",
      },
    );
  }
...
async function syncPublishingAccountsFromLocalEngine() {
  ...
  const accounts = await request(
    ...
    recordClassifiedAccessIssue(
      "publishing accounts sync failed",
      error,
      "修复 /publishing/accounts 或 3011 Runtime 账号接口。",
      {
        area: "publishing-center",
        requirement:
          "3010 必须把 3011 Runtime 本地账号同步为可审计 publish_accounts。",
      },
    );
    return null;
  });
...
async function verifyUnifiedPublishingAccounts(accounts) {
  const localAccounts = accounts.filter(
    (account) =>
      ...
      String(account.id || "").startsWith("local-engine:"),
  );
  ...
  if (!localAccounts.length) {
    record(
      ...
      "确认 3011 Runtime 有账号，并修复 /publishing/accounts 同步。",
      {
        area: "publishing-center",
        requirement:
          "页面读取到的 Runtime 账号必须同步到 3010 publish_accounts。",
      },
    );
    return false;
  }
...
async function verifyPublishRiskGuard(payload) {
  ...
  const guarded = await request(
    "POST",
    "/auto-upload/publish",
    { payloads: [realPublishProbePayload] },
    { timeoutMs: Math.max(timeoutMs, publishTimeoutMs) },
  ).then(
    (value) => ({ ok: true, value }),
    (error) => ({ ok: false, error }),
  );
  if (guarded.ok) {
    if (guarded.value?.riskAudit?.target === "auto-upload-publish:dry-run") {
      record(
        "PASS",
        "publish direct submit stayed in dry-run guard",
        JSON.stringify(guarded.value || {}),
        "未确认发布被安全降级为 dry-run，没有形成真实平台提交。",
        {
          area: "permission-escape",
          requirement: "发布动作缺少人工确认时必须被风控或 dry-run 保护。",
        },
      );
      return;
    }
  ...
  if (isRiskConfirmationError(guarded.error)) {
    record(
      "PASS",
      "publish direct submit blocked by backend risk gate",
  ...
  recordClassifiedAccessIssue(
    "publish direct submit failed before risk gate",
    guarded.error,
    "补齐账号、素材、3011 Runtime 后重跑；若仍非风控错误则排查发布接口。",
    {
      area: "publishing-center",
...
async function verifyPublishConfirmationSession(payload) {
  const session = await request("POST", "/local-engine/agent-sessions", {
    ...
    instruction:
      "商业验收：发布中心真实提交必须停在待我确认，不允许绕过审批直接发布。",
    source: "publishing",
    ...
    record(
      "FAILED",
      "publishing confirmation session creation failed",
      error.message,
      "检查 Agent 会话 resumeAction=auto-upload-publish。",
      {
        area: "publishing-center",
  ...
  record(
    "PASS",
    "publishing action parked in confirmation queue",
    `session=${session.id}, confirmation=${confirmation.id}`,
    "",
    {
      area: "publishing-center",
  ...
  const queue = await request(
    "GET",
    `/local-engine/confirmations?status=pending&sessionId=${encodeURIComponent(session.id)}`,
  ).catch((error) => {
    record(
      "FAILED",
      "publishing confirmation query failed",
      error.message,
      "检查确认队列查询接口。",
      {
        area: "publishing-center",
  ...
  if (
    Array.isArray(queue) &&
    queue.some((item) => item.id === confirmation.id)
  ) {
    record(
      "PASS",
      "publishing confirmation queryable by session",
    ...
    record(
      "FAILED",
      "publishing confirmation not returned by query",
  ...
  const rejected = await request(
    "POST",
    `/local-engine/confirmations/${confirmation.id}/reject`,
    {
      operator,
    ...
    record(
      "FAILED",
      "publishing confirmation reject failed",
      error.message,
      "检查发布确认拒绝接口。",
      {
        area: "publishing-center",
  ...
  if (rejected?.status === "cancelled") {
    record(
      "PASS",
      "publishing confirmation reject cancels session",
      `session=${session.id}`,
      "",
      {
        area: "publishing-center",
    ...
    record(
      "FAILED",
      "publishing confirmation reject did not cancel session",
...
async function verifyRealPublishSubmission(payload) {
  const placeholderFiles = [
    ...
    /commercial-acceptance-missing-(material|account)/.test(
      String(filePath || ""),
    ),
  );
  if (placeholderFiles.length) {
    record(
      ...
      "发布中心必须提供真实账号文件和素材文件；如果 Runtime 已有账号/素材但没有 filePath，需要修复账号/素材同步字段。",
      {
        area: "publishing-center",
        requirement:
          "真实发布提交必须使用真实账号文件和素材文件，不能用占位路径。",
      },
    );
    return;
  }
  const confirmation = await createPublishRiskConfirmation(payload);
  if (!confirmation?.confirmationId) {
    record(
      "FAILED",
      "real publish confirmation creation failed",
      JSON.stringify(confirmation || {}),
      "发布提交前必须先签发一次性确认编号。",
      {
        area: "publishing-center",
        requirement: "真实发布提交必须先获得确认编号。",
      },
    );
    return;
  }
  const result = await request(
    ...
    recordClassifiedAccessIssue(
      "real publish submission failed",
      error,
      "如果账号/素材/3011 Runtime 缺失则补齐；如果接口结构错误则修复发布接口。",
      {
        area: "publishing-center",
        requirement:
          "显式授权后真实发布接口必须返回 taskIds/results/riskAudit。",
      },
    );
    return null;
  });
  ...
  const hasStructuredResult =
    ...
    Array.isArray(result.batchResult?.platforms);
  if (
    !result.riskAudit ||
    !hasStructuredResult
  ) {
    record(
      ...
      "发布提交必须返回可审计的风控信息和平台回执；平台返回 blocked 时也应保留结构化结果。",
      {
        area: "publishing-center",
  ...
  if (!confirmations.length) {
    const blockedPlatforms = Array.isArray(result.platforms)
      ? result.platforms.filter((entry) => entry?.status && entry.status !== "success")
      : [];
    if (blockedPlatforms.length || result.summary?.blocked || result.summary?.notIntegrated) {
      record(
        "BLOCKED",
        "real publish blocked by platform/runtime gate",
        JSON.stringify(result),
        "补齐平台账号权限、页面状态、云端积分冻结或平台回读后再重跑。",
        {
          area: "publishing-center",
          requirement: "真实发布受平台或运行时阻断时必须明确 BLOCKED。",
        },
      );
      return;
    }
  ...
  record(
    "PASS",
    "real publish submission returned platform confirmation",
    confirmations.join(" | "),
    "",
    {
      area: "publishing-center",
...
async function createPublishRiskConfirmation(payload) {
  const confirmation = await request(
    ...
    record(
      "FAILED",
      "publish confirmation creation failed",
      error.message,
      "检查发布确认签发接口。",
      {
        area: "publishing-center",
        requirement: "真实发布前必须先签发确认编号。",
      },
    );
    return null;
  });
  if (confirmation?.confirmationId) {
    record(
      "PASS",
      "publish confirmation issued",
      `confirmation=${confirmation.confirmationId}`,
      "",
      {
        area: "publishing-center",
        requirement: "真实发布提交前必须先签发确认编号。",
      },
    );
  }
  return confirmation;
...
async function runContentToPublishingCheck() {
  ...
  const [materials, topics, articles, accounts] = await Promise.all([
    request("GET", "/materials?limit=10").catch((error) => {
      recordClassifiedAccessIssue(
        "content pipeline materials query failed",
        error,
        "修复素材列表接口。",
        {
    ...
    request("GET", "/topics?limit=10&sortBy=date-desc").catch((error) => {
      recordClassifiedAccessIssue(
        "content pipeline topics query failed",
        error,
        "修复选题列表接口。",
        {
          area: "content-pipeline",
    ...
    request("GET", "/articles?limit=10").catch((error) => {
      recordClassifiedAccessIssue(
        "content pipeline articles query failed",
        error,
        "修复文章列表接口。",
        {
          area: "content-pipeline",
    ...
    request("GET", "/publishing/accounts?validate=1&force=1", undefined, {
      ...
      recordClassifiedAccessIssue(
        "content pipeline publish account query failed",
        error,
        "修复发布账号统一接口。",
        {
          area: "content-pipeline",
  ...
  const topicItems = [...seededTopics, ...unwrapItems(topics)].filter(
    (topic, index, all) =>
      topic?.id && all.findIndex((item) => item?.id === topic.id) === index,
  );
  const articleItems = unwrapItems(articles);
  const accountItems = Array.isArray(accounts)
    ? accounts
    : unwrapItems(accounts);

  if (materialItems.length) {
    record(
      "PASS",
      "content pipeline materials readable",
    ...
    record(
      "BLOCKED",
      "content pipeline has no materials",
      "materials=0",
      "先准备真实素材或完成素材采集。",
      {
        area: "content-pipeline",
  ...
  if (topicItems.length) {
    record(
      "PASS",
      "content pipeline topics readable",
    ...
    record(
      "BLOCKED",
      "content pipeline has no topics",
      "topics=0",
      "先完成选题挖掘或创建测试选题。",
      {
        area: "content-pipeline",
  ...
  const publishableArticles = articleItems.filter(
    (article) =>
      article?.id &&
      ["article", "xiaohongshu"].includes(
        String(article.contentType || article.content_type || "article"),
      ),
  );
  if (publishableArticles.length && !latestPublishableArticle) {
    latestPublishableArticle =
      (await loadCanonicalArticle(publishableArticles[0].id)) ||
      publishableArticles[0];
  }
  if (publishableArticles.length) {
    record(
      "PASS",
      "content pipeline articles readable",
      `articles=${publishableArticles.length}`,
      "",
      {
        area: "content-pipeline",
    ...
    record(
      "WARN",
      "content pipeline has no existing publishable articles",
      "articles=0; will generate in real cycles",
      "继续执行真实内容生成闭环。",
      {
        area: "content-pipeline",
        requirement: "内容链路必须能产生可进入发布中心的文章/笔记。",
    ...
    record(
      "BLOCKED",
      "content pipeline has no publishable articles",
      "articles=0",
      "先生成文章或小红书笔记。",
      {
        area: "content-pipeline",
  ...
  const readyAccounts = accountItems.filter((account) =>
    isReadyAccount(account),
  );
  if (readyAccounts.length && !latestReadyAccount) {
    latestReadyAccount = readyAccounts[0];
  }
  if (readyAccounts.length) {
    record(
      "PASS",
      "content pipeline publish accounts ready",
    ...
    record(
      "BLOCKED",
      "content pipeline has no ready publish account",
...
async function runContentPipelineCycles(topicItems, readyAccounts) {
  ...
  for (const [index, topic] of topics.entries()) {
    const article = await request(
      ...
      record(
        "FAILED",
        `content pipeline generate failed #${index + 1}`,
        error.message,
        "排查文章/小红书生成接口和默认模型。",
        {
          area: "content-pipeline",
    ...
    latestPublishableArticle =
      (await loadCanonicalArticle(article.id)) || article;
    ...
    const imported = await request(
      ...
      recordClassifiedAccessIssue(
        `content pipeline material import failed #${index + 1}`,
        error,
        "给生成内容补齐可发布素材，或修复导入接口。",
        {
          area: "content-pipeline",
    ...
    if (imported?.imported?.length || imported?.files?.length) {
      ...
      record(
        "PASS",
        `content pipeline cycle ready #${index + 1}`,
        `article=${article.id}, imported=${imported.imported?.length || imported.files?.length}`,
        "",
        {
          area: "content-pipeline",
      ...
      record(
        "BLOCKED",
        `content pipeline cycle has no importable material #${index + 1}`,
        `article=${article.id}`,
        "生成内容缺本地可上传素材，不能算进入发布中心。",
        {
          area: "content-pipeline",
          requirement: "每条链路必须生成内容并进入发布中心素材池。",
        },
  ...
  if (completed === contentPipelineCycles) {
    record(
      "PASS",
      "content to publishing 5-cycle loop passed",
      `cycles=${completed}/${contentPipelineCycles}`,
      "",
      {
        area: "content-pipeline",
    ...
    record(
      "FAILED",
      "content to publishing 5-cycle loop incomplete",
      `cycles=${completed}/${contentPipelineCycles}`,
      "修复失败轮次后重跑。",
      {
        area: "content-pipeline",
...
async function ensureContentPipelineSeedData() {
  ...
  const topics = unwrapItems(
    await request(
      "GET",
      `/topics?limit=${contentPipelineCycles}&sortBy=date-desc`,
    ).catch(() => null),
  );
  if (topics.length >= contentPipelineCycles)
    return topics.slice(0, contentPipelineCycles);

  const materials = unwrapItems(
    await request(
      "GET",
      `/materials?limit=${Math.max(contentPipelineCycles, 10)}`,
    ).catch(() => null),
  );
  ...
  for (let index = 0; index < missing; index += 1) {
    ...
    await request("POST", "/topics", {
      title,
      description:
        "用于 KaypalAI 内容创作平台商用验收的内容生产到发布中心链路。",
      ...
      (topic) => {
        ...
        record(
          "PASS",
          "content pipeline seed topic created",
      ...
      (error) => {
        recordClassifiedAccessIssue(
          "content pipeline seed topic creation failed",
          error,
          "修复选题创建接口或素材关联后重跑。",
          {
            area: "content-pipeline",
...
async function runUnauthorizedMatrixCheck() {
  ...
  const checks = [
    {
      ...
      body: {
        payloads: [buildPublishPayload([], [], { debugDryRun: false })],
        confirmationId: "commercial-unauthorized-publish",
      },
      expected: "risk",
    ...
    },
    {
      ...
      body: {
        operator,
        reason: "commercial acceptance unauthorized takeover probe",
      },
      expected: "risk-or-blocked",
  ...
  for (const check of checks) {
    ...
    if (
      isRiskConfirmationError(result.error) ||
      (check.expected === "risk-or-blocked" &&
        isBlockedAccessError(result.error))
    ) {
      record(
        "PASS",
        `unauthorized action blocked: ${check.label}`,
...
async function checkCommercialRiskGate() {
  ...
  const session = await request("POST", "/local-engine/agent-sessions", {
    ...
    instruction:
      "商业验收：申请远程接管桌面微信，准备群发/删除/发送内容；必须进入审批并由脚本拒绝。",
    ...
    record(
      "FAILED",
      "risk gate session creation failed",
      error.message,
      "检查 Agent 会话和权限风控接口。",
    );
    return null;
  ...
  if (!confirmation) {
    record(
      "FAILED",
      "high-risk session did not stop at confirmation",
  ...
  const checkKeys = new Set(
    (confirmation.requiredChecks || []).map((item) => item.key),
  );
  const requiredChecks = [
    ...
    "misfire-protection",
    "double-confirmation",
    "remote-takeover-audit",
  ];
  const missing = requiredChecks.filter((key) => !checkKeys.has(key));
  if (missing.length) {
    record(
      "FAILED",
      "risk confirmation missing required checks",
      missing.join(", "),
      "补齐权限风控检查项后重试。",
    );
    ...
    record(
      "FAILED",
      "remote takeover audit not required",
      JSON.stringify(confirmation.riskPolicy || {}),
      "远程/接管类动作必须写入审计策略。",
    );
  } else {
    record(
      "PASS",
      "high-risk action blocked by confirmation and audit",
      `confirmation=${confirmation.id}`,
    );
  }
  ...
  const rejected = await request(
    ...
    record(
      "FAILED",
      "risk confirmation reject failed",
      error.message,
      "检查确认拒绝接口。",
    );
    return null;
  ...
  if (rejected?.status === "cancelled") {
    record(
      "PASS",
      "risk confirmation reject cancels session",
      `session=${session.id}`,
    );
  } else if (rejected) {
    record(
      "FAILED",
      "risk confirmation reject did not cancel session",
      `status=${rejected.status}`,
      "拒绝高风险动作后会话必须停止。",
    );
  }
...
async function runFailureRecoveryCheck() {
  ...
  const failed = await request("POST", `/local-engine/tasks/${task.id}/fail`, {
    ...
    record(
      "FAILED",
      "mark recovery task failed failed",
      error.message,
      "检查任务失败回写接口。",
    );
    return null;
  ...
  if (failed?.status !== "failed") {
    record(
      "FAILED",
      "recovery task did not enter failed status",
  ...
  const retry = await request(
    "POST",
    `/local-engine/tasks/${task.id}/retry`,
    {},
  ).catch((error) => {
    record(
      "FAILED",
      "retry recovery task failed",
      error.message,
      "检查任务重试接口。",
    );
    return null;
  ...
  record(
    "PASS",
    "failure recovery creates retry task",
    `failed=${task.id}, retry=${retry.id}`,
  );
...
async function runEvidenceAuditCheck() {
  ...
  const records = await request(
    "GET",
    "/local-engine/records/export?limit=200",
  ).catch((error) => {
    record(
      "FAILED",
      "interaction records export failed",
      error.message,
      "检查执行记录导出接口。",
    );
    return null;
  ...
  if (records?.content && records?.filename) {
    record(
      "PASS",
      "interaction records export returns audit file",
      `${records.filename}, bytes=${records.content.length}`,
    );
  } else if (records) {
    record(
      "FAILED",
      "interaction records export missing content",
  ...
  const cleanupPreview = await request(
    "GET",
    "/local-engine/evidence/cleanup-preview?retentionDays=7",
  ).catch((error) => {
    record(
      "FAILED",
      "evidence cleanup preview failed",
      error.message,
      "检查证据治理接口。",
    );
    return null;
  ...
  if (cleanupPreview && Number.isInteger(cleanupPreview.candidateCount)) {
    record(
      "PASS",
      "evidence cleanup preview is auditable",
      `candidate=${cleanupPreview.candidateCount}, bytes=${cleanupPreview.totalBytes || 0}`,
    );
  } else if (cleanupPreview) {
    record(
      "FAILED",
      "evidence cleanup preview missing counts",
      JSON.stringify(cleanupPreview),
      "清理预览不能直接删除，必须先返回候选清单。",
    );
  }
}
...
async function runPressureCheck() {
  ...
  for (let index = 0; index < pressureCycles; index += 1) {
    const session = await request("POST", "/local-engine/agent-sessions", {
      ...
      record(
        "FAILED",
        `pressure session create failed #${index + 1}`,
        error.message,
        "检查连续创建 Agent 会话时的稳定性。",
      );
      return null;
    });
    ...
    if (!confirmation) {
      record(
        "FAILED",
        `pressure session missing confirmation #${index + 1}`,
        `session=${session.id}, status=${session.status}`,
        "dryRun 压测会话必须进入待确认。",
      );
      continue;
    ...
    if (pressureApprove) {
      await approveAgentConfirmation(
        confirmation.id,
        `commercial pressure approve #${index + 1}`,
      );
  ...
  if (sessions.length !== pressureCycles) {
    record(
      "FAILED",
      "pressure cycle count mismatch",
      `expected=${pressureCycles}, created=${sessions.length}`,
      "排查连续创建会话失败的接口或资源限制。",
    );
  } else {
    record(
      "PASS",
      "pressure cycles completed without fake success",
      `cycles=${sessions.length}, approve=${pressureApprove}`,
    );
  }

  const pending = await request(
    "GET",
    "/local-engine/confirmations?status=pending",
  ).catch(() => null);
  if (Array.isArray(pending)) {
    const leaked = pending.filter((item) =>
      sessions.some((session) => session.id === item.sessionId),
    );
    if (leaked.length) {
      record(
        "FAILED",
        "pressure confirmations leaked pending state",
        leaked.map((item) => item.id).join(", "),
        "压测结束后本轮确认不应遗留 pending。",
      );
    } else {
      record(
        "PASS",
        "pressure confirmations drained",
...
async function runRealExecutionChecks() {
  ...
  const douyinLeads = await loadRealExecutionLeads("douyin");
  const readyAccounts = (browserStatus.accounts || []).filter(
  ...
  for (const type of requiredTaskTypes) {
    const account =
      platformTypeForTask(type) === 2 ? wechatAccount : douyinAccount;
    const lead =
      platformTypeForTask(type) === 3
        ? pickRealExecutionLead(douyinLeads, type)
        : null;
    if (!account) {
      record(
        "BLOCKED",
    ...
    await runOneRealTask(type, account, lead);
  }
  ...
  if (!realAutoSendEnabled || !approveAutoSendEnabled) {
    record(
      ...
      "确认四个入口都有测试账号、测试评论/私信对象，并允许真实发送后，同时设置 COMMERCIAL_REAL_AUTO_SEND=1 与 COMMERCIAL_APPROVE_AUTO_SEND=1。",
      {
        area: "real-execution",
        requirement:
          "四条客户互动真实自动发送闭环必须显式授权，每条至少连续 5 轮。",
      },
    );
    return;
  }
  ...
  for (const type of requiredTaskTypes) {
    ...
    const lead =
      platformTypeForTask(type) === 3
        ...
        : null;
    if (!account) continue;
    await runRealAutoSendLoop(type, account, lead);
  }
}

async function runOneRealTask(type, account, lead = null) {
  const route = taskRoute(type);
  const label = taskLabel(type);
  const targetPayload = buildRealExecutionTargetPayload(type, account, lead);
  ...
  const preflight = await waitForTask(
    task.id,
    [
      "waiting_for_send_confirmation",
      "completed",
    ...
    { attempts: realExecutionPollAttempts },
  );
  if (!preflight) return;
  ...
  if (evidenceCount < 1) {
    record(
      "FAILED",
      `${label} missing preflight evidence`,
      `task=${task.id}`,
      "真实执行必须留下入口、读取或失败证据。",
    );
  } else {
    record(
      "PASS",
      `${label} preflight evidence captured`,
      `task=${task.id}, evidence=${evidenceCount}`,
    );
  }
  ...
  if (preflight.status === "no_target") {
    record(
      "BLOCKED",
      `${label} has no real target`,
      preflight.resultSummary?.detail || preflight.nextAction || "no_target",
      preflight.nextAction || "给测试账号准备一条可处理评论/私信/会话后重试。",
    );
    return;
  }
  ...
  if (preflight.status === "skipped") {
    record(
      "BLOCKED",
      `${label} skipped by real executor`,
      preflight.nextAction || "skipped",
      "确认测试对象未被规则跳过后重试。",
    );
    return;
  }
  ...
  if (preflight.status === "completed") {
    record(
      "FAILED",
      `${label} completed before approval`,
      `task=${task.id}`,
      "真实发送/填草稿动作必须先进入人工确认。",
    );
    return;
  ...
  if (preflight.status !== "waiting_for_send_confirmation") {
    record(
      "FAILED",
      `${label} unexpected preflight status`,
      `status=${preflight.status}`,
      "真实任务必须停在待确认或明确失败/BLOCKED。",
    );
    return;
  }
  ...
  const approved = await request(
    "POST",
    `/local-engine/tasks/${task.id}/approve`,
    {
      operator,
      ...
      draftBeforeFillConfirmed:
        type === "wechat-reply-draft" ? true : undefined,
      targetContact:
        type === "wechat-reply-draft" ? wechatTargetContact : undefined,
      targetConfirmed: true,
      contentConfirmed: true,
      checklistConfirmed: true,
      commercialPermissionConfirmed: true,
      misfireProtectionConfirmed: true,
      doubleConfirmationConfirmed:
        preflight.requiresDoubleConfirmation === true,
      riskConfirmation: {
        confirmed: true,
  ...
  if (finalTask.status === "completed" && countTaskEvidence(finalTask) > 0) {
    ...
    record(
      "PASS",
      `${label} real read/draft execution completed`,
      `task=${task.id}, evidence=${countTaskEvidence(finalTask)}`,
    );
    return;
  }
  if (finalTask.status === "no_target") {
    record(
      "BLOCKED",
      `${label} final status no target`,
      `task=${task.id}`,
      finalTask.nextAction || "给测试账号准备一条可处理评论/私信/会话后重试。",
    );
    return;
  }
  ...
  record(
    "FAILED",
    `${label} final status not accepted`,
    `status=${finalTask.status}`,
    finalTask.nextAction || "排查真实执行链路。",
  );
}

async function runRealAutoSendLoop(type, account, lead = null) {
  const label = taskLabel(type);
  let completed = 0;
  for (let index = 0; index < realAutoSendCycles; index += 1) {
    ...
    const task = await request("POST", taskRoute(type), {
      ...
      record(
        "FAILED",
        `${label} auto-send task creation failed #${index + 1}`,
        error.message,
        "检查真实账号、测试对象和本地执行器。",
      );
      return null;
    ...
    if (finalTask.status === "completed" && countTaskEvidence(finalTask) > 0) {
      ...
      const realInputOk = verifyRealCustomerInput(
        finalTask,
        `${label} auto-send #${index + 1}`,
      );
      const readbackOk = verifySendReadback(
        finalTask,
        `${label} auto-send #${index + 1}`,
      );
      const aiGenerated = verifyReplyGeneratedByAi(
        finalTask,
        `${label} auto-send #${index + 1}`,
      );
  ...
  if (completed === realAutoSendCycles) {
    record(
      "PASS",
      `${label} real auto-send loop passed`,
      `cycles=${completed}/${realAutoSendCycles}`,
      "",
      {
        area: "real-execution",
    ...
    record(
      "BLOCKED",
      `${label} real auto-send loop incomplete`,
      `cycles=${completed}/${realAutoSendCycles}`,
      "补齐真实测试对象、平台页面状态或 AI 鉴权后重跑，不能把部分通过当商用完成。",
      {
        area: "real-execution",
        requirement: "每个客户互动入口至少连续 5 轮真实自动发送闭环。",
      },
...
async function loadRealExecutionLeads(platform) {
  const leads = await request(
    "GET",
    `/growth/leads?platform=${encodeURIComponent(platform)}&status=all`,
  ).catch(() => null);
  return Array.isArray(leads) ? leads : unwrapItems(leads);
}

function pickRealExecutionLead(leads, type) {
  const items = Array.isArray(leads) ? leads : [];
  if (!items.length) return null;
  const directMessage = type.includes("direct-message");
  const scored = items
    .filter(
      (lead) =>
        lead &&
        typeof lead.sourceText === "string" &&
        lead.sourceText.trim(),
    )
    .map((lead) => {
      const nickname = String(lead.nickname || "").trim();
      const sourceText = String(lead.sourceText || "").trim();
      let score = Number(lead.score || 0);
      if (/[？?]/.test(sourceText)) score += 25;
      if (sourceText.length > 4) score += 10;
      if (/微信|私信|联系|怎么|多少/.test(sourceText)) score += 15;
      if (/验收|测试|placeholder|demo|mock/i.test(nickname + sourceText)) {
        score -= 60;
      }
      if (directMessage && /微信|私信|联系/.test(sourceText)) score += 10;
      return { lead, score };
    })
    .sort((left, right) => right.score - left.score);
  return scored[0]?.lead || items[0] || null;
}

function buildRealExecutionTargetPayload(type, account, lead, options = {}) {
  const accountName =
    account.displayName ||
    account.profileName ||
    account.userName ||
  ...
  const targetName =
    type === "wechat-reply-draft"
      ? wechatTargetContact || "商业验收微信测试对象"
      : lead?.nickname || "商业验收真实测试对象";
  const sourceText =
    lead?.sourceText ||
    (type === "wechat-reply-draft"
      ? wechatTargetContact || "商业验收微信测试对象"
  ...
  return {
    ...
    metadata:
      options.cycleIndex !== undefined
        ? {
            commercialCycleIndex: options.cycleIndex + 1,
            commercialTargetLeadId: lead?.id || undefined,
            commercialTargetNickname: lead?.nickname || undefined,
          }
        : undefined,
  };
}
...
function verifyReplyGeneratedByAi(task, label) {
  if (task.replyGeneratedBy === "ai") {
    record(
      "PASS",
      `${label} reply generated by AI`,
      `replyGeneratedBy=${task.replyGeneratedBy}`,
      "",
      {
        area: "real-execution",
        requirement:
          "真实客户互动闭环必须由 AI 按真实客户内容生成回复；规则兜底不能计入通过。",
      },
    );
    return true;
  }
...
function verifyCdpRuntimeMode(task, label) {
  ...
  if (runtimeMode === "persistent-cdp-browser") {
    record(
      "PASS",
      `${label} uses CDP persistent browser`,
  ...
  record(
    "WARN",
    `${label} runtimeMode not confirmed as CDP`,
...
function verifySendReadback(task, label) {
  const replyText = normalizeProofText(
    task.replyText || task.resultSummary?.replyText,
  );
  if (!replyText) {
    record(
      "FAILED",
      `${label} missing reply text for readback check`,
      `task=${task.id || "-"}`,
      "真实自动发送必须记录发送内容，才能回读比对。",
      {
        area: "real-execution",
  ...
  const readbackProofs = proofTexts.filter((text) =>
    isReadbackProofForReply(text, replyText),
  );
  if (sendProofs.length > 0) {
    if (readbackProofs.length > 0) {
      record(
        "PASS",
        `${label} has send and reply-matched readback evidence`,
        `send=${sendProofs.length}, readback=${readbackProofs.length}`,
        "",
        {
          area: "real-execution",
      ...
      record(
        "FAILED",
        `${label} has send but no reply-matched readback evidence`,
        `send=${sendProofs.length}, reply=${replyText.slice(0, 80)}`,
        "发送后必须有包含实际回复内容的回读确认证据。",
        {
          area: "real-execution",
          requirement:
            "发送后应有回读确认，不能只靠 editorCleared/replyVisible。",
        },
      );
      return false;
    }
...
function verifyRealCustomerInput(task, label) {
  const sourceText = normalizeProofText(
    task.sourceText || task.resultSummary?.sourceText,
  );
  const replyText = normalizeProofText(
    task.replyText || task.resultSummary?.replyText,
  );
  const fakeSourcePattern =
    /商业验收|请忽略|等待本机读取真实对象|commercial acceptance/i;
  ...
  record(
    "PASS",
    `${label} used real customer input`,
    `sourceLength=${sourceText.length}, replyLength=${replyText.length}`,
    "",
    {
      area: "real-execution",
...
function isSendProof(text) {
  ...
  if (
    /editorCleared|replyVisible/i.test(normalized) &&
    !/发送|sent|submit|publish/i.test(normalized)
  ) {
    return false;
  }
  ...
  return /已发送|发送成功|点击发送|submit.*success|send\s*(ok|success|confirmed)/i.test(
    normalized,
  );
}
...
function isReadbackProofForReply(text, replyText) {
  ...
  if (
    /editorCleared|replyVisible/i.test(normalized) &&
    !/回读|readback|页面看到|确认|confirmed|platform/i.test(normalized)
  ) {
    return false;
  }
  return /回读|readback|页面看到|确认|confirmed|platform|已在页面|last message/i.test(
    normalized,
  );
}
...
function verifyCommercialBoundary(task, label) {
  ...
  if (
    !boundary.commercialExecutionAllowed ||
    boundary.permissionStatus !== "allowed"
  ) {
    const statusAllowed =
      boundary.permissionStatus === "allowed" ||
      boundary.permissionStatus === "approval_required";
    if (boundary.commercialExecutionAllowed && statusAllowed) {
      record(
        "PASS",
        `${label} commercial executable permission allowed`,
        `permission=${boundary.permissionStatus}; ${boundary.message || ""}`,
      );
      return true;
    }
    record(
...
function recordClassifiedTaskFailure(label, task, area = "real-execution") {
  const reason =
    ...
    "unknown failure";
  const isBlocked =
    /未登录|登录失效|账号|权限|执行器|本地引擎|桌面|微信|没有可处理|no target|扫码|401|403|仍在加载|未进入可回复状态|规则兜底|AI 模型|AI 按真实客户内容|鉴权|Unauthorized|云端积分|积分冻结|billing reserve|runtime billing reserve/i.test(
      reason,
    );
  record(
        ...
        : "排查业务执行错误后重试。"),
    {
      area,
      requirement:
        "真实任务失败必须按账号/权限/测试对象阻断与接口/业务错误区分。",
    },
  );
}
...
async function approveAgentConfirmation(id, note) {
  return request("POST", `/local-engine/confirmations/${id}/approve`, {
    ...
    record(
      "FAILED",
      `agent confirmation approve failed: ${id}`,
      error.message,
      "检查 Agent 确认审批接口。",
    );
    return null;
...
async function waitForTask(id, statuses, options = {}) {
  return waitFor(
    async () => {
...
async function waitFor(producer, label, options = {}) {
  const attempts = Math.max(1, Number(options.attempts || pollAttempts));
  let lastError;
  ...
  record(
    "FAILED",
    `${label} timed out`,
    lastError?.message || `attempts=${attempts}`,
    "查看任务/会话记录和 .local-logs 后重试。",
  );
  return null;
}
...
async function exportTaskDiagnostics(taskId, label) {
  const exported = await request(
    "GET",
    `/local-engine/tasks/${taskId}/diagnostics/export`,
  ).catch((error) => {
    record(
      "FAILED",
      `task diagnostics export failed: ${label}`,
      error.message,
      "检查诊断包导出接口。",
    );
    return null;
  ...
  if (
    !exported.mimeType?.toLowerCase().startsWith("application/json") ||
    content?.task?.id !== taskId
  ) {
    record(
      "FAILED",
      `task diagnostics export invalid: ${label}`,
      JSON.stringify({
        filename: exported.filename,
        mimeType: exported.mimeType,
      }),
      "诊断包必须是 JSON 且包含 task.id。",
    );
    return null;
  }
  const hasAuditFields =
    content.task?.safetyBoundary &&
    content.task?.riskPolicy &&
    content.task?.failureAnalysis;
  if (!hasAuditFields) {
    record(
      "FAILED",
      `task diagnostics missing audit fields: ${label}`,
      `task=${taskId}`,
      "诊断包必须包含 safetyBoundary、riskPolicy、failureAnalysis。",
    );
    return null;
  }
  artifacts.exportedDiagnostics.push({
    taskId,
    label,
    filename: exported.filename,
    evidenceCount: content.task?.diagnostics?.evidenceCount || 0,
  });
  record(
    "PASS",
    `task diagnostics export valid: ${label}`,
    `${exported.filename}`,
  );
  return content;
}
...
async function exportAgentEvidence(sessionId, label) {
  const exported = await request(
    "GET",
    `/local-engine/agent-sessions/${sessionId}/evidence/export`,
  ).catch((error) => {
    record(
      "FAILED",
      `agent evidence export failed: ${label}`,
      error.message,
      "检查 Agent 证据导出接口。",
    );
    return null;
  ...
  if (
    ...
    record(
      "FAILED",
      `agent evidence export invalid: ${label}`,
      JSON.stringify({
        filename: exported.filename,
        mimeType: exported.mimeType,
      }),
      "证据包必须是 JSON 且包含 session.id。",
    );
    return null;
  }
  if (
    !Array.isArray(content.evidence) ||
    !Array.isArray(content.replay?.timeline)
  ) {
    record(
      "FAILED",
      `agent evidence export missing replay fields: ${label}`,
      `session=${sessionId}`,
      "证据包必须包含 evidence 和 replay.timeline。",
    );
    return null;
  }
  artifacts.exportedEvidence.push({
    sessionId,
    label,
    filename: exported.filename,
    evidenceCount: content.evidence.length,
    timelineCount: content.replay.timeline.length,
  });
  record(
    "PASS",
    `agent evidence export valid: ${label}`,
    `${exported.filename}, evidence=${content.evidence.length}`,
  );
  return content;
}
...
function pickAccount(accounts, explicitId, platformType) {
  const normalizedPlatformType = Number(platformType) || 0;
  if (explicitId) {
    const directMatch = accounts.find((account) =>
      accountMatchesExplicitRef(account, explicitId),
    );
    const resolvedType = directMatch
      ? resolvePublishAccountType(directMatch)
      : 0;
    if (
      directMatch &&
      (!normalizedPlatformType ||
        resolvedType === 0 ||
        resolvedType === normalizedPlatformType)
    ) {
      return directMatch;
    }
  }
  if (normalizedPlatformType) {
    const platformMatch = accounts.find(
      (account) => resolvePublishAccountType(account) === normalizedPlatformType,
    );
  ...
  return accounts.find(
    (account) =>
      !normalizedPlatformType ||
      resolvePublishAccountType(account) === normalizedPlatformType,
  );
}

function accountMatchesExplicitRef(account, explicitRef) {
  const target = normalizeAccountRef(explicitRef);
  if (!target) return false;
  return accountIdentityValues(account).some(
    (value) => normalizeAccountRef(value) === target,
  );
}

function accountIdentityValues(account) {
  const config = parsePublishAccountConfig(account);
  return [
    account?.id,
    account?.stableId,
    account?.accountId,
...
function isReadyAccount(account) {
  ...
  const statusLabel = String(
    account?.statusLabel ?? account?.status_label ?? "",
  ).toLowerCase();
  return (
    account?.status === 1 ||
    [
...
function resolvePublishAccountType(account) {
  const config = parsePublishAccountConfig(account);
  const candidates = [
    account?.type,
  ...
  for (const candidate of candidates) {
    const resolved = resolvePublishPlatformType(candidate);
    if (resolved > 0) return resolved;
  }
  return 0;
}

function resolvePublishPlatformType(value) {
...
function taskRoute(type) {
  const routes = {
    ...
    "wechat-channel-direct-message-reply":
      "/local-engine/channel-messages/tasks",
...
function collectPublishConfirmations(result) {
  ...
  for (const entry of candidates) {
    ...
    const externalId =
      entry?.externalId || evidence.externalId || entry?.articleId;
    ...
    if (
      ...
      confirmations.push(
        [
          entry.platform || entry.type || "platform",
          externalId || publishUrl || source,
        ]
          .filter(Boolean)
          .join(":"),
      );
  ...
  if (
    result?.evidence?.externalId ||
    result?.evidence?.publishUrl ||
    result?.publishUrl
  ) {
    confirmations.push(
      result.evidence.externalId ||
...
function buildPublishPayload(accounts = [], materials = [], options = {}) {
  const requestedAccountRef = normalizePublishAccountRef(publishAccountFile);
  const preferredPublishType = resolvePreferredPublishType();
  const readyAccounts = Array.isArray(accounts)
    ? accounts.filter((account) => isReadyAccount(account))
    : [];
  const selectedAccount = requestedAccountRef
    ? findMatchingPublishAccount(readyAccounts, requestedAccountRef) ||
      findMatchingPublishAccount(readyAccounts, normalizePublishAccountRef(latestReadyAccount?.filePath)) ||
      findMatchingPublishAccount(readyAccounts, normalizePublishAccountRef(latestReadyAccount?.id)) ||
      null
    : pickAccount(readyAccounts, undefined, preferredPublishType) ||
      findMatchingPublishAccount(
        readyAccounts,
        normalizePublishAccountRef(latestReadyAccount?.filePath),
      ) ||
      findMatchingPublishAccount(
        readyAccounts,
        normalizePublishAccountRef(latestReadyAccount?.id),
      ) ||
      readyAccounts[0] ||
      latestReadyAccount ||
      null;
  const material = Array.isArray(materials)
    ? materials.find((item) =>
        isPublishableImageFile(item.filePath || item.filepath || item.path),
      )
    : null;
  const sourceArticle = latestPublishableArticle || null;
  const sourceBody =
    extractArticleBody(sourceArticle) || "commercial acceptance source body";
  const fallbackMaterialFile = resolveFallbackPublishMaterialFile();
  const fileList = [
    publishMaterialFile ||
      material?.filePath ||
      material?.filepath ||
      material?.path ||
      fallbackMaterialFile ||
      "/tmp/commercial-acceptance-missing-material.png",
  ];
  const resolvedAccountFile =
    normalizePublishAccountRef(publishAccountFile) ||
    normalizePublishAccountRef(selectedAccount?.filePath) ||
    normalizePublishAccountRef(selectedAccount?.path) ||
    normalizePublishAccountRef(selectedAccount?.accountFile) ||
    "/tmp/commercial-acceptance-missing-account.json";
  const selectedAccountConfig = parsePublishAccountConfig(selectedAccount);
  const selectedEngineAccountId = Number(
    selectedAccountConfig.engineAccountId ||
      selectedAccount?.engineAccountId ||
      selectedAccount?.id ||
      0,
  );
  const accountIds =
    Number.isInteger(selectedEngineAccountId) && selectedEngineAccountId > 0
      ? [selectedEngineAccountId]
      : [];
  const payloadType =
    resolvePublishAccountType(selectedAccount) || preferredPublishType;
  const accountList = [resolvedAccountFile];
  const isVideo = fileList.some((filePath) =>
    /\.(mp4|mov|m4v|avi|webm)$/i.test(filePath),
  );
  const articleId = sourceArticle?.id || "";
  const articleUpdatedAt = sourceArticle?.updatedAt
    ? new Date(sourceArticle.updatedAt).toISOString()
  ...
  return {
    ...
    accountIdentity: selectedAccount
      ? {
          ...
          name:
            ...
            `account-${selectedAccount.id || basename(resolvedAccountFile)}`,
          platform:
            selectedAccount.platform ||
            platformNameForPublishType(payloadType),
          status: String(selectedAccount.status || "ready"),
        }
      : undefined,
    body: sourceBody,
    sourceIdentity: articleId
      ? {
          sourceType: "article",
          sourceId: articleId,
          title: sourceArticle?.title || "commercial-acceptance-source",
          contentType:
            sourceArticle?.contentType ||
            sourceArticle?.content_type ||
            "article",
          contentFormat:
            sourceArticle?.contentFormat ||
            sourceArticle?.content_format ||
            "markdown",
    ...
    timeJitterMinutes: 0,
    debugDryRun:
      options.debugDryRun ?? !(realPublishEnabled && approvePublishEnabled),
    debugDryRunHoldBrowser:
      options.debugDryRunHoldBrowser ??
      !(realPublishEnabled && approvePublishEnabled),
    category: 0,
  };
}

function normalizePublishAccountRef(value) {
  const text = String(value || "").trim();
  if (!text) return "";
  return basename(text);
}

function findMatchingPublishAccount(accounts, ref) {
  if (!ref) return null;
  const normalizedRef = normalizePublishAccountRef(ref);
  return (
    accounts.find((account) => {
      ...
      const candidates = [
        ...
        .filter(Boolean)
        .map((item) => normalizePublishAccountRef(item));
      return candidates.includes(normalizedRef);
    }) || null
  );
}

function parsePublishAccountConfig(account) {
...
function platformNameForPublishType(type) {
  const names = {
    1: "xiaohongshu",
    2: "wechat-channel",
    3: "douyin",
...
function resolvePreferredPublishType() {
  const parsed = Number(process.env.COMMERCIAL_PUBLISH_TYPE || 3);
  return Number.isInteger(parsed) && parsed > 0 ? parsed : 3;
}

function isPublishableImageFile(filePath) {
  return /\.(png|jpe?g|webp|gif)$/i.test(String(filePath || ""));
}

function resolveFallbackPublishMaterialFile() {
  const roots = [
    process.cwd(),
    join(process.cwd(), "..", "New project", "ai-content"),
    join(process.cwd(), "..", "ai-content"),
  ];
  const filenames = [
    "account_1.png",
    "account_2.png",
  ...
  for (const root of roots) {
    for (const filename of filenames) {
      const candidate = join(root, "data", "avatars", filename);
...
function verifyFileSelectionMatrix(fileAccess) {
  ...
  const missingRoots = requiredRoots.filter(
    (key) => !roots.some((root) => root.key === key),
  );
  const blockedRoots = roots.filter(
    (root) =>
      requiredRoots.includes(root.key) && (!root.exists || !root.readable),
  );
  ...
  if (blockedRoots.length) {
    record(
      ...
      blockedRoots
        .map(
          (root) =>
            `${root.key}:exists=${root.exists},readable=${root.readable},path=${root.path}`,
        )
        .join(" | "),
...
function isBlockedAccessError(error) {
  ...
  return /未登录|登录|账号|cookie|Cookie|扫码|授权|权限|permission|forbidden|unauthori[sz]ed|不可访问|未启动|Runtime|runtime|素材.*不存在|素材.*不可读|请选择|缺少|不存在或不可读/i.test(
    text,
  );
}
...
function isRiskConfirmationError(error) {
  const text = `${error?.message || ""} ${JSON.stringify(error?.body || {})}`;
  return /风控|人工确认|确认动作|风险|approval_required|riskAudit|禁止动作|阻断|角色|SUPER_ADMIN|ADMIN|套餐|升级|确认编号|confirmationId|一次性确认/i.test(
    text,
  );
}
...
function matrixFor(item) {
  return {
    ...
    requirement:
      item.requirement || defaultRequirement(inferMatrixArea(item.label)),
  };
}
...
function inferMatrixArea(label = "") {
  ...
  if (
    /content|article|topic|material|xiaohongshu|素材|选题|文章|小红书/.test(
      text,
    )
  )
  ...
  if (/risk|unauthorized|permission|takeover|风控|越权/.test(text))
    return "permission-escape";
  ...
  if (/readiness|runtime|executor|browser|account|capabilit/.test(text))
    return "readiness";
  return "general";
}
...
function defaultRequirement(area) {
  const requirements = {
    authentication:
      "必须使用真实 Kaypal 登录态进入后台，缺页面 cookie、交互式登录或账号密码兜底为 BLOCKED。",
    readiness: "真实账号、执行器、桌面权限和本地引擎必须 ready。",
    "batch-tasks": "批量任务必须创建真实任务记录并导出逐对象矩阵。",
    "content-pipeline":
      "素材采集、选题、内容生成、进入发布中心必须连续 5 条验收。",
    "publishing-center": "发布中心必须验证账号、素材、确认队列和真实提交边界。",
    "desktop-file-selection":
      "桌面控制必须覆盖 file-selection 权限与素材/账号/日志路径。",
    ...
    "real-execution":
      "真实账号执行缺账号/权限/对象为 BLOCKED，接口错为 FAILED。",
  ...
  return [
    {
      ...
      nextAction:
        "设置 COMMERCIAL_INTERACTIVE_LOGIN=1 走 Kaypal 页面授权；或传入页面 cookie/session token；本地验收可显式设置 COMMERCIAL_LOCAL_ACCEPTANCE_LOGIN=1。",
    },
    {
      key: "realExecutionSwitch",
      label: "真实执行开关",
      ready: realExecutionEnabled,
      detail: realAcceptanceEnabled
        ? "COMMERCIAL_REAL_ACCEPTANCE=1"
        : `COMMERCIAL_REAL_EXECUTION=${realExecutionEnabled ? "1" : "<unset>"}`,
    ...
    },
    {
      ...
      detail: realAcceptanceEnabled
        ? "COMMERCIAL_REAL_ACCEPTANCE=1"
        : `COMMERCIAL_APPROVE_DRAFTS=${approveDraftsEnabled ? "1" : "<unset>"}`,
      requiredFor: ["real-execution"],
      nextAction:
        "确认当前窗口和对象为测试环境后设置 COMMERCIAL_APPROVE_DRAFTS=1。",
    },
    {
      ...
      detail: realAcceptanceEnabled
        ? `COMMERCIAL_REAL_ACCEPTANCE=1, cycles=${realAutoSendCycles}`
        : `COMMERCIAL_REAL_AUTO_SEND=${realAutoSendEnabled ? "1" : "<unset>"}, COMMERCIAL_APPROVE_AUTO_SEND=${approveAutoSendEnabled ? "1" : "<unset>"}, cycles=${realAutoSendCycles}`,
      requiredFor: ["real-execution"],
      nextAction:
        "确认四个入口都有真实测试对象并允许真实发送后，同时设置 COMMERCIAL_REAL_AUTO_SEND=1 与 COMMERCIAL_APPROVE_AUTO_SEND=1。",
    },
    {
      ...
      ready:
        !requiredTaskTypes.includes("wechat-reply-draft") ||
        Boolean(wechatTargetContact),
      detail: wechatTargetContact
        ? `target=${maskValue(wechatTargetContact)}`
        : "COMMERCIAL_WECHAT_TARGET_CONTACT 未设置",
    ...
    },
    {
      ...
      nextAction:
        "设置 COMMERCIAL_PUBLISH_ACCOUNT_FILE 与 COMMERCIAL_PUBLISH_MATERIAL_FILE，或在发布中心准备可读真实测试账号和素材。",
    },
    {
      ...
      detail: realAcceptanceEnabled
        ? `COMMERCIAL_REAL_ACCEPTANCE=1, cycles=${contentPipelineCycles}`
        : `COMMERCIAL_REAL_CONTENT_PIPELINE=${realContentPipelineEnabled ? "1" : "<unset>"}, COMMERCIAL_APPROVE_CONTENT_PIPELINE=${approveContentPipelineEnabled ? "1" : "<unset>"}, cycles=${contentPipelineCycles}`,
    ...
    },
    {
      ...
      nextAction:
        "确认使用真实测试发布后同时设置 COMMERCIAL_REAL_PUBLISH=1 与 COMMERCIAL_APPROVE_PUBLISH=1。",
    },
  ];
}
...
function printInputChecklist() {
  ...
  console.log(
    `Input checklist: READY=${checklist.length - missing.length} MISSING=${missing.length}`,
  );
...
function record(status, label, detail = "", nextAction = "", matrix = {}) {
  ...
  const suffix = [item.detail, item.nextAction ? `next=${item.nextAction}` : ""]
    .filter(Boolean)
    .join(" | ");
...
async function writeReport() {
  ...
  const reportPath = join(
    reportDir,
    `commercial-acceptance-${timestampForFile()}.json`,
  );
  const report = {
...
function printSummary() {
  console.log("");
  console.log(
    `Summary: PASS=${passCount} WARN=${warnCount} BLOCKED=${blockedCount} FAILED=${failCount}`,
  );
  ...
  for (const cell of matrix.filter((item) => item.area !== "general")) {
    console.log(
      `- ${cell.status} ${cell.area}: PASS=${cell.pass} WARN=${cell.warn} BLOCKED=${cell.blocked} FAILED=${cell.failed}`,
    );
    if (
      cell.status === "FAILED" ||
      cell.status === "BLOCKED" ||
...
function formatBlockers(items = []) {
  ...
  return items
    .map(
      (item) =>
        `${item.capability || item.label || item.key || "item"}:${item.message || item.summary || item.nextAction || ""}`,
    )
    .join(" | ");
}
...
function firstNextAction(items = []) {
  return Array.isArray(items)
    ? items.map((item) => item.nextAction).find(Boolean)
    : "";
}
...
function describeAccounts(accounts = []) {
  return accounts
    ...
    .map(
      (account) => `${account.id}:${account.platform}:${account.displayName}`,
    )
    .join(" | ");
}
