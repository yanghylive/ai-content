...
@Injectable()
export class KaypalModelSyncService implements OnApplicationBootstrap {
  ...
  async getStatus(
    ...
    if (!auth) {
      return {
        ...
        source: 'missing',
        message:
          '本地未配置 Kaypal 默认模型，也没有可复用的 Kaypal 登录态/API Key。',
        nextAction:
    ...
    if (!provider?.defaultModel) {
      return {
        ...
        nextAction:
          '先在 Kaypal 模型台启用一个带默认模型的 provider，再回到 3010 同步。',
      };
    }
    ...
    return {
      ...
      nextAction:
        '执行 /ai-models/kaypal/sync，把 Kaypal 默认模型落到 3010 本地默认模型配置。',
    };
  }
  ...
  private async getLocalKaypalDefault(): Promise<KaypalModelSyncStatus> {
    ...
    const modelIds = [
      ...new Set(rows.map((row) => row.modelId).filter(Boolean)),
    ];
    if (!modelIds.length) {
      return {
        configured: false,
  ...
  private async resolveKaypalAuth(
    request?: Request,
  ...
  private async fetchKaypalStatus(
    auth: KaypalAuthContext,
  ): Promise<KaypalStatusPayload> {
    const adminStatus = await this.fetchKaypalAdminStatus(auth).catch(
      (error) => {
        this.logger.warn(
    ...
    let chatModels: KaypalChatModelsPayload | null = null;
    try {
      chatModels = await this.fetchKaypalChatModels(auth);
    } catch (error) {
      this.logger.warn(
      ...
      const fallbackStatus = this.getEnvFallbackStatus();
      if (fallbackStatus) {
        this.logger.warn(
          'Kaypal model list unavailable; using configured fallback model.',
        );
  ...
  private async fetchKaypalAdminStatus(
    ...
    const response = await fetch(
      new URL('/api/admin/ai-service/status', baseUrl),
      {
        method: 'GET',
        headers: {
        ...
        signal: AbortSignal.timeout(
          Number(
            this.config.get<string>('KAYPAL_MODEL_SYNC_TIMEOUT_MS') || 10000,
          ),
        ),
      },
    );
  ...
  private async fetchKaypalChatModels(
    ...
    if (
      !chatPayload ||
      (!Array.isArray(chatPayload.models) && !chatPayload.defaultModel)
    ) {
      throw new ServiceUnavailableException(
        'Kaypal 普通模型列表返回结构不完整。',
      );
    }
    return chatPayload;
  }
