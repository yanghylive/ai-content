...
export class CrawlProcessor {
  ...
  async process(job: { data?: any } & Record<string, any>): Promise<any> {
    ...
    try {
      ...
      // 保存结果（去重）
      const { savedCount, createdMaterialIds } =
        await this.rssCrawler.saveResults(results);
      ...
      this.logger.log(
        `采集任务完成: ${sourceName}, 获取 ${results.length} 条, 新增 ${savedCount} 条`,
      );
      await this.systemLogsService.record(
        `✅ 渠道【${sourceName}】采集完成: 共拉取素材 ${results.length} 篇，入库 ${savedCount} 篇`,
        'success',
      );
