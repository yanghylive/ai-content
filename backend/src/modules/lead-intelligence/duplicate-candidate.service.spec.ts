  ...
  it('merge：迁移事件/内容 + 合并字段 + 删除 source（可撤销审计 id）', async () => {
    const prisma = makePrisma({
      platformIdentity: {
        findUnique: jest
          .fn()
          .mockResolvedValueOnce({ id: 'target-1', tenantId: 't1', nickname: 'A', profileUrl: null, avatarHash: null, verified: false, identityConfidence: 50, firstSeenAt: new Date('2026-01-01'), lastSeenAt: new Date('2026-01-02') })
          .mockResolvedValueOnce({ id: 'source-1', tenantId: 't1', nickname: 'B', profileUrl: 'https://x', avatarHash: 'h', verified: true, identityConfidence: 90, firstSeenAt: new Date('2025-12-01'), lastSeenAt: new Date('2026-02-01') }),
    ...
    expect(r.auditId).toContain('dup-merge');
