...
export class RealContentTools {
  ...
  build(): BusinessToolRegistry {
    ...
    r.register('content_generate', async (ctx, req) => {
      const p = req.payload ?? {};
      const title = String(p.title ?? '未命名内容').slice(0, 120);
      const content = String(p.content ?? '');
      if (!content.trim()) {
        throw makeError('INVALID_PLAN', { details: { toolName: 'content_generate', reason: 'content 不能为空（禁止伪造内容）' } });
      }
      const article = await this.prisma.article.create({
        data: {
          title,
          content,
          contentType: p.contentType === 'xiaohongshu' ? 'xiaohongshu' : 'article',
          contentFormat: p.contentFormat === 'html' ? 'html' : 'markdown',
          workspaceBrief: (p.brief ? { brief: String(p.brief) } : {}) as never,
      ...
      const exec: ToolExecution = {
        data: { contentId: article.id, title: article.title, version: 1, checksum, contentType: article.contentType },
        evidence: [],
        usage: { inputTokens: Math.max(1, Math.ceil(content.length / 4)), modelTokens: 0, computeUnits: 1, usageId: `cg_${article.id.slice(-8)}` },
        status: 'succeeded',
        artifacts: [{ type: 'content_draft', uri: `/api/articles/${article.id}`, checksum, version: 1 }],
      };
      return exec;
    });
    ...
    r.register('content_review', async (ctx, req) => {
      const p = req.payload ?? {};
      const content = String(p.content ?? '');
      const titles = Array.isArray(p.titles) ? p.titles.map(String) : [String(p.title ?? '未命名')];
      if (!content.trim()) {
        throw makeError('INVALID_PLAN', { details: { toolName: 'content_review', reason: 'content 不能为空' } });
      }
      const result = await this.reviewService.reviewAndRevise({
        ...
        aiFlavorScore: p.aiFlavorScore != null ? Number(p.aiFlavorScore) : undefined,
      });
      if (!result.review.pass) {
        throw makeError('CONTENT_REVIEW_FAILED', {
          details: { score: result.review.score, threshold: 70, issues: result.review.issues },
        });
      }
      const exec: ToolExecution = {
        data: {
        ...
        evidence: [],
        usage: { inputTokens: Math.ceil(content.length / 4), modelTokens: 0, computeUnits: 1, usageId: `cr_${Date.now().toString(36)}` },
        status: 'succeeded',
      };
      return exec;
    });
    ...
    r.register('lead_normalize', async (ctx, req) => {
      const p = req.payload ?? {};
      const rawLeads = Array.isArray(p.leads) ? (p.leads as Array<Record<string, unknown>>) : [];
      const normalized = rawLeads.map((raw) => {
        const phone = normalizePhone(String(raw.phone ?? raw.mobile ?? ''));
        const wechat = normalizeWechat(String(raw.wechat ?? raw.wx ?? ''));
        const externalUserId = String(raw.externalUserId ?? raw.userId ?? '').trim();
        const nickname = String(raw.nickname ?? '').trim();
        const sourceUrl = String(raw.sourceUrl ?? '').trim();
        const confidence = computeConfidence({ phone, wechat, externalUserId, nickname, sourceUrl });
        return {
          phone,
          wechat,
          externalUserId,
      ...
      const exec: ToolExecution = {
        ...
        evidence: [],
        usage: { inputTokens: normalized.length * 20, modelTokens: 0, computeUnits: 1, usageId: `ln_${Date.now().toString(36)}` },
        status: 'succeeded',
      };
      return exec;
    });
    ...
    r.register('publish_execute', async () => {
      throw makeError('TOOL_EXECUTION_FAILED', {
        details: { reason: 'publish_execute 未接入真实 RPA 发布（禁止假成功，需 RPA 平台适配器）' },
      });
    });
    r.register('interaction_reply_execute', async () => {
      throw makeError('TOOL_EXECUTION_FAILED', {
        details: { reason: 'interaction_reply_execute 未接入真实 RPA 互动（禁止假成功，需 RPA 平台适配器）' },
      });
    });
...
export function leadDedupeKey(input: { phone?: string; wechat?: string; externalUserId?: string; nickname?: string }): string {
...
/** 置信度：标识强度 + 数量 */
export function computeConfidence(input: { phone: string; wechat: string; externalUserId: string; nickname: string; sourceUrl: string }): number {
  let score = 0;
