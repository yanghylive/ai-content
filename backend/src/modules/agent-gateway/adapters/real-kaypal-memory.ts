...
export class RealKaypalMemoryAdapter implements KaypalMemoryAdapter {
  ...
  private async request(
    ...
    try {
      try {
        ...
        throw makeError('MEMORY_TIMEOUT', {
          details: { context: 'network', reason },
        });
      }
    } finally {
      clearTimeout(timer);
  ...
  async search(
    ...
    const res = await this.request(
      `/api/memory?${p.toString()}`,
      undefined,
      accessToken,
    );
    const body = (await this.ensureOk(res, 'search')) as {
      items?: Array<Record<string, unknown>>;
    };
    const items = (body.items ?? []).map((i) => ({
      id: String(i.id),
      namespace: nsKey(ns),
      content: String((i.content as string) ?? ''),
      scope: ns.scope,
      source: String(
        ((i.metadata as Record<string, unknown> | undefined)
          ?.source as string) ?? 'remote',
      ),
      createdAt: String((i.createdAt as string) ?? ''),
      metadata:
        (i.metadata as Record<string, unknown> | undefined) ?? undefined,
    }));
  ...
  async add(
    ...
    if (!body.id)
      throw makeError('MEMORY_REJECTED', {
        details: { context: 'add', reason: '响应缺少 id' },
      });
  ...
  async delete(
    ...
    if (!id) return false;
    const res = await this.request(
      `/api/memory/long?ids=${encodeURIComponent(id)}`,
      { method: 'DELETE' },
      accessToken,
    );
  ...
  async export(
    ...
    const res = await this.request(
      '/api/memory/list?tier=long&limit=100',
      undefined,
      accessToken,
    );
    const body = (await this.ensureOk(res, 'export')) as {
      items?: Array<Record<string, unknown>>;
    };
