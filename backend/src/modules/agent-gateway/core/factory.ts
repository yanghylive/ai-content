...
export function createAgentGateway(
  opts: {
    ...
    idempotency?: {
      claim: (...a: unknown[]) => unknown;
      markDone: (...a: unknown[]) => unknown;
    ...
    approvals?: {
      ...
      consume: (...a: unknown[]) => unknown;
      reject?: (...a: unknown[]) => unknown;
    };
  ...
  const idempotency = (opts.idempotency ??
    new IdempotencyStore()) as IdempotencyStore;
  const approvals = (opts.approvals ??
    new ApprovalService()) as ApprovalService;
  ...
  const octop =
    opts.octop ?? new MockOctopAdapter(registry.list().map((s) => s.name));
