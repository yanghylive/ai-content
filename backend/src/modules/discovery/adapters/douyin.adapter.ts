...
export class DouyinAdapter implements DiscoveryAdapter {
  ...
  capabilities(): Promise<DiscoveryCapability> {
    const browserReady =
      this.config.browserEnabled !== false && Boolean(this.runner);
  ...
  async *discover(
    ...
    if (mode === 'video-link') {
      ...
      yield {
        ...
        sourceContent: {
          externalContentId:
            url.split('/').filter(Boolean).pop() ?? 'douyin-link',
          url,
          contentType: 'video',
          rawHash,
        },
    ...
    if (!this.runner) {
      throw new BrowserDiscoverError(
        'no_browser_session',
        '抖音浏览器会话未启用（无官方 API 授权，需浏览器会话进行发现）',
      );
    }
