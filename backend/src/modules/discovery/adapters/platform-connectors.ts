...
import { emptyAsyncIterable } from './async-iterable';
import type {
  DiscoveryCapability,
  DiscoveryContext,
  DiscoveryInput,
...
export class ShipinhaoAdapter implements DiscoveryAdapter {
  ...
  discover(
    ...
    throw new Error(
      'unsupported: 视频号无官方授权时仅支持人工链接导入（VideoLinkAdapter）',
    );
  }

  fetchContent(ref: ExternalContentRef, _ctx: DiscoveryContext) {
    return Promise.resolve({
      externalContentId: ref.externalContentId ?? ref.url ?? 'shipinhao',
      url: ref.url ?? '',
      contentType: 'video',
      rawHash: `sp-${ref.url ?? ref.externalContentId ?? ''}`,
    });
  }
...
export class WecomAdapter implements DiscoveryAdapter {
  ...
  fetchContent(ref: ExternalContentRef, _ctx: DiscoveryContext) {
    return Promise.resolve({
      externalContentId: ref.externalContentId ?? ref.url ?? 'wecom',
      url: ref.url ?? '',
      contentType: 'message',
      rawHash: `wecom-${ref.url ?? ref.externalContentId ?? ''}`,
    });
  }
...
export class KuaishouAdapter implements DiscoveryAdapter {
  ...
  capabilities(): Promise<DiscoveryCapability> {
    ...
    return Promise.resolve<DiscoveryCapability>({
      platform: 'kuaishou',
      modes: browserReady
        ? ['keyword', 'video-link', 'target-account']
        : ['video-link'],
      ...
      ...(browserReady
        ...
        : {
            unavailableReason:
              '快手需浏览器会话（用户登录态）或官方授权。未启用时仅支持人工链接导入。',
          }),
    });
  }
  ...
  async *discover(
    ...
    if (input.mode === 'keyword') {
      const keyword =
        typeof input.input?.keyword === 'string' ? input.input.keyword : '';
    ...
    if (input.mode === 'target-account') {
      const targetId =
        typeof input.input?.targetId === 'string' ? input.input.targetId : '';
  ...
  fetchContent(ref: ExternalContentRef, _ctx: DiscoveryContext) {
    return Promise.resolve({
      externalContentId: ref.externalContentId ?? ref.url ?? 'kuaishou',
      url: ref.url ?? '',
      contentType: 'video',
      rawHash: `ks-${ref.url ?? ref.externalContentId ?? ''}`,
    });
  }
...
export class XiaohongshuAdapter implements DiscoveryAdapter {
  ...
  capabilities(): Promise<DiscoveryCapability> {
    ...
    return Promise.resolve<DiscoveryCapability>({
      ...
      ...(browserReady
        ...
        : {
            unavailableReason:
              '小红书需浏览器会话（用户登录态）或官方 connector 接入；未启用时仅支持人工链接导入。',
          }),
    });
  }
  ...
  fetchContent(ref: ExternalContentRef, _ctx: DiscoveryContext) {
    return Promise.resolve({
      externalContentId: ref.externalContentId ?? ref.url ?? 'xhs',
      url: ref.url ?? '',
      contentType: 'note',
      rawHash: `xhs-${ref.url ?? ref.externalContentId ?? ''}`,
    });
  }
