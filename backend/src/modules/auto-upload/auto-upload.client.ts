...
export type AutoUploadDouyinCommentDraftResult = {
  accountId: number;
  accountName: string;
  platformType: number;
  platformName: string;
  url: string;
  status: 'draft_filled' | 'comment_missing' | 'editor_missing';
  message: string;
  targetText: string;
  replyText: string;
...
export type AutoUploadWechatChannelCommentDraftResult =
  AutoUploadDouyinCommentDraftResult;

export type AutoUploadDouyinCommentSendResult = Omit<
  AutoUploadDouyinCommentDraftResult,
  'status'
> & {
  status: 'sent' | 'send_failed' | AutoUploadDouyinCommentDraftResult['status'];
  sent?: boolean;
  sentAt?: string;
...
export type AutoUploadWechatChannelCommentSendResult =
  AutoUploadDouyinCommentSendResult;

export type AutoUploadDouyinMessageDraftResult = {
  accountId: number;
  accountName: string;
  platformType: number;
  platformName: string;
  url: string;
  status: 'draft_filled' | 'message_missing' | 'editor_missing';
  message: string;
  targetText: string;
  replyText: string;
  editorTag?: string | null;
...
export type AutoUploadWechatChannelMessageDraftResult =
  AutoUploadDouyinMessageDraftResult;

export type AutoUploadDouyinMessageSendResult = Omit<
  AutoUploadDouyinMessageDraftResult,
  'status'
> & {
  status: 'sent' | 'send_failed' | AutoUploadDouyinMessageDraftResult['status'];
  sent?: boolean;
  sentAt?: string;
  editorCleared?: boolean;
  replyVisible?: boolean;
...
export type AutoUploadWechatChannelMessageSendResult =
  AutoUploadDouyinMessageSendResult;
...
export class AutoUploadClient {
  ...
  async draftDouyinCommentReply(input: {
    accountId: number;
    targetText: string;
    replyText: string;
  }): Promise<AutoUploadDouyinCommentDraftResult> {
    try {
      return await this.getEngineJson<AutoUploadDouyinCommentDraftResult>(
      ...
      const message = error instanceof Error ? error.message : 'unknown error';
      throw new ServiceUnavailableException(`抖音回复草稿填入失败：${message}`);
    }
  }

  async sendDouyinCommentReply(input: {
    accountId: number;
    targetText: string;
    replyText: string;
  }): Promise<AutoUploadDouyinCommentSendResult> {
    try {
      return await this.getEngineJson<AutoUploadDouyinCommentSendResult>(
      ...
      const message = error instanceof Error ? error.message : 'unknown error';
      throw new ServiceUnavailableException(`抖音回复自动发送失败：${message}`);
    }
  }

  async draftDouyinMessageReply(input: {
    accountId: number;
    targetText: string;
    replyText: string;
  }): Promise<AutoUploadDouyinMessageDraftResult> {
    try {
      return await this.getEngineJson<AutoUploadDouyinMessageDraftResult>(
      ...
      const message = error instanceof Error ? error.message : 'unknown error';
      throw new ServiceUnavailableException(`抖音私信草稿填入失败：${message}`);
    }
  }

  async sendDouyinMessageReply(input: {
    accountId: number;
    targetText: string;
    replyText: string;
  }): Promise<AutoUploadDouyinMessageSendResult> {
    try {
      return await this.getEngineJson<AutoUploadDouyinMessageSendResult>(
      ...
      const message = error instanceof Error ? error.message : 'unknown error';
      throw new ServiceUnavailableException(`抖音私信自动发送失败：${message}`);
    }
  }

  async draftWechatChannelCommentReply(input: {
    accountId: number;
    targetText: string;
    replyText: string;
  }): Promise<AutoUploadWechatChannelCommentDraftResult> {
    try {
      return await this.getEngineJson<AutoUploadWechatChannelCommentDraftResult>(
  ...
  async sendWechatChannelCommentReply(input: {
    accountId: number;
    targetText: string;
    replyText: string;
  }): Promise<AutoUploadWechatChannelCommentSendResult> {
    try {
      return await this.getEngineJson<AutoUploadWechatChannelCommentSendResult>(
  ...
  async draftWechatChannelMessageReply(input: {
    accountId: number;
    targetText: string;
    replyText: string;
  }): Promise<AutoUploadWechatChannelMessageDraftResult> {
    try {
      return await this.getEngineJson<AutoUploadWechatChannelMessageDraftResult>(
  ...
  async sendWechatChannelMessageReply(input: {
    accountId: number;
    targetText: string;
    replyText: string;
  }): Promise<AutoUploadWechatChannelMessageSendResult> {
    try {
      return await this.getEngineJson<AutoUploadWechatChannelMessageSendResult>(
