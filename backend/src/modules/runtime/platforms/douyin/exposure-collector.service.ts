...
export class DouyinExposureCollector {
  ...
  private diagnose(
    ...
    // 2026-08-31 复盘：滑块验证页文字是「请完成下列验证后继续」，
    // 旧正则（验证码|安全验证|滑块|行为验证|验证通过）匹配不上 → 被误判 platform_changed，
    // 用户看到的是「页面结构变化」而非风控。补充真实变体（含拖动滑块/完成拼图/验证后继续）。
    if (
      /验证码|安全验证|滑块|行为验证|验证通过|请完成.{0,8}验证|验证后继续|完成拼图/.test(
        text,
      )
    ) {
      return {
        ...
        message:
          '抖音页面弹出人机验证（滑块/验证码），采集已被平台风控拦截。请在内置浏览器中打开抖音手动完成验证后重试；近期频繁触发说明执行间隔过密，建议拉大间隔。',
      };
    }
  ...
  private diagnoseSearch(
    ...
    // 同上：覆盖「请完成下列验证后继续」等真实滑块验证文案，避免误判成页面结构变化。
    if (
      /验证码|安全验证|滑块|行为验证|验证通过|请完成.{0,8}验证|验证后继续|完成拼图/.test(
        text,
      )
      ...
      return {
        status: 'captcha_required',
        message:
          '抖音搜索页弹出人机验证（滑块/验证码），采集已被平台风控拦截。请在内置浏览器中打开抖音手动完成验证后重试；近期频繁触发说明执行间隔过密，建议拉大间隔。',
      };
    }
  ...
  private diagnoseIdentityGate(
    ...
    if (
      ...
      return {
        status: 'captcha_required',
        message:
          '抖音目标主页弹出人机验证（滑块/验证码），采集已被平台风控拦截。请在内置浏览器中打开抖音手动完成验证后重试。',
      };
    }
