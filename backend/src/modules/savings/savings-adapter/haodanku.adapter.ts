...
export class HaodankuAdapter implements SavingsAdapter {
  ...
  /** 运营位选品（好单库 column：type=1今日上新 2 9.9包邮 3 30元封顶 4聚划算 5淘抢购 9天猫 13淘宝） */
  async featured(type: number, limit = 10): Promise<OfferSnapshot[]> {
    const data = await this.callGet<Array<Record<string, unknown>>>('column', {
      min_id: 1,
      back: limit,
      type,
    });
