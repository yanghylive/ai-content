...
/* ===== AI 助手能力中心（2026-09-03）=====
 * 单一事实源：展示清单与 TOOLS 白名单同文件维护，前端通过
 * GET /api/ai-gateway/capabilities 拉取（不再前端手抄清单）。
 * 增删工具时：1) 在 TOOLS 注册 schema；2) 在此增删同名条目；
 * getCapabilityCenter() 只返回真正在白名单里的项，防幽灵能力展示。 */
...
const CAPABILITY_META: AiCapabilityGroup[] = [
  {
    title: '内容创作与运营',
    items: [
      {
        key: 'topic_hot',
        name: '热点选题',
        desc: '拉今日全网热榜（抖音/头条/知乎，含热度），帮你定选题',
        example: '今天有什么热点选题？',
      },
      {
        key: 'compliance_check',
        name: '违禁词体检',
        desc: '发布前检查平台违禁词，返回风险词与替换建议',
        example: '帮我检查这段文案有没有违禁词',
      },
      {
        key: 'knowledge_search',
        name: '品牌知识检索',
        desc: '从你的品牌知识库取真实资料（产品/门店/话术），创作不编造',
        example: '查一下我们品牌资料里怎么写产品卖点',
      },
      {
        key: 'content_generate',
        name: '内容文案生成',
        desc: '按平台风格写文案（公众号/小红书/抖音等）',
        example: '帮我写一篇小红书种草文',
      },
      {
        key: 'image_generate',
      ...
      },
      {
        key: 'video_download',
        name: '视频素材下载',
        desc: '分享链接去水印下载（抖音/快手/B站/YouTube 等主流平台）',
        example: '下载这个抖音视频素材',
      },
      {
        key: 'material_save',
      ...
      },
      {
        ...
        name: '定时发布',
        desc: '排期发布到平台（高风险写操作，需你确认）',
        example: '今晚 8 点发布到公众号',
      },
    ],
  },
  {
    ...
    items: [
      {
        key: 'parse_product',
        name: '商品解析',
        desc: '解析商品链接/口令 → 价格、优惠券、预计返利与净成本',
        example: '看看这个链接能省多少',
      },
      {
        key: 'compare_offers',
        name: '多平台比价',
        desc: '关键词跨平台比价（淘宝/京东/拼多多）',
        example: '帮我比价这款空气炸锅',
      },
      {
        key: 'create_price_watch',
        name: '降价/返利监控',
        desc: '低于目标价或返利达标时提醒你',
        example: '这款洗发水降到 39 以下提醒我',
      },
      {
        key: 'get_rebate_balance',
        name: '返利与额度查询',
        desc: '查返利余额明细与 AI 额度',
        example: '我还有多少返利？',
      },
      {
        key: 'query_cps_orders',
        name: '订单返利查询',
        desc: '查我的订单与返利结算状态',
        example: '我的订单返利到账了吗',
      },
      {
        key: 'convert_rebate_to_credit',
        name: '返利兑 AI 额度',
        desc: '返利余额兑换 AI 额度（需确认后执行）',
        example: '把返利余额换成 AI 额度',
      },
      {
        key: 'withdraw_rebate',
        name: '返利提现',
        desc: '提现到支付宝/微信（需确认后执行）',
        example: '我要提现 50 块',
      },
      {
        key: 'recommend_restock',
        name: '门店补货建议',
        desc: '根据采购清单给补货建议',
        example: '店里抽纸快没了，列个补货清单',
      },
    ],
  },
  {
    ...
    items: [
      {
        key: 'growth_playbooks',
        name: '获客方案库',
        desc: '美业/餐饮/教育等行业获客打法清单',
        example: '餐饮行业怎么做获客？',
      },
      {
        key: 'workflow_create',
      ...
      },
      {
        key: 'lead_list',
        name: '线索查询',
        desc: '查线索并按状态筛选（新线索/已触达/高意向）',
        example: '有哪些高意向线索',
      },
      {
        key: 'task_draft',
        name: '任务草稿',
        desc: '把获客/触达/复盘意图转成草稿，你确认后才执行',
        example: '帮我找抖音装修客户',
      },
    ],
  },
...
/** 能力中心：只返回 TOOLS 白名单中真实存在的展示项（动态核对，防漂移） */
export function getCapabilityCenter() {
  const live = new Set(TOOLS.map((t) => t.function.name));
  const groups = CAPABILITY_META.map((g) => ({
    title: g.title,
    items: g.items.filter((i) => live.has(i.key)),
