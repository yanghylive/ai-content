...
> = {
  ...
  'wechat-channel': {
    'comment-reply': 'https://channels.weixin.qq.com/platform/interaction/comment',
    'direct-message-reply': 'https://channels.weixin.qq.com/platform/private_msg',
  },
};
...
export class PlatformInteractionExecutor {
  ...
  async dispatch(input: PlatformDispatchInput): Promise<PlatformDispatchResult> {
    if (this.mockMode) {
      this.logger.error('DISPATCH_MOCK=true - blocked fake interaction success');
      return {
        status: 'failed',
        message: 'DISPATCH_MOCK=true：已阻断客户互动执行，不能跳过真实浏览器操作后返回成功。',
        nextAction: '关闭 DISPATCH_MOCK，并通过 Playwright MCP 执行真实读取、填入、发送和回读。',
      };
    }
  ...
  private async dispatchWithLocalBrowser(
    ...
    try {
      ...
      await page.waitForTimeout(input.platform === 'wechat-channel' ? 2500 : 1800);
      await this.clickWechatChannelEntryIfNeeded(page, input);
      await this.waitForPlatformInteractionSettled(page, input.platform, input.taskType);
      const loginState = await this.checkLoginState(page, input.platform);
      if (!loginState.ok) {
        const screenshot = await this.captureSessionScreenshot(session.key, `${input.platform}-not-logged-in`);
        return {
      ...
      if (actionResult.status === 'sent') {
        ...
        if (hasVisualFailure) {
          ...
          actionResult.message = '已点击发送，但页面显示发送异常标记，未确认真实发出。';
          actionResult.nextAction = '请检查平台是否弹出验证、账号是否限制发送或当前消息类型是否不支持网页回复。';
        }
      }
      ...
      return {
        ...
        profileDir: session.profileDir,
        visibleWindow: session.visibleWindow,
        nextAction: '检查平台页面是否加载完成、账号是否登录、目标对象是否仍可见。',
      };
    }
  }

  private async dispatchViaMcp(input: PlatformDispatchInput): Promise<PlatformDispatchResult> {
    ...
    try {
      ...
      if (!runtimeStatus.online || !runtimeStatus.visibleWindow || runtimeStatus.isolated) {
        return {
          status: 'failed',
          ...
          nextAction: '请确认 3011 Runtime 已启动可见持久浏览器，且未使用 headless/isolated 模式。',
        };
      }
      ...
      if (fillOk && clickOk && input.action === 'send') {
        ...
        replyVisible = this.snapshotContainsReply(readbackText, input.replyText);
        if (!replyVisible) {
          this.logger.warn(
      ...
      const finalStatus = fillOk && clickOk && sendReadbackOk
        ? (input.action === 'send' ? 'sent' : 'draft_filled')
        : 'failed';
      const finalMessage = fillOk && clickOk && sendReadbackOk
  ...
  private async readWithLocalBrowser(input: PlatformReadInput): Promise<Record<string, any>> {
    const targetUrl = PLATFORM_URLS[input.platform][input.taskType];
    ...
    if (input.platform === 'douyin' && input.taskType === 'direct-message-reply') {
  ...
  private async getDouyinMessageTabState(page: Page): Promise<{ activeTab?: string } | null> {
    try {
      return await page.evaluate(() => {
        const normalize = (value: unknown) => String(value || '').replace(/\s+/g, ' ').trim();
        const visible = (node: Element | null): node is HTMLElement => {
          if (!node || !(node as HTMLElement).getBoundingClientRect) return false;
          const rect = (node as HTMLElement).getBoundingClientRect();
          const style = window.getComputedStyle(node);
          return rect.width > 0 && rect.height > 0 && style.display !== 'none' && style.visibility !== 'hidden';
        };
        const labels = ['朋友私信', '陌生人私信', '群消息'];
        const tabs = Array.from(document.querySelectorAll('button, [role="tab"], div, span'))
          ...
          .filter(Boolean) as Array<{ text: string; x: number; y: number; selected: boolean }>;
  ...
  private async waitForPlatformInteractionSettled(
    ...
    if (platform === 'douyin' && taskType === 'direct-message-reply') {
      ...
      if (state.loadBlocked) {
        this.logger.warn(`Douyin message page not settled: ${state.reason || 'loading'}`);
      }
      return state;
    }
    if (platform === 'douyin' && taskType === 'comment-reply') {
      ...
      if (state.loadBlocked) {
        this.logger.warn(`Douyin comment page not settled: ${state.reason || 'loading'}`);
      }
      return state;
    }
  ...
  private async clickDouyinMessageTab(page: Page, label: string): Promise<boolean> {
    return page
      .evaluate((tabLabel) => {
        const normalize = (value: unknown) => String(value || '').replace(/\s+/g, ' ').trim();
        const visible = (node: Element | null): node is HTMLElement => {
        ...
        const candidates = Array.from(document.querySelectorAll('button, [role="tab"], div, span'))
          .filter((node) => {
            ...
            if (normalize((node as HTMLElement).innerText || node.textContent) !== tabLabel) return false;
            const rect = (node as HTMLElement).getBoundingClientRect();
  ...
  private async waitForDouyinMessagePageSettled(
    page: Page,
    timeoutMs: number,
  ): Promise<{ loadBlocked?: boolean; reason?: string; visibleLoaders?: number }> {
    ...
    while (Date.now() < deadline) {
      lastState = await page
        .evaluate(() => {
          ...
          const visibleLoaders = Array.from(
            ...
            if (!visible(node)) return false;
            const rect = (node as HTMLElement).getBoundingClientRect();
            return rect.x > 250 && rect.y > 120 && rect.width <= 180 && rect.height <= 180;
          }).length;
          return {
            hasEmptyState: /暂无|没有|空空如也|还没有|未收到私信|没有收到私信/.test(text),
            hasConversationHint: /未读|分钟前|小时前|昨天|今天|\d{1,2}:\d{2}|回复|发送/.test(text),
      ...
      if (lastState.hasConversationHint && Number(lastState.contentLength || 0) > 20) return {};
      if (Number(lastState.visibleLoaders || 0) === 0 && Number(lastState.contentLength || 0) > 16) return {};
      await page.waitForTimeout(900).catch(() => undefined);
    }
    return {
      loadBlocked: true,
      reason: String(lastState.textSample || '抖音私信页会话列表持续加载，未进入可读取状态。'),
    };
  }
  ...
  private async waitForDouyinCommentPageSettled(
    ...
    while (Date.now() < deadline) {
      lastState = await page
        .evaluate(() => {
          ...
          const visibleLoaders = Array.from(
            ...
            if (!visible(node)) return false;
            const rect = (node as HTMLElement).getBoundingClientRect();
            return rect.x > 250 && rect.y > 260 && rect.width <= 180 && rect.height <= 180;
          }).length;
          const commentAreaText = Array.from(
            document.querySelectorAll('main, section, [class*="comment"], [class*="Comment"], div'),
          )
            .filter((node) => visible(node))
            .filter((node) => {
              const rect = (node as HTMLElement).getBoundingClientRect();
              return rect.x > 220 && rect.y > 250 && rect.width > 260 && rect.height > 40;
            })
            .map((node) => normalize(node.textContent))
            .join(' ')
            .slice(0, 1200);
          const hasEmptyState = /暂无评论|没有评论|暂无数据|暂无互动|还没有评论/.test(text);
          const hasCommentSignal =
            /回复|点赞|评论|刚刚|今天|昨天|\d{1,2}:\d{2}|分钟前|小时前/.test(commentAreaText) &&
      ...
      if (Number(lastState.visibleLoaders || 0) === 0 && String(lastState.commentAreaText || '').length > 20) {
        return {};
      }
      await page.waitForTimeout(900).catch(() => undefined);
    }
    return {
      loadBlocked: true,
      reason: String(lastState.commentAreaText || lastState.textSample || '抖音评论页持续加载，未进入可读取状态。'),
    };
  }
  ...
  private async detectSendFailureMarker(page: Page): Promise<boolean> {
    try {
      ...
      if (/评论成功|发送成功|回复成功/.test(text)) return false;
      ...
      for (let offset = 0; offset < redMask.length; offset += 1) {
        ...
        if (inReplyArea && pixels >= 18 && pixels <= 900 && width <= 60 && height <= 60) {
          return true;
        }
      }
      return false;
  ...
  private async clickWechatChannelEntryIfNeeded(
    ...
    if (await this.isWechatChannelTargetEntryReady(page, input.taskType)) return;
    const labels =
    ...
    for (const label of labels) {
      try {
        await page.getByText(label, { exact: true }).first().click({ timeout: 1200 });
      ...
      const clickTarget = await page.evaluate((taskType) => {
        ...
        const clickableAncestor = (node: HTMLElement) => {
          ...
          for (let depth = 0; current && depth < 6; depth += 1) {
            ...
            if ((clickable || looksLikeCard) && visible(current)) return current;
        ...
        const nodes = Array.from(document.querySelectorAll('div, section, article, a, button'));
        const candidates = nodes
          ...
          .map((node) => {
            ...
            const rect = (node as HTMLElement).getBoundingClientRect();
            const clickable =
              (node as HTMLElement).closest('a, button, [role="button"], [tabindex]') ||
              node;
            ...
            return {
              ...
              preferredScore: preferredWords.filter((word) => text.includes(word)).length,
              titleScore: preferredWords.some((word) => firstLine === word || firstLine.includes(word)) ? 4 : 0,
            };
          })
          ...
          .filter((item) => {
            const lowerNoise = /退出登录|系统设置|帮助中心|服务协议|隐私政策/.test(item.text);
            return !lowerNoise;
          })
          .sort(
            (a, b) =>
    ...
    for (const label of secondaryLabels) {
      try {
        const clickTarget = await page.evaluate((targetLabel) => {
          ...
          const nodes = Array.from(document.querySelectorAll('a, button, [role="button"], [role="tab"], div, span'));
          const picked = nodes
            .filter((node) => visible(node))
            .map((node) => {
              const rect = (node as HTMLElement).getBoundingClientRect();
            ...
            .filter((item) => item.text === targetLabel || item.text.startsWith(targetLabel))
            .filter((item) => item.rect.width >= 30 && item.rect.width <= 300 && item.rect.height >= 18 && item.rect.height <= 120)
            .sort((a, b) => a.rect.y - b.rect.y || a.rect.x - b.rect.x)[0]?.node;
          if (!picked) return false;
          const clickable = picked.closest('a, button, [role="button"], [role="tab"], [tabindex]') as HTMLElement | null;
          ...
          return {
            ...
            width: rect.width,
            height: rect.height,
            text: normalize(target.innerText || target.textContent).slice(0, 120),
          };
        }, label);
  ...
  private async isWechatChannelTargetEntryReady(
    ...
    if (taskType === 'comment-reply') {
      return /视频号助手\s*·\s*评论|评论管理/.test(text) &&
        /回复|删除|筛选|导出|评论/.test(text);
    }
    return /视频号助手\s*·\s*私信|私信管理|打招呼消息|全部私信/.test(text) &&
      /私信|消息/.test(text);
  }
  ...
  private async ensureWechatChannelEntryReadyOrThrow(
    ...
    const onAssistantHome =
      ...
      !/视频号助手\s*·\s*(?:评论|私信)|评论管理|私信管理|打招呼消息|全部私信/.test(text);
    const currentUrl = page.url();
  ...
  private async ensureWechatChannelEntryReadyOrRecover(
    ...
    try {
      ...
      if (!/入口未打开|视频号(?:评论|私信)业务区|视频号助手首页/.test(firstMessage)) {
        throw firstError;
      }
      this.logger.warn(
    ...
    const recovered = await this.browser.recoverWechatChannelSessionFromLegacyProfile({
      accountId: input.accountId,
      taskType: input.taskType,
    });
    if (!recovered) {
  ...
  private async checkLoginState(
    ...
    const loginPrompt =
      /扫码登录|验证码登录|密码登录|登录\/注册|登录或注册|请先登录|未登录|二维码/.test(text);
  ...
  private async performDomInteraction(
    ...
    if (input.platform === 'wechat-channel' && input.taskType === 'direct-message-reply') {
      return this.performWechatChannelInteraction(page, input);
    }
    if (input.platform === 'wechat-channel' && input.taskType === 'comment-reply') {
      return this.performWechatChannelInteraction(page, input);
    }
    if (input.platform === 'douyin' && input.taskType === 'direct-message-reply') {
    ...
    const action = await targetFrame.evaluate(
      ({ targetText, replyText, shouldSend, missingStatus, isMessage }) => {
        ...
        const readValue = (node: Element) =>
          normalize('value' in node ? (node as HTMLInputElement).value : node.textContent);
        const setEditorValue = (editor: Element, value: string) => {
          ...
          editor.dispatchEvent(new KeyboardEvent('keydown', { bubbles: true, key: value.slice(-1) || 'a' }));
          editor.dispatchEvent(new KeyboardEvent('keyup', { bubbles: true, key: value.slice(-1) || 'a' }));
        };
        ...
        const candidateRows = Array.from(document.querySelectorAll(rowSelector))
          .filter((node) => visible(node) && textMatches(node.textContent || '', targetText))
          .map((node) => {
            const rect = node.getBoundingClientRect();
            const text = normalize(node.textContent || '');
            const className = String((node as HTMLElement).className || '');
            const replySignal = /回复|发送|删除|未读|刚刚|今天|昨天|\d{1,2}:\d{2}|分钟前|小时前/.test(text);
            const classSignal = /comment|message|chat|session|conversation|item/i.test(className);
            const score =
              (replySignal ? 60 : 0) +
              (classSignal ? 40 : 0) +
              (rect.x > 120 ? 10 : 0) +
        ...
        return delay(isMessage ? 2400 : 900).then(async () => {
          ...
          const replyTriggers = Array.from(
            ...
            .filter((node) => visible(node) && normalize(node.textContent) === '回复')
            .sort((a, b) => {
          ...
          if (replyTriggers[0]) {
            try {
              replyTriggers[0].dispatchEvent(new MouseEvent('mouseover', { bubbles: true }));
              replyTriggers[0].dispatchEvent(new MouseEvent('mousedown', { bubbles: true }));
              (replyTriggers[0] as HTMLElement).click();
              replyTriggers[0].dispatchEvent(new MouseEvent('mouseup', { bubbles: true }));
            } catch {}
            await delay(900);
          }
          ...
          const disabled = (node: Element) => {
            ...
            return (
              ...
              aria === 'true' ||
              /disabled/.test(String((node as HTMLElement).className || '').toLowerCase())
            );
          };
          const findButtons = () =>
            Array.from(
              document.querySelectorAll('button, [role="button"], span, div, a'),
            )
              .filter((node) => {
                ...
                if (!isButton && (rect.width > 180 || rect.height > 64)) return false;
                return Math.abs(rect.y - editorRect.y) <= 280 || rect.y > editorRect.y - 80;
              })
              .map((node) => {
                const rect = node.getBoundingClientRect();
                const priority =
                  node.tagName === 'BUTTON' ? 0 : node.getAttribute('role') === 'button' ? 1 : 2;
                const distance = Math.abs(rect.y - editorRect.y) + Math.abs(rect.x - editorRect.x);
                return { node, rect, priority, distance, text: normalize(node.textContent) };
              })
              .sort((a, b) => a.priority - b.priority || a.distance - b.distance);
          let buttons = findButtons();
          for (let attempt = 0; buttons.length === 0 && attempt < 5; attempt += 1) {
            (editor as HTMLElement).focus();
          ...
          const editorsAfter = Array.from(
            ...
            .filter(visible)
            .map(readValue);
          const replyStillInEditor = editorsAfter.some((value) => value.includes(replyPrefix));
          const bodyHasReply = bodyText.includes(replyPrefix);
          const hasSendFailureText =
            /发送失败|发送异常|重新发送|网络异常|稍后再试|未发送|消息发送失败|只有群主和管理员可以发消息|没有发言权限|不能发送消息|无权发送|无法发送/.test(bodyText);
          const hasReplyErrorMarker = Array.from(document.querySelectorAll('svg, i, span, div'))
            .filter(visible)
            .some((node) => {
              const rect = node.getBoundingClientRect();
              const style = window.getComputedStyle(node);
              const text = normalize(node.textContent);
              const color = `${style.color} ${style.backgroundColor} ${style.borderColor}`.toLowerCase();
              const replyNode = Array.from(document.querySelectorAll('div, span, p'))
                .filter(visible)
                .find((candidate) => normalize(candidate.textContent).includes(replyPrefix)) as HTMLElement | undefined;
                ...
                bodyHasReply &&
                  ...
                  (
                    (replyRect &&
                      ...
                      Math.abs((rect.y + rect.height / 2) - (replyRect.y + replyRect.height / 2)) <= 80) ||
                    (!replyRect &&
          ...
          const sent = bodyHasReply && !replyStillInEditor && !hasSendFailureText && !hasReplyErrorMarker;
          return {
            ...
            readbackText: bodyHasReply ? replyText : editorsAfter.find((value) => value.includes(replyPrefix)) || '',
            nextAction: sent ? undefined : '请检查平台是否弹出验证、发送按钮是否失效、账号是否限制发送或页面结构是否变化。',
          };
        });
      },
      {
        ...
        missingStatus:
          input.taskType === 'comment-reply' ? 'comment_missing' : 'message_missing',
  ...
  private async performDouyinMessageInteraction(
    ...
    const routeCapture = await this.installDouyinImRouteCapture(page.context(), trace);
    await this.installDouyinImWindowCapture(page);
    try {
    await this.openDouyinMessagePage(page);
    const scanResult = await this.scanDouyinMessageTabs(page, 10, input.targetText);
    ...
    if (loadBlocked && !(scanResult.messages || []).length) {
      return {
        ...
        message: String(loadBlocked.emptyReason || '抖音私信页会话列表持续加载，未进入可回复状态。'),
        nextAction: String(loadBlocked.nextAction || '请确认抖音创作者后台私信页能正常显示后重试。'),
      };
    }
    const targetState = await page.evaluate(
      async ({ targetText }) => {
        ...
        const scoreClickTarget = (node: Element, sourceNode: Element) => {
          ...
          if (rect.width >= 260 && rect.width <= 900) score += 34;
          ...
          if (rect.width > 900) score -= 120;
          if (rect.height > 260) score -= 38;
          if (rect.height > 180) score -= 100;
          if (rect.x < 260 || rect.x > 760) score -= 50;
        ...
        const targetNodes = allNodes.filter(
          (node) => visible(node) && textMatchesTarget((node as HTMLElement).innerText || node.textContent || ''),
        );
        ...
        const rowText = bestTarget.text || normalize((messageNode as HTMLElement).innerText || messageNode.textContent);
        if (rowLooksUnreplyable(rowText) && rowLooksUnreplyable(normalize(targetText))) {
          return {
            status: 'message_missing',
        ...
        try {
          (messageNode as HTMLElement).scrollIntoView({ block: 'center', inline: 'nearest' });
          await delay(350);
        } catch {}
        ...
        return {
          ...
          selected: { text: targetText, context: rowText.slice(0, 180) },
          targetClickRect: { x: rect.x, y: rect.y, width: rect.width, height: rect.height },
        };
      },
      { targetText: input.targetText },
    );

    if (targetState.status !== 'target_ready' || !targetState.targetClickRect) {
      return {
        status: targetState.status === 'message_missing' ? 'message_missing' : 'failed',
        message: targetState.message || '未在当前私信页找到目标私信，未发送。',
        nextAction: '请检查抖音私信列表是否仍包含目标对象。',
      };
    }

    const targetRect = targetState.targetClickRect;
    const targetX = targetRect.x + Math.min(Math.max(targetRect.width / 2, 40), 220);
    ...
    const editorState = await page.evaluate(() => {
      ...
      const nodes = Array.from(
        document.querySelectorAll('textarea, [contenteditable="true"], input[type="text"], [role="textbox"]'),
      )
        .filter(visible)
        .map((node) => {
          const rect = node.getBoundingClientRect();
          const value = normalize('value' in node ? (node as HTMLInputElement).value : (node as HTMLElement).innerText || node.textContent);
          return {
            tag: node.tagName,
            value,
    ...
    if (editorState.status !== 'editor_ready' || !editorState.editorRect) {
      ...
      return {
        ...
        nextAction: '抖音网页没有打开会话详情；需要重新校准私信列表点击区域或确认该消息类型是否支持网页回复。',
      };
    }

    const editorRect = editorState.editorRect;
    const editorX = editorRect.x + Math.min(Math.max(editorRect.width / 2, 2), 160);
    ...
    const sendButton = await page.evaluate(
      ({ replyText }) => {
        ...
        const editorValue = (editor: Element) =>
          normalize('value' in editor ? (editor as HTMLInputElement).value : (editor as HTMLElement).innerText || editor.textContent);
        ...
        const candidates = Array.from(document.querySelectorAll('button, [role="button"], span, div'))
          ...
          .map((node) => {
            ...
            const tag = String(node.tagName || '').toUpperCase();
            const role = String(node.getAttribute('role') || '').toLowerCase();
            const priority = tag === 'BUTTON' ? 0 : role === 'button' ? 1 : tag === 'SPAN' ? 2 : 3;
    ...
    if (sendButton.status !== 'ready_to_click_send' || !sendButton.sendButtonRect) {
      return {
        status: sendButton.status === 'editor_input_failed' ? 'editor_missing' : 'send_failed',
      ...
      async ({ replyText }) => {
        ...
        const readState = () => {
          const editors = Array.from(
            ...
            .filter(visible)
            .map((node) =>
              normalize('value' in node ? (node as HTMLInputElement).value : (node as HTMLElement).innerText || node.textContent),
            );
          const replyStillInEditor = editors.some((value) => value.includes(replyPrefix));
          const bodyText = normalize(document.body.innerText);
          const hasSendFailureText =
            /发送失败|发送异常|重新发送|网络异常|稍后再试|未发送|消息发送失败|没有发言权限|不能发送消息|无权发送|无法发送/.test(bodyText);
          return {
            replyStillInEditor,
    ...
    return {
      ...
      replyVisible: Boolean(verify.bodyHasReply),
        nextAction: sent ? undefined : '请检查平台是否弹出验证、账号是否限制发送或当前消息类型是否支持网页回复。',
      };
    } finally {
  ...
  private async readDouyinMessagesWithLocalBrowser(
    ...
    const scan = await this.scanDouyinMessageTabs(page, limit);
    await page.waitForTimeout(1200).catch(() => undefined);
    const windowCapture = await this.collectDouyinImWindowCapture(page, trace, limit);
    const evidence = await this.captureSessionScreenshot(
      session.key,
    ...
    const readableMessages = domMessages.length ? domMessages : textFallbackMessages;
    const messages = readableMessages.length
      ? this.mergeDouyinMessageCandidates(readableMessages, trace, limit)
      : [];
  ...
  private async openDouyinMessagePage(page: Page): Promise<void> {
    ...
    if (!page.url().includes('following/chat')) {
      for (const label of ['互动管理', '私信管理', '私信', '用户私信', '消息']) {
        try {
  ...
  private async scanDouyinMessageTabs(
    ...
    const tabs = ['全部', '朋友私信', '陌生人私信', '群消息'];
    const scannedTabs: Array<Record<string, any>> = [];
    let bestScan: (Record<string, any> & { messages?: Array<Record<string, any>> }) | null = null;
    const target = this.normalizeInteractionText(targetText);
    const emptyScan = async (reason: string): Promise<Record<string, any> & { messages: Array<Record<string, any>> }> => ({
      url: page.url(),
      title: await page.title().catch(() => ''),
      totalCandidates: 0,
      messages: [],
    ...
    const evaluateScan = async (reason: string): Promise<Record<string, any> & { messages: Array<Record<string, any>> }> => {
      const scan = (await page
        .evaluate(this.douyinMessageScanScript(), limit)
        .catch((error) =>
          emptyScan(
      ...
      return {
        ...
        pageTextSample: String(scan.pageTextSample || (await this.pageText(page, 800))),
      };
    };
    ...
    for (let index = 0; index < tabs.length; index += 1) {
      ...
      if (target && this.douyinMessageScanHasTarget(scan, target)) {
      ...
      if (!bestScan || Number(scan.totalCandidates || 0) > Number(bestScan.totalCandidates || 0)) {
        bestScan = scan;
      }
      if (index === 0 && !target) {
        const pageText = this.normalizeInteractionText(String(scan.pageTextSample || ''));
        const emptyMatch = pageText.match(/还没有收到私信|暂无私信|暂无消息|没有收到私信|没有私信|暂无会话/);
        if (emptyMatch) {
    ...
    const fallback =
      bestScan ||
      (await evaluateScan('抖音私信页最终扫描未返回可用结果'));
    fallback.scannedTabs = scannedTabs;
    const stillLoading = scannedTabs.some((tab) => Number(tab.loading || 0) > 0);
    const hasUsableText = scannedTabs.some((tab) => Number(tab.totalCandidates || 0) > 0);
    const pageText = this.normalizeInteractionText(String(fallback.pageTextSample || ''));
    const emptyMatch = pageText.match(/还没有收到私信|暂无私信|暂无消息|没有收到私信|没有私信|暂无会话/);
    if (emptyMatch && !hasUsableText) {
      ...
      fallback.loadBlockedReason = '抖音私信页会话列表持续加载，未进入可读取状态。';
    }
    return fallback;
  }
  ...
  private douyinMessageScanHasTarget(
    ...
    const target = this.normalizeInteractionText(targetText);
    if (!target) return Boolean(scan?.messages?.length);
    return Boolean(
  ...
  private douyinMessageLoadBlockedSummary(scan: Record<string, any> | null | undefined) {
    if (!scan?.loadBlocked) return null;
    return {
      totalCandidates: scan.totalCandidates || 0,
      usableCount: 0,
      emptyReason: scan.loadBlockedReason || '抖音私信页会话列表持续加载，未进入可读取状态。',
      blocked: true,
  ...
  private extractDouyinMessageCandidatesFromPageText(
    ...
    while ((match = rowPattern.exec(text)) && rows.length < limit) {
      ...
      const context = [rawContact, rawTime, rawMessage].filter(Boolean).join(' ');
  ...
  private cleanDouyinMessagePreviewText(value: string): string {
    ...
    text = text.replace(
      new RegExp(`^.{1,80}?\\s+${timePattern}\\s+`, 'i'),
      '',
    );
    ...
    return `(limit) => {
      ...
      const isNoise = (text) => {
        ...
        if (/^[\\u4e00-\\u9fa5A-Za-z0-9_·\\-]{1,24}(?:📷|✅|✔|V)?$/.test(text)
          && !/[？?吗呢吧呀哦]|预约|价格|多少|怎么|哪里|联系|电话|微信|私信|在吗|在哪|要|买|发|帮|看/.test(text)) {
          return true;
        }
      ...
      const rowLooksLikeSessionList = (text) =>
        ...
        /你收到一条新类型消息|请打开抖音\\s*app\\s*查看|分享\\[视频\\]|昨天|\\d{1,2}:\\d{2}/.test(text) &&
        text.length > 260;
  ...
  private async installDouyinImRouteCapture(
    ...
    const handler: DouyinImRouteCapture['handler'] = async (route) => {
      ...
      try {
        ...
        const candidates = this.extractDouyinImMessageCandidatesFromPayload(text, 20);
        appendCapture({
          kind: 'routeCapture',
          url: url.slice(0, 500),
        ...
        appendCapture({
          ...
          method: request.method(),
          resourceType: request.resourceType(),
          errorText: error instanceof Error ? error.message.slice(0, 240) : String(error).slice(0, 240),
          timestamp: new Date().toISOString(),
        });
  ...
  private async detachDouyinImRouteCapture(
    ...
    for (const pattern of routeCapture.patterns) {
      await context.unroute(pattern, routeCapture.handler).catch(() => undefined);
    }
  }
  ...
  private async collectDouyinImWindowCapture(
    ...
    const captures = await page
      .evaluate(() =>
        ((window as any).__kaypalDouyinImResponses || []).slice(-30).map((item: any) => ({
          kind: item.kind,
          url: item.url,
          status: item.status,
          body: item.body,
    ...
    for (const item of captures || []) {
      ...
      for (const candidate of this.extractDouyinImMessageCandidatesFromPayload(body, limit)) {
        candidates.push({
          ...candidate,
  ...
  private extractDouyinImMessageCandidatesFromPayload(
    ...
    const addCandidate = (text: unknown, source: string, context?: unknown) => {
      ...
      candidates.push({
        ...
        looksLikeMessage: true,
        source,
        context: this.normalizeInteractionText(String(context || '')).slice(0, 260),
        score: source.includes('content') || source.includes('message') ? 80 : 55,
      });
    };
    const walk = (value: unknown, path = '', parent?: Record<string, unknown>) => {
      if (candidates.length >= limit) return;
      if (value && typeof value === 'object' && !Array.isArray(value)) {
        const obj = value as Record<string, unknown>;
        const compactContext = ['nickname', 'nick_name', 'conversation_name', 'text', 'content', 'msg_content']
          .map((key) => obj[key])
        ...
        for (const [key, child] of Object.entries(obj)) {
          ...
          if (typeof child === 'string') {
            ...
            if (
              messageKeys.has(key) ||
              ['text', 'content', 'message', 'msg'].some((token) => lowerKey.includes(token))
            ) {
    ...
    if (!candidates.length) {
      const regex = /"(?:text|content|msg_content|message|preview|abstract)"\s*:\s*"([^"]{2,240})"/gi;
      let match: RegExpExecArray | null;
      while ((match = regex.exec(payloadText || '')) && candidates.length < limit) {
  ...
  private looksLikeDouyinCustomerMessage(text: string): boolean {
    ...
    if (!normalized || normalized.length < 2 || normalized.length > 240) return false;
    ...
    if (noiseFragments.some((fragment) => normalized.toLowerCase().includes(fragment.toLowerCase()))) {
      return false;
    }
    if (['全部', '朋友私信', '陌生人私信', '群消息', '发送', '搜索', '抖音', '首页'].includes(normalized)) {
      return false;
    }
      ...
      const state = await page
        .evaluate(() => {
          ...
          const stillLoading = Array.from(
            document.querySelectorAll('[class*="loading"], [class*="Loading"], [class*="spin"], [class*="Spin"], .semi-spin, svg'),
          ).some((node) => {
            if (!visible(node)) return false;
            const rect = (node as HTMLElement).getBoundingClientRect();
            return rect.x > window.innerWidth * 0.35 && rect.y > 140 && rect.width <= 180 && rect.height <= 180;
          });
          const detailHint = /发送|请输入|回复|表情|聊天|消息/.test(bodyText);
          return { ready: false, stillLoading, detailHint };
        })
        .catch(() => ({ ready: false, stillLoading: false, detailHint: false }));
  ...
  private async readDouyinCommentsWithLocalBrowser(
    ...
    if (!(await this.isDouyinCommentPageReady(page))) {
      ...
      return {
        ...
        message: '抖音评论管理页未成功打开，已停止读取，避免把作品管理页误判为评论。',
        summary: {
          totalCandidates: 0,
          usableCount: 0,
  ...
  private extractDouyinCommentCandidatesFromPageText(
    ...
    const windowText = text.includes('评论管理')
      ...
      : text;
    const stopAt = windowText.search(/作品列表|共\d+个视频|发布作品|互动管理|数据中心/);
    ...
    const looksLikeComment = (value: string) => {
      ...
      if (/^(刚刚|今天|昨天|\d+分钟前|\d+小时前|\d+天前)$/.test(item)) return false;
    ...
    const add = (value: string, source: string, context = '') => {
      ...
      candidates.push({
        ...
        score:
          70 +
          (/[？?吗呢吧呀哦]|预约|价格|多少|怎么|哪里|联系|电话|微信|私信/.test(item)
            ? 20
            : 0),
      });
    };
    ...
    while ((match = commentPattern.exec(scope)) && candidates.length < limit) {
      const body = normalize(match[3]).replace(/\s*(回复|删除|举报|查看\d+条回复).*$/, '');
      add(body, 'page-text-comment-row', match[0]);
    }
    if (!candidates.length) {
      const loosePattern = /有爱评论，说点儿好听的～\s+(.{2,180}?)(?=\s+(?:回复|删除|举报|查看\d+条回复|作品列表|$))/g;
  ...
  private async isDouyinCommentPageReady(page: Page): Promise<boolean> {
    return page
      .evaluate(() => {
        ...
        const hasCommentManager = text.includes('评论管理') && text.includes('选择作品');
  ...
  private async openDouyinCommentEntryFromContentManage(page: Page): Promise<boolean> {
    const clicked = await page
      .evaluate(() => {
        ...
        const scoreCandidate = (node: HTMLElement) => {
          ...
          const rect = node.getBoundingClientRect();
          if (rect.x < 220 || rect.y < 160 || rect.width < 20 || rect.height < 18) return -1;
          if (!/评论\s*\d+|\d+\s*(?:评论)?/.test(text)) return -1;
          if (/作品管理|全部作品|编辑作品|设置权限|删除作品/.test(text) && text.length > 80) return -1;
  ...
  private async clickDouyinNavItem(page: Page, label: string): Promise<boolean> {
    return page
      .evaluate((targetLabel) => {
        ...
        const node = Array.from(document.querySelectorAll('a, button, [role="button"], div, span'))
          .filter((item) => visible(item) && normalize((item as HTMLElement).innerText || item.textContent) === targetLabel)
          .sort(
            (a, b) =>
              (a as HTMLElement).getBoundingClientRect().x -
              (b as HTMLElement).getBoundingClientRect().x,
  ...
  private async dismissDouyinOverlays(page: Page): Promise<void> {
    await page
      .evaluate(() => {
        ...
        for (const node of Array.from(document.querySelectorAll('button, [role="button"], span, div'))) {
          if (!visible(node)) continue;
          const text = normalize((node as HTMLElement).innerText || node.textContent);
  ...
  private douyinCommentScanHasTarget(
    scan: { comments?: Array<Record<string, any>> } | null | undefined,
    targetText: string,
  ): boolean {
    const target = this.normalizeInteractionText(targetText);
    if (!target) return Boolean(scan?.comments?.length);
    return Boolean(
      scan?.comments?.some((item) => {
        const text = this.normalizeInteractionText(String(item.text || ''));
        return text && (text === target || text.includes(target) || target.includes(text));
      }),
    );
  ...
  private async chooseDouyinCommentWorkWithCandidates(
    ...
    const scanCurrent = async (
      ...
      const scan = await page.evaluate(this.douyinCommentScanScript(), {
        limit: scanLimit,
        rules,
      });
      return {
    ...
    const canSwitch = await page
      .evaluate(() => {
        ...
        const nodes = Array.from(document.querySelectorAll('button, [role="button"], div, span'))
          .filter((node) => visible(node) && normalize((node as HTMLElement).innerText || node.textContent) === '选择作品')
          .sort(
            (a, b) =>
              (b as HTMLElement).getBoundingClientRect().x -
              (a as HTMLElement).getBoundingClientRect().x,
      ...
      .evaluate((max) => {
        ...
        const candidates = Array.from(document.querySelectorAll('div, li, tr, section'))
          ...
          .map((node, index) => {
            ...
            const commentCount = parseCommentCount((node as HTMLElement).innerText || node.textContent || '');
    ...
    let bestScanWithComments: Record<string, any> | null =
      (initial.comments || []).length ? initial : null;
          ...
          ({ text, index: itemIndex }) => {
            ...
            const nodes = Array.from(document.querySelectorAll('div, li, tr, section'))
              .filter((node) => {
                ...
                return visible(node) &&
                  rect.x > window.innerWidth * 0.55 &&
                  normalize((node as HTMLElement).innerText || node.textContent).includes(String(text).slice(0, Math.min(String(text).length, 80)));
              })
              .sort((a, b) => {
            ...
            const fallback = Array.from(document.querySelectorAll('div, li, tr, section')).filter((node) => {
              const rect = (node as HTMLElement).getBoundingClientRect();
              return visible(node) && rect.x > window.innerWidth * 0.55 && node.querySelector('img') && /发布于/.test(normalize((node as HTMLElement).innerText || node.textContent));
    ...
    return {
      ...
      targetMatched: target ? this.douyinCommentScanHasTarget(fallback, target) : undefined,
    };
  }
  ...
  private async fillAndSendDouyinCommentReplyOnPage(
    ...
    const target: any = await page.evaluate(
      ({ targetText }) => {
        const normalize = (value: unknown) =>
          String(value || '')
            .replace(/\s+/g, ' ')
        ...
        const nodes = Array.from(document.querySelectorAll('div, span, p, li, td'))
          .filter((node): node is HTMLElement =>
            visible(node) && normalize((node as HTMLElement).innerText || node.textContent).includes(targetText),
          )
          .map((node) => {
            const rect = node.getBoundingClientRect();
        ...
        for (const item of nodes) {
          ...
          const replyNodes = Array.from(root.node.querySelectorAll('button, [role="button"], span, div'))
            .filter((node): node is HTMLElement =>
              visible(node) && normalize((node as HTMLElement).innerText || node.textContent) === '回复',
            )
    ...
    const replyOpened: any = await page.evaluate(
      ({ targetText, rootRect }) => {
        const normalize = (value: unknown) =>
          String(value || '')
            .replace(/\s+/g, ' ')
    ...
    const editor: any = await page.evaluate(
      ({ rootRect }) => {
        const normalize = (value: unknown) => String(value || '').replace(/\s+/g, ' ').trim();
        const visible = (node: Element | null): node is HTMLElement => {
          if (!node || !(node as HTMLElement).getBoundingClientRect) return false;
        ...
        const explicitReplyEditors = allEditors
          ...
          .sort((a, b) => {
            const aDistance = Math.abs(a.rect.y - rootY) + Math.abs(a.rect.x - rootX);
            const bDistance = Math.abs(b.rect.y - rootY) + Math.abs(b.rect.x - rootX);
        ...
        const picked = targetReplyEditors[0] || explicitReplyEditors[0] || fallback[0];
        if (!picked) return { status: 'editor_missing', message: '没有找到可编辑回复框。' };
        picked.node.focus();
    ...
    const activeEditor: any = await page.evaluate(
      ({ rootRect }) => {
        const visible = (node: Element | null): node is HTMLElement => {
          if (!node || !(node as HTMLElement).getBoundingClientRect) return false;
          const rect = (node as HTMLElement).getBoundingClientRect();
        ...
        const editors = Array.from(
          ...
          .map((node) => {
            ...
            const distance = Math.abs(rect.y - rootY) + Math.abs(rect.x - rootX);
            const priority = (replyEditor ? 0 : 40) + (inTargetRow ? 0 : 20) + (editable ? 0 : 5);
            return { node, rect, priority, distance };
          })
          .sort((a, b) => a.priority - b.priority || a.distance - b.distance || b.rect.width - a.rect.width);
        const editor = editors[0]?.node;
    ...
    await page.mouse.click(
      Number(activeRect.x || 0) + Math.max(Number(activeRect.width || 1) / 2, 1),
      Number(activeRect.y || 0) + Math.max(Number(activeRect.height || 1) / 2, 1),
    );
    await page.keyboard.press('Meta+A');
    await page.keyboard.press('Backspace');
    await page.keyboard.insertText(input.replyText);
        ...
        const editors = Array.from(
          ...
          .map((node) => {
            ...
            return {
              ...
              rect,
              priority,
              targetDistance: Math.abs(rect.y - rootY) + Math.abs(rect.x - rootX),
    ...
    const sendButton: any = await page.evaluate(
      ({ replyText }) => {
        const normalize = (value: unknown) => String(value || '').replace(/\s+/g, ' ').trim();
        const visible = (node: Element | null): node is HTMLElement => {
          if (!node || !(node as HTMLElement).getBoundingClientRect) return false;
        ...
        const allEditors = Array.from(
          ...
          .map((node) => {
            const rect = node.getBoundingClientRect();
            const value = normalize('value' in node ? (node as HTMLInputElement).value : node.textContent);
            const placeholder = normalize(node.getAttribute('placeholder'));
        ...
        const allButtons = Array.from(document.querySelectorAll('button, [role="button"], span, div'))
          .filter((node): node is HTMLElement => {
            ...
            if (!/^(发送|回复|提交)$/.test(text)) return false;
            const rect = (node as HTMLElement).getBoundingClientRect();
            const tag = String((node as HTMLElement).tagName || '').toUpperCase();
            const role = String(node.getAttribute('role') || '').toLowerCase();
            const isRealButton = tag === 'BUTTON' || role === 'button';
          ...
          .filter((item) => {
            ...
            const nearVertical = item.rect.y >= editor.rect.y - 20 && item.rect.y <= editor.rect.y + 120;
            const toRight = item.rect.x >= editor.rect.x;
            return nearVertical && toRight;
          })
          .sort((a, b) => a.priority - b.priority || Number(a.isDisabled) - Number(b.isDisabled) || a.distance - b.distance);
    ...
    if (sendButton.status !== 'send_button_ready') {
      return {
        status: sendButton.status === 'editor_missing' ? 'editor_missing' : 'send_failed',
    ...
    const buttonX = Number(buttonRect.x || 0) + Math.max(Number(buttonRect.width || 1) / 2, 1);
    const buttonY = Number(buttonRect.y || 0) + Math.max(Number(buttonRect.height || 1) / 2, 1);
    ...
    const verify: any = await page.evaluate(
      async ({ targetText, replyText }) => {
        const delay = (ms: number) => new Promise((resolve) => setTimeout(resolve, ms));
        const normalize = (value: unknown) => String(value || '').replace(/\s+/g, ' ').trim();
        const visible = (node: Element | null): node is HTMLElement => {
          if (!node || !(node as HTMLElement).getBoundingClientRect) return false;
        ...
        const readState = () => {
          ...
          const visibleEditors = Array.from(
            ...
            .filter((node): node is HTMLElement => visible(node))
            .map((node) => normalize('value' in node ? (node as HTMLInputElement).value : node.textContent));
          const rows = Array.from(
            document.querySelectorAll('[class*="cmt-item"], [class*="comment"], [class*="Comment"], [class*="item"], [class*="Item"], li, tr, section, div'),
          ).filter(
          ...
          const editors = rows
            .flatMap((row) =>
              Array.from(row.querySelectorAll('textarea, [contenteditable="true"], input[type="text"], [role="textbox"]')),
            )
            .filter((node): node is HTMLElement => visible(node))
            .map((node) => normalize('value' in node ? (node as HTMLInputElement).value : node.textContent));
          const replyStillInEditor = editors.some((value) => value.includes(replyPrefix));
        ...
        for (let i = 0; i < 10; i += 1) {
          ...
          if (
            ...
            (!state.replyStillInEditor && (state.bodyHasReply || state.editorGone))
          ) {
            return state;
          }
          await delay(600);
      ...
      { targetText: input.targetText, replyText: input.replyText },
    );
    const sent =
      Boolean(verify.bodyOnlyReplyVisible) ||
      (!verify.replyStillInEditor && (Boolean(verify.bodyHasReply) || Boolean(verify.editorGone)));
    ...
    return {
      ...
      readbackText: verify.bodyHasReply ? input.replyText : undefined,
      replyVisible: Boolean(verify.bodyHasReply),
      nextAction: sent ? undefined : '检查抖音是否弹出验证、账号是否限制发送或目标评论是否已变化。',
    };
  }

  private async openMessageConversation(frame: Page | Frame, targetText: string) {
    const clickTarget = await frame.evaluate((targetText) => {
      const normalize = (value: unknown) =>
        String(value || '')
          .replace(/\s+/g, ' ')
          .replace(/[\u200b\u200c\u200d\ufeff]/g, '')
      ...
      const candidates = Array.from(
        ...
        .map((node) => {
          ...
          const score =
            (rowLike ? 100 : 0) +
            (/\d{1,2}:\d{2}|昨天|今天|星期|分钟前|小时前/.test(text) ? 20 : 0) -
      ...
      if (!picked) return null;
      return {
        x: picked.rect.x + Math.min(Math.max(picked.rect.width * 0.28, 80), picked.rect.width / 2),
  ...
  private async readWechatChannelWithLocalBrowser(
    ...
    const targetKind = input.taskType === 'comment-reply' ? 'comments' : 'messages';
    ...
    try {
      ...
      const itemKey = targetKind === 'comments' ? 'looksLikeComment' : 'looksLikeMessage';
      let scanResult: Record<string, any> = {};
      let navigationState: Record<string, any> = {};
      let items: Array<Record<string, any>> = [];
      const evaluateWechatChannelScan = async (): Promise<Record<string, any> & { items: Array<Record<string, any>> }> => {
        const scan = (await contentFrame
          .evaluate(this.wechatChannelScanScript(), {
            limit,
            itemKey,
          })
          .catch((error) => ({
      ...
      if (targetKind === 'comments') {
        ...
        items = this.mergeWechatChannelCandidates(scanResult.items || [], trace.events, targetKind, limit);
      } else {
        const tabStates: Array<Record<string, any>> = [];
        for (const labelName of ['current', '私信', '打招呼消息']) {
          const tabState =
            labelName === 'current'
              ? await this.prepareWechatChannelMessageTab(page)
              : await this.clickWechatChannelMessageTab(page, labelName);
  ...
  private async performWechatChannelInteraction(
    ...
    try {
      ...
      const contentFrame = await this.wechatChannelContentFrame(page, targetKind);
      await page.waitForTimeout(2500).catch(() => undefined);
      ...
      const missingStatus = targetKind === 'comments' ? 'comment_missing' : 'message_missing';
      let actionResult: any = await contentFrame.evaluate(
        async ({ targetText, replyText, send, missingStatus, traceTarget }) => {
          ...
          const targetVariants = Array.from(
            new Set([normalizedTarget, normalizedTarget.replace(/\]\[/g, '] [')].filter(Boolean)),
          );
            ...
            targetNode =
              allNodes
                ...
                .sort((a, b) => {
                  const ar = (a as HTMLElement).getBoundingClientRect();
                  const br = (b as HTMLElement).getBoundingClientRect();
                  const aBubble = /bubble|content-left|message/i.test(String((a as HTMLElement).className || '')) ? 0 : 1;
                  const bBubble = /bubble|content-left|message/i.test(String((b as HTMLElement).className || '')) ? 0 : 1;
                  return aBubble - bBubble || (ar.width * ar.height) - (br.width * br.height);
                })[0] || null;
          }
          if (isMessageTarget && !targetNode) {
            targetNode =
              Array.from(document.querySelectorAll('.session-wrap, [class*="session"], [class*="Session"]'))
                .filter((node) => visible(node))
                .filter((node) => {
                  const text = normalize((node as HTMLElement).innerText || node.textContent);
                ...
                .sort((a, b) => {
                  ...
                  const classPriority =
                    (String((a as HTMLElement).className || '').includes('session-wrap') ? 0 : 1) -
                    (String((b as HTMLElement).className || '').includes('session-wrap') ? 0 : 1);
          ...
          if (!targetNode && targetAuthor) {
            targetNode =
              allNodes
                .filter((node) => visible(node) && normalize((node as HTMLElement).innerText || node.textContent).includes(targetAuthor))
                .sort((a, b) => {
                  const ar = (a as HTMLElement).getBoundingClientRect();
                  const br = (b as HTMLElement).getBoundingClientRect();
                  return (String((a as HTMLElement).className || '').includes('comment-item') ? 0 : 1) -
                    (String((b as HTMLElement).className || '').includes('comment-item') ? 0 : 1) ||
          ...
          const root = isMessageTarget
            ...
            : targetNode.closest('.comment-item, tr, li, [class*="comment"], [class*="Comment"], [class*="message"], [class*="Message"], [class*="item"], [class*="Item"]') || document.body;
	          if (!isMessageTarget) {
	            const commentRect = (root as HTMLElement).getBoundingClientRect?.() || targetRect;
	            const replyTriggers = Array.from(document.querySelectorAll('.action-item, button, [role="button"], span, div, a'))
	              .filter((node) => {
	                if (!visible(node) || normalize((node as HTMLElement).innerText || node.textContent) !== '回复') return false;
	                const rect = (node as HTMLElement).getBoundingClientRect();
	                const inRoot = root.contains(node);
	                const nearTarget =
	                  rect.y >= Math.min(commentRect.y, targetRect.y) - 40 &&
	                  rect.y <= Math.max(commentRect.y + commentRect.height, targetRect.y + targetRect.height) + 180 &&
	              ...
	              .sort((a, b) => {
	                ...
	                const actionPriority =
	                  (String((a as HTMLElement).className || '').includes('action-item') ? 0 : 1) -
	                  (String((b as HTMLElement).className || '').includes('action-item') ? 0 : 1);
	                const aDistance = Math.abs(ar.y - targetRect.y) + Math.abs(ar.x - targetRect.x);
	                const bDistance = Math.abs(br.y - targetRect.y) + Math.abs(br.x - targetRect.x);
	                return actionPriority || aDistance - bDistance || (ar.width * ar.height - br.width * br.height);
	              });
	          ...
	          const findEditor = () => {
	            ...
	            const rootRect = (root as HTMLElement).getBoundingClientRect?.() || targetRect;
	            for (const selector of selectors) {
	              const nodes = Array.from(document.querySelectorAll(selector))
	                .filter(visible)
	                .map((node) => {
	                  const rect = (node as HTMLElement).getBoundingClientRect();
	                  const placeholder = normalize((node as HTMLElement).getAttribute('placeholder'));
	                  const inRoot = root.contains(node);
	                  const nearTarget =
	                    rect.y >= Math.min(rootRect.y, targetRect.y) - 40 &&
	                    rect.y <= Math.max(rootRect.y + rootRect.height, targetRect.y + targetRect.height) + 220 &&
	                    rect.x >= Math.min(rootRect.x, targetRect.x) - 80;
	                  const replyLike =
	                    /^回复|写评论|评论/.test(placeholder) ||
	                    /reply|comment|textarea|input/i.test(String((node as HTMLElement).className || ''));
	                  const score =
	                    (inRoot ? 0 : 40) +
	                    (nearTarget ? 0 : 30) +
	                    (replyLike ? 0 : 12) +
	          ...
	          if (!editor) {
	            const editorCount = document.querySelectorAll('textarea, [contenteditable="true"], input[type="text"], [role="textbox"]').length;
	            const replyCount = Array.from(document.querySelectorAll('button, [role="button"], span, div, a'))
	              .filter((node) => normalize((node as HTMLElement).innerText || node.textContent) === '回复').length;
	            return {
	              status: 'editor_missing',
	              sent: false,
          ...
          if (!send) {
            setEditorValue(editor, replyText);
            await delay(500);
            return { status: 'draft_filled', sent: false, message: '视频号回复草稿已填入，未点击发送。', editorTag: (editor as HTMLElement).tagName };
          }
          const editorRect = editor.getBoundingClientRect();
          return {
            status: 'editor_found',
      ...
      if (input.action === 'send' && actionResult.status === 'editor_found') {
        ...
        await page.mouse.click(
          Number(editorRect.x || 0) + Math.min(Math.max(Number(editorRect.width || 1) / 2, 2), 180),
          Number(editorRect.y || 0) + Math.max(Number(editorRect.height || 1) / 2, 1),
        );
        await page.keyboard.press('Meta+A');
        await page.keyboard.press('Backspace');
        await page.keyboard.insertText(input.replyText);
        ...
        actionResult = await contentFrame.evaluate(
          async ({ replyText, traceTarget }) => {
            ...
            const isDisabled = (node: Element) => {
              const aria = String(node.getAttribute('aria-disabled') || '').toLowerCase();
              return Boolean((node as HTMLButtonElement).disabled) || aria === 'true' || /disabled/.test(String((node as HTMLElement).className || '').toLowerCase());
            };
            const candidates = Array.from(document.querySelectorAll('button, [role="button"], span, div'))
              .filter((node): node is HTMLElement => {
                if (!visible(node) || isDisabled(node)) return false;
                ...
                if (!/^(发送|回复|提交|评论)$/.test(text)) return false;
                const rect = (node as HTMLElement).getBoundingClientRect();
                const isRealButton = (node as HTMLElement).tagName === 'BUTTON' || node.getAttribute('role') === 'button';
                if (!isRealButton && (rect.width > 180 || rect.height > 64)) return false;
              ...
              .sort((a, b) => a.priority - b.priority || a.distance - b.distance);
            if (!candidates.length) {
              return { status: 'send_failed', sent: false, message: '回复已写入，但没有识别到视频号发送按钮。', editorTag: editor.node.tagName };
            }
          ...
          { replyText: input.replyText, traceTarget },
        );
      }
      return {
        status:
          actionResult.status === 'comment_missing' || actionResult.status === 'message_missing'
            ? actionResult.status
        ...
        readbackText: actionResult.readbackText || (actionResult.replyVisible ? input.replyText : undefined),
        replyVisible: Boolean(actionResult.replyVisible),
        nextAction: actionResult.sent || actionResult.status === 'draft_filled'
          ? undefined
  ...
  private async openWechatChannelPage(page: Page, targetKind: 'comments' | 'messages'): Promise<void> {
    const targetUrl =
      targetKind === 'comments'
  ...
  private async wechatChannelContentFrame(
    ...
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
      const frame = page.frames().find((candidate) => candidate.url().includes(expected));
  ...
  private async ensureWechatChannelReadyPage(page: Page, targetLabel: string): Promise<void> {
    const url = page.url().toLowerCase();
    const bodyText = await this.pageText(page, 1500).catch(() => '');
    const loggedOut =
      url.includes('login') ||
      (/一站式服务/.test(bodyText) && /让创作更简单/.test(bodyText) && /多人运营/.test(bodyText));
    if (loggedOut) {
      throw new Error(`视频号账号未登录，不能读取或回复${targetLabel}。请先重新登录视频号账号。`);
    }
  }
    ...
    const result = await frame
      .evaluate(
        ({ targetText }) => {
          ...
          const parseCommentCount = (text: string) => {
            const numbers = (normalize(text).match(/\b\d+\b/g) || []).map(Number);
  ...
  private async prepareWechatChannelMessageTab(
    ...
    for (const label of ['打招呼消息', '私信', '全部私信']) {
      const clicked = await frame
        .evaluate((label) => {
          ...
          const nodes = Array.from(document.querySelectorAll('button, [role="button"], div, span, a'))
            .filter((node) => visible(node) && normalize((node as HTMLElement).innerText || node.textContent) === label)
            .sort((a, b) => (a as HTMLElement).getBoundingClientRect().y - (b as HTMLElement).getBoundingClientRect().y);
          const node = nodes[0] as HTMLElement | undefined;
          if (!node) return false;
          node.click();
          return true;
  ...
  private async clickWechatChannelMessageTab(page: Page, label: string): Promise<Record<string, any>> {
    const frame = await this.wechatChannelContentFrame(page, 'messages');
    const clicked = await frame
      .evaluate((label) => {
        const normalize = (value: unknown) => String(value || '').replace(/\s+/g, ' ').trim();
      ...
      .catch(() => false);
    if (clicked) {
      await page.waitForTimeout(2600).catch(() => undefined);
    }
    return { selectedTab: label, clicked: Boolean(clicked) };
  }
      ...
      .evaluate(
        ({ targetText }) => {
          ...
          const looksOwnReply = (text: string) =>
            /收到，看到你发的是|你把具体想咨询的问题发我|有具体问题直接发我|我按实际情况/.test(text);
          const rows = Array.from(document.querySelectorAll('.session-wrap, .scroll-list .session-wrap, [class*="session-wrap"], [class*="SessionWrap"]'))
            ...
            .map((node) => {
              const rect = (node as HTMLElement).getBoundingClientRect();
              const text = normalize((node as HTMLElement).innerText || node.textContent);
              const author = normalize((node as HTMLElement).querySelector('.name, [class*="name"], [class*="Name"], .title')?.textContent || '');
              const content = normalize((node as HTMLElement).querySelector('.feed-info, [class*="feed-info"], [class*="content"], [class*="Content"], [class*="desc"], [class*="Desc"]')?.textContent || '');
  ...
  private startWechatChannelResponseTrace(
    ...
    const handler = async (response: import('playwright').Response) => {
      ...
      const shouldCapture =
        ...
        ['comment', 'private', 'message', 'msg', 'session', 'post_list'].some((fragment) =>
          lowerUrl.includes(fragment),
        );
      if (!shouldCapture) return;
      try {
        const text = await response.text();
        const candidates = this.extractWechatChannelCandidatesFromPayload(text, lowerUrl, limit);
        const event: DouyinImTraceEvent = {
          kind: 'responseBody',
          url: url.slice(0, 500),
          status: response.status(),
  ...
  private extractWechatChannelCandidatesFromPayload(
    ...
    const addCandidate = (
      ...
      if (extra) {
        for (const keyName of ['commentId', 'sessionId', 'username', 'readFlag', 'createTime']) {
    ...
    const walk = (value: unknown, path = '') => {
      ...
      if (value && typeof value === 'object' && !Array.isArray(value)) {
        ...
        if (['msgContent', 'messageContent', 'lastMsg', 'lastMsgContent', 'content'].some((key) => key in obj)) {
          const author = obj.nickname || obj.name || obj.sessionName || obj.fromNickname;
          for (const key of ['msgContent', 'messageContent', 'lastMsgContent', 'lastMsg', 'content', 'summary']) {
            if (typeof obj[key] !== 'string') continue;
            const kind = defaultKind === 'message' ? 'message' : (defaultKind as 'comment' | 'message');
            addCandidate(
              kind,
              obj[key],
              `${path}.${key}`,
  ...
  private mergeWechatChannelCandidates(
    ...
    const merged: Array<Record<string, any>> = [];
    const seen = new Set<string>();
    const expectedKey = targetKind === 'comments' ? 'looksLikeComment' : 'looksLikeMessage';
  ...
  private findWechatChannelTraceCandidate(
    ...
    for (const event of [...(trace || [])].reverse()) {
      for (const item of event.wechatChannelCandidates || []) {
        ...
        const text = this.normalizeInteractionText(String(item.text || ''));
        if (!normalizedTarget || text === normalizedTarget || text.includes(normalizedTarget) || normalizedTarget.includes(text)) {
          return item;
        }
      }
    }
  ...
  private looksLikeWechatChannelCustomerText(text: string): boolean {
    ...
    if (['评论', '私信', '打招呼消息', '全部', '视频', '图文', '回复', '发送', '删除', '点赞'].includes(normalized)) {
      return false;
    }
    if (['私信 打招呼消息', '全部私信', '全部评论', '评论权限 写评论', '评论权限', '写评论'].includes(normalized)) {
      return false;
    }
    if (/^共\d+个$/.test(normalized)) return false;
    if (/ 20/.test(normalized) && /20\d{2}年\d{1,2}月\d{1,2}日\s+\d{1,2}:\d{2}/.test(normalized)) return false;
    if (/^20\d{2}年\d{1,2}月\d{1,2}日\s+\d{1,2}:\d{2}$/.test(normalized)) return false;
    if (/^.+?\s+\d{1,2}月\d{1,2}日\s+\d{1,2}:\d{2}$/.test(normalized)) return false;
    if (normalized.startsWith('共') && (normalized.includes('条评论') || normalized.includes('个'))) return false;
    if (/20\d{2}[/-]\d{1,2}[/-]\d{1,2}/.test(normalized) && (normalized.includes('#') || normalized.length > 36)) return false;
    if (/^\d+|20\d{2}[/-]\d{1,2}[/-]\d{1,2}|\d{1,2}:\d{2}|刚刚|今天|昨天$/.test(normalized)) return false;
  ...
  private async prepareWechatMessageSession(frame: Page | Frame, targetText: string) {
    await frame.evaluate(
      ({ targetText }) => {
        ...
        for (const label of labels) {
          const node = Array.from(document.querySelectorAll('button, [role="button"], div, span, a'))
            .find((item) => visible(item) && normalize(item.textContent) === label);
          if (node) {
            try {
              (node as HTMLElement).click();
            } catch {}
  ...
  private async selectWechatCommentWork(frame: Page | Frame, targetText: string) {
    await frame.evaluate(
      ({ targetText }) => {
        const normalize = (value: unknown) => String(value || '').replace(/\s+/g, ' ').trim();
        const visible = (node: Element | null): node is HTMLElement => {
        ...
        const isNavText = (text: string) =>
          /^(视频|图文|视频 图文|全部|全部视频|全部图文|评论|评论管理|互动管理|视频号助手|视频号助手 · 评论|搜索|筛选|导出)$/.test(text);
        const bodyText = normalize(document.body.innerText || document.body.textContent || '');
        const hasLoadedCommentDetail =
          /回复|删除|举报|点赞|刚刚|今天|昨天|\d{1,2}:\d{2}|分钟前|小时前/.test(bodyText) &&
          !isNavText(normalize(targetText));
        if (targetText && bodyText.includes(targetText) && hasLoadedCommentDetail) return;
        const rows = Array.from(document.querySelectorAll('li, tr, section, div'))
          .map((node) => {
            if (!visible(node)) return false;
            const rect = (node as HTMLElement).getBoundingClientRect();
            ...
            const hasWorkSignal =
              /评论|\d{1,2}:\d{2}|20\d{2}[/-]\d{1,2}[/-]\d{1,2}|昨天|今天|分钟前|小时前/.test(text);
            const hasNonZeroCount = /(?:评论|留言)?\s*[1-9]\d*\s*(?:条|$)|[1-9]\d*\s*(?:评论|留言)/.test(text);
            const score =
          ...
          .filter(Boolean) as Array<{ node: HTMLElement; score: number; y: number }>;
  ...
  private async waitForOwnerPage(target: Page | Frame, ms: number): Promise<void> {
    ...
    const rows = await targetFrame.evaluate(
      ({ limit, isMessage, platform }) => {
        ...
        if (platform === 'douyin' && isMessage) {
          ...
          const hasConversationHint = /未读|分钟前|小时前|昨天|今天|\d{1,2}:\d{2}|回复|发送/.test(bodyText);
          const hasEmptyState = /暂无私信|暂无消息|没有私信|没有收到私信|还没有收到私信|暂无会话/.test(bodyText);
          if (!hasEmptyState && visibleLoaders > 0 && (!hasConversationHint || contentText.length < 20)) {
            return [];
          }
        }
        const exactNoise = new Set([
          ...
          '首页', '活动管理', '内容管理', '互动管理', '变现中心',
          '创作中心', '通知', '网址', '抖音', '发送', '搜索',
          '全部', '朋友私信', '陌生人私信', '群消息', '加载中', '暂无',
          '高清发布', '发布视频', '发布图文', '站内信', '星图',
          '关注管理', '粉丝管理', '评论管理', '弹幕管理', '私信管理',
          '我知道了', '稍后再看', '关闭', '加载中，请稍候...',
          '视频', '图文', '视频 图文', '全部视频', '全部图文',
          '视频号助手', '视频号助手 · 评论',
          '私信', '打招呼消息', '全部私信', '视频号助手 · 私信',
        ]);
        const containsNoise = [
          '你收到一条新类型消息',
          '请打开抖音app查看',
        ...
        const rowLooksUnreplyable = (text: string) =>
          /你收到一条新类型消息|请打开抖音\s*app\s*查看|分享\[视频\]|\[视频\]|\[图片\]|该消息类型暂不支持|当前版本暂不支持|只有群主和管理员可以发消息|但你可以浏览/.test(text);
        const looksLikeDouyinGroupConversation = (text: string) =>
          /群消息|粉丝群|群聊|官方群|交流群|客户群|社群/.test(text) ||
          /群\s*(?:\d{1,2}:\d{2}|刚刚|今天|昨天|\d+分钟前|\d+小时前)/.test(text);
        const cleanDouyinMessageText = (value: string) => normalize(value)
          .replace(/^(?:刚刚|今天|昨天|\d{1,2}:\d{2}|\d+分钟前|\d+小时前)\s*/, '')
          .replace(/\s*(置顶|已读|删除|回复|举报|拉黑|标记未读)+\s*$/g, '')
          .replace(/\s*(置顶|已读|删除|回复|举报|拉黑|标记未读)\s*/g, '')
          .trim();
        const cleanDouyinCommentText = (value: string) => normalize(value)
          .replace(/^(?:刚刚|今天|昨天|\d{1,2}:\d{2}|\d+分钟前|\d+小时前)\s*/, '')
        ...
        const noise = [
          '首页', '评论管理', '私信管理', '互动管理', '创作者中心', '视频号助手',
          '全部', '搜索', '筛选', '发送', '回复', '取消', '确定', '暂无', '加载中',
          '隐私政策', '用户服务协议', '平台通知', '系统通知',
        ];
        const looksLikeNavigationNoise = (value: string, rowValue: string) =>
          /通知\s*网址\s*抖音|高清发布|作品管理|内容管理|互动管理|创作者服务中心/.test(value) ||
          (/通知/.test(rowValue) && /网址/.test(rowValue) && /抖音/.test(rowValue));
        const nodes = Array.from(
          document.querySelectorAll(
        ...
        if (platform === 'wechat-channel' && isMessage) {
          const isNavText = (text: string) =>
            /^(私信|打招呼消息|全部私信|消息|用户消息|视频号助手|视频号助手 · 私信|搜索|筛选|暂无|加载中)$/.test(text);
          const cleanWechatMessageText = (value: string) => normalize(value)
            .replace(/^(?:刚刚|今天|昨天|\d{1,2}:\d{2}|\d+分钟前|\d+小时前)\s*/, '')
            .replace(/\s*(?:已读|未读|回复|删除|举报|拉黑|标记未读)+\s*/g, ' ')
            .replace(/\s+/g, ' ')
            .trim();
          const rowNodes = Array.from(
            ...
            .filter((node) => {
              ...
              if (rect.x < 160 || rect.y < 120 || rect.width < 180 || rect.height < 32) return false;
              if (rowText.length < 2 || rowText.length > 360) return false;
              if (isNavText(rowText) || exactNoise.has(rowText)) return false;
              if (/视频号助手\s*·\s*私信/.test(rowText) && rowText.length < 40) return false;
              return /未读|回复|发送|刚刚|今天|昨天|\d{1,2}:\d{2}|分钟前|小时前|客户|用户|粉丝/.test(rowText);
            })
            .sort(
              (a, b) =>
                (a as HTMLElement).getBoundingClientRect().y -
          ...
          for (const node of rowNodes) {
            const childTexts = Array.from(node.querySelectorAll('span, p, div'))
              ...
              .map((child) => cleanWechatMessageText(normalize(child.textContent)))
              .filter(Boolean)
              .filter((text, index, arr) => arr.indexOf(text) === index)
              .filter((text) => text.length >= 2 && text.length <= 180)
              .filter((text) => !isNavText(text) && !exactNoise.has(text) && !noise.includes(text))
              .filter((text) => !/^(?:回复|发送|删除|举报|刚刚|今天|昨天|\d{1,2}:\d{2}|\d+分钟前|\d+小时前|\d+)$/.test(text));
            const rowText = cleanWechatMessageText(normalize(node.textContent));
            const text =
              childTexts.find((item) => /[？?吗呢吧呀哦]|价格|多少|怎么|哪里|联系|电话|微信|私信|预约|内容|具体|可以|想问|请问|买|发|帮|看|在吗/.test(item)) ||
            ...
            seen.add(key);
            const rect = (node as HTMLElement).getBoundingClientRect();
            const sender = childTexts.find((item) => item !== text && item.length <= 24);
            candidates.push({
              text,
              sender,
              looksLikeMessage: true,
        ...
        if (platform === 'wechat-channel' && !isMessage) {
          const isNavText = (text: string) =>
            /^(视频|图文|视频 图文|全部|全部视频|全部图文|评论|评论管理|互动管理|视频号助手|视频号助手 · 评论|搜索|筛选|导出)$/.test(text);
          const cleanWechatCommentText = (value: string) => normalize(value)
            .replace(/^(?:刚刚|今天|昨天|\d{1,2}:\d{2}|\d+分钟前|\d+小时前)\s*/, '')
            .replace(/\s*(?:作者|回复|删除|举报|点赞|置顶|展开|收起)+\s*/g, ' ')
          ...
          const rowNodes = Array.from(
            ...
            .filter((node) => {
              const rect = (node as HTMLElement).getBoundingClientRect();
              const rowText = normalize(node.textContent);
              if (rect.x < window.innerWidth * 0.42 || rect.y < 120 || rect.width < 180 || rect.height < 28) return false;
              if (rowText.length < 2 || rowText.length > 360) return false;
              if (isNavText(rowText) || exactNoise.has(rowText)) return false;
              if (/视频号助手\s*·\s*评论/.test(rowText) && rowText.length < 40) return false;
              return /回复|删除|举报|点赞|刚刚|今天|昨天|\d{1,2}:\d{2}|分钟前|小时前/.test(rowText);
            })
            .sort(
              (a, b) =>
                (a as HTMLElement).getBoundingClientRect().y -
          ...
          for (const node of rowNodes) {
            const childTexts = Array.from(node.querySelectorAll('span, p, div'))
              .filter((child) => visible(child))
              .map((child) => cleanWechatCommentText(normalize(child.textContent)))
              .filter(Boolean)
              .filter((text, index, arr) => arr.indexOf(text) === index)
              .filter((text) => text.length >= 2 && text.length <= 180)
              .filter((text) => !isNavText(text) && !exactNoise.has(text) && !noise.includes(text))
            ...
            const text =
              childTexts.find((item) => /[？?吗呢吧呀哦]|价格|多少|怎么|哪里|联系|电话|微信|私信|预约|内容|具体|好听|不错|可以|想问|请问|买|发|帮|看/.test(item)) ||
              childTexts.find((item) => item.length >= 4) ||
              (rowText.length >= 4 && !isNavText(rowText) ? rowText : '');
            if (!text || isNavText(text)) continue;
            const key = text.slice(0, 80);
        ...
        if (platform === 'douyin' && isMessage) {
          const rowNodes = Array.from(
            ...
            .filter((node) => {
              const rect = (node as HTMLElement).getBoundingClientRect();
              return rect.x >= 220 && rect.y >= 130 && rect.width >= 260 && rect.height >= 34;
            })
            .sort(
              (a, b) =>
                (a as HTMLElement).getBoundingClientRect().y -
          ...
          for (const node of rowNodes) {
            ...
            if (!/\d{1,2}:\d{2}|星期|昨天|今天|分钟前|小时前/.test(rowText)) continue;
            const childTexts = Array.from(node.querySelectorAll('span, p, div'))
              .filter((child) => visible(child))
              ...
              .filter((text) => !containsNoise.some((item) => text.includes(item)))
              .map(cleanDouyinMessageText)
              .filter(Boolean);
            const textPool = childTexts.length ? childTexts : [cleanDouyinMessageText(rowText)];
            const bodyCandidates = textPool
              .filter((text) => !/^\d{1,2}:\d{2}$|^星期.|^昨天$|^今天$|^\d+分钟前$|^\d+小时前$/.test(text))
              .filter((text) => !/^[\u4e00-\u9fa5A-Za-z0-9_·\-]{1,24}(?:📷|✅|✔|V)?$/.test(text))
              .filter((text) => /[？?吗呢吧呀哦]|预约|价格|多少|怎么|哪里|联系|电话|微信|私信|地址|入口|购买|商品|门店|想问|请问|可以|需要|发我|帮/.test(text));
            const text = bodyCandidates[0] || textPool.find((item) => item.length >= 6 && !/^\S{1,24}(?:📷)?\d{1,2}:\d{2}$/.test(item));
            ...
            seen.add(key);
            const rect = (node as HTMLElement).getBoundingClientRect();
            const nickname = textPool.find((item) => item !== text && item.length <= 24);
            candidates.push({
              text,
              sender: nickname,
              looksLikeMessage: true,
        ...
        if (platform === 'douyin' && !isMessage) {
          const rowNodes = Array.from(
            ...
            .filter((node) => {
              const rect = (node as HTMLElement).getBoundingClientRect();
              const rowText = normalize(node.textContent);
              if (rect.x < 240 || rect.y < 300 || rect.width < 260 || rect.height < 44) return false;
              if (looksLikeNavigationNoise(rowText, rowText)) return false;
              if (!/回复|删除|举报|点赞|刚刚|今天|昨天|\d{1,2}:\d{2}|分钟前|小时前/.test(rowText)) return false;
              return true;
            })
            .sort(
              (a, b) =>
          ...
          for (const node of rowNodes) {
            const childTexts = Array.from(node.querySelectorAll('span, p, div'))
              .filter((child) => visible(child))
              .map((child) => cleanDouyinCommentText(normalize(child.textContent)))
              .filter(Boolean)
              .filter((text, index, arr) => arr.indexOf(text) === index)
              .filter((text) => text.length >= 2 && text.length <= 180)
              ...
              .filter((text) => !/^[\u4e00-\u9fa5A-Za-z0-9_·\-]{1,24}$/.test(text));
            const rowText = cleanDouyinCommentText(normalize(node.textContent));
            const text =
              childTexts.find((item) => /[？?吗呢吧呀哦]|价格|多少|怎么|哪里|联系|电话|微信|私信|预约|内容|具体|好听|不错|可以|想问|请问|买|发|帮|看/.test(item)) ||
              childTexts.find((item) => item.length >= 4) ||
              (rowText.length >= 4 && !isDouyinCommentControlText(rowText) ? rowText : '');
        ...
        for (const node of nodes) {
          ...
          if (platform === 'douyin' && isMessage && rowLooksUnreplyable(rowText)) continue;
          if (platform === 'douyin' && isMessage) {
          ...
          if (platform === 'douyin' && isMessage && /^[\u4e00-\u9fa5A-Za-z0-9_·\-]{1,24}(?:📷|✅|✔|V)?$/.test(text) && !/[？?吗呢吧呀哦]|预约|价格|多少|怎么|哪里|联系|电话|微信|私信|在吗|在哪|要|买|发|帮|看/.test(text)) continue;
          if (/^\d+$|^\d{1,2}:\d{2}$|^\d+分钟前$|^\d+小时前$|^刚刚$|^昨天$|^今天$/.test(text)) continue;
          if (noise.some((item) => text === item || (text.length > 8 && text.includes(item)))) continue;
          const signal = isMessage
            ? /私信|消息|回复|未读|刚刚|今天|昨天|\d{1,2}:\d{2}|分钟前|小时前/.test(text)
            : /回复|删除|点赞|评论|刚刚|今天|昨天|\d{1,2}:\d{2}|分钟前|小时前/.test(text);
  ...
  private snapshotContainsReply(snapshotText: string | undefined, replyText: string) {
