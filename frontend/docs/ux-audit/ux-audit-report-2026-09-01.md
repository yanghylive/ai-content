...
1. **视频生成入口过多且不可区分**:内容运营页同时给"AI 生视频"（ai-video-gen）与"视频成片"（video-studio）与"图生视频"（video-generation）三个相似入口,普通用户无法判断差异,建议按使用场景合并为一个「AI 视频」入口 + 二级细分。
2. **孤儿页面造成死胡同**:从能力页/素材页进入 `/video-workshop` 或 `/seedance-video` 后,页面内没有回到内容运营或上一级的导航（需要验证返回链）。
...
7. `war-room` 归属统一（互动中心 or 执行中心,二选一）
8. 用量与费用路径迁到 `/settings/costs`（或保留路径但明确场景）
9. 命名规范:`monitor-new`/`report-new` 改为 `/new` 子路径
