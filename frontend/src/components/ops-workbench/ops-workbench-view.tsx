...
import type {
  ...
  OpsWorkbenchWechatSendPolicy,
} from "./wechat-card";
import {
  wechatLiveAutoReplySkill,
  wechatSessionAutoReplySkill,
...
export function OpsWorkbenchView() {
  ...
  const [activeDouyinTask, setActiveDouyinTask] =
  ...
  }, [agentS.agentSSession?.id, getAgentSEvents, setAgentSEvents]);
  useEffect(() => {
  ...
  const handleStartDouyinCommentReply = useCallback(async () => {
    ...
    try {
      ...
      addToast({
        title: "启动失败",
        description: toPublicError(error, "评论回复任务未启动，请重试。"),
        color: "danger",
      });
    } finally {
  ...
  const handleStartDouyinDirectMessageReply = useCallback(async () => {
    ...
    try {
      ...
      addToast({
        title: "启动失败",
        description: toPublicError(error, "私信回复任务未启动，请重试。"),
        color: "danger",
      });
    } finally {
      setDouyinTaskBusy(false);
  ...
  const handleDouyinSendModeChange = useCallback(
    (mode: "auto-send" | "approval-send") => {
      douyin.setDouyinSendMode(mode);
    },
    [douyin],
  );
  const handleRunWechatTask = useCallback(
    async (mode: WechatExecutionMode) => {
      const contact = wechat.wechatReplyContact.trim();
      const draft = wechat.wechatReplyDraft.trim();
      if (!contact || !draft) {
        addToast({
          title: "缺少联系人或回复内容",
          description: "微信真实发送必须先明确目标联系人和要发的内容。",
          color: "danger",
        });
        return;
      }
      try {
        ...
        await agentS.runAgentSTask({
          ...
          localControllerPermissionMode:
            skillRequest.localControllerPermissionMode,
          commercialExecutionRequested: mode === "auto-send",
        });
        const currentState =
          wechat.wechatBatchState || INITIAL_WECHAT_BATCH_STATE;
        wechat.setWechatBatchState(
          buildWechatModeStartingState(currentState, { mode, contact }),
        );
        ...
        addToast({
          title: "启动失败",
          description: toPublicError(error, "微信回复任务未启动，请重试。"),
          color: "danger",
        });
      }
    },
  ...
  const handleRunWechatLiveConversation = useCallback(async () => {
    ...
    try {
      ...
      const currentState =
        wechat.wechatBatchState || INITIAL_WECHAT_BATCH_STATE;
      wechat.setWechatBatchState(
        buildWechatModeStartingState(currentState, {
          mode: "auto-send",
          contact: "当前微信会话",
        }),
      );
      ...
      addToast({
        title: "启动失败",
        description: toPublicError(error, "微信当前会话回复任务未启动，请重试。"),
        color: "danger",
      });
    }
  }, [wechat, agentS]);
  const handleWechatContactChange = useCallback(
    (contact: string) => {
      wechat.setWechatReplyContact(contact);
    },
    [wechat],
  );
  const handleWechatDraftChange = useCallback(
    (draft: string) => {
      wechat.setWechatReplyDraft(draft);
    },
    [wechat],
  );
  const handleWechatContextChange = useCallback(
    (context: string) => {
  ...
  const handleSkipWechat = useCallback(() => {
    const currentState = wechat.wechatBatchState || INITIAL_WECHAT_BATCH_STATE;
    wechat.setWechatBatchState(
      buildWechatSkippedState(currentState, {
  ...
  const effectiveDouyinBatchState =
    douyin.douyinBatchState || INITIAL_DOUYIN_BATCH_STATE;
  const effectiveWechatBatchState =
    wechat.wechatBatchState || INITIAL_WECHAT_BATCH_STATE;
  const douyinTaskRunning =
    activeDouyinTask?.status === "queued" ||
    activeDouyinTask?.status === "running";
  ...
  const douyinTaskDetail = activeDouyinTask
    ? activeDouyinTask.failureReason ||
      activeDouyinTask.nextAction ||
      latestDouyinTaskEvent?.message ||
      activeDouyinTask.statusLabel
    : null;
  const activeDouyinTaskHasReadback =
    hasInteractionReadbackProof(activeDouyinTask);
  ...
  if (activeDouyinTask?.status === "completed" && activeDouyinTaskHasReadback) {
    ...
    activeDouyinTask?.status === "failed" ||
    activeDouyinTask?.status === "blocked" ||
    activeDouyinTask?.status === "skipped"
  ) {
    douyinCardStatus = "attention";
  ...
  const douyinCardProps = {
    ...
    summary: activeDouyinTask
      ? `正在按抖音后台流程处理：${activeDouyinTask.typeLabel}`
    ...
    roundStatusLabel:
      activeDouyinTask?.statusLabel ||
      effectiveDouyinBatchState.lastOutcomeTitle ||
      "准备开始",
    roundStatusDetail:
      douyinTaskDetail ||
    ...
    progressHint: effectiveDouyinBatchState.active
      ...
      : "尚未开始。",
    lastOutcomeTitle: activeDouyinTask
      ? "最近一次任务结果"
      : effectiveDouyinBatchState.lastOutcomeTitle,
    lastOutcomeDetail: activeDouyinTask
      ? [
          douyinTaskDetail,
          activeDouyinTask.status === "completed" &&
          !activeDouyinTaskHasReadback
            ? "未在页面结果中确认本次回复内容，暂不能标记为发送成功。"
            : "",
    ...
    recentOutcomeItems: effectiveDouyinBatchState.recentOutcomes?.map(
      (outcome) => ({
        title:
          outcome.kind === "sent"
            ? "已发送"
    ...
    pauseResumeLabel: effectiveDouyinBatchState.paused
      ? "已暂停"
      : effectiveDouyinBatchState.active
        ? "运行中"
        : "待开始",
    ...
    stageLabel:
      activeDouyinTask?.statusLabel ||
      (effectiveDouyinBatchState.active ? "处理中" : "待开始"),
  ...
  const latestWechatAgentEvent = [...agentS.agentSEvents].sort(
    (a, b) => Number(b.seq || 0) - Number(a.seq || 0),
  )[0];
  const latestWechatAgentStatus = String(
    latestWechatAgentEvent?.status || agentS.agentSSession?.status || "",
  );
  const wechatAgentRunning =
    latestWechatAgentStatus === "running" ||
    latestWechatAgentStatus === "waiting_approval";
  ...
  if (latestWechatAgentStatus === "completed") {
    ...
    latestWechatAgentStatus === "failed" ||
    latestWechatAgentStatus === "cancelled"
  ) {
  ...
  const wechatSendPolicy: OpsWorkbenchWechatSendPolicy =
    wechat.wechatExecutionMode === "auto-send"
      ? "auto-send"
  ...
  const wechatCardProps = {
    ...
    guardState: wechat.wechatReplyContact
      ? ("instruction-guarded" as const)
      : ("not-configured" as const),
    batchHeadline: effectiveWechatBatchState.active
      ? "批量处理中"
      : effectiveWechatBatchState.completed
        ? "本轮已完成"
        : undefined,
    ...
    batchProgressHint: effectiveWechatBatchState.nextCandidate
      ? `下一条候选：${effectiveWechatBatchState.nextCandidate}`
      : undefined,
    lastOutcomeTitle: wechatAgentDetail
      ? "Agent-S 桌面执行结果"
      : effectiveWechatBatchState.lastOutcomeTitle,
    lastOutcomeDetail:
      wechatAgentDetail || effectiveWechatBatchState.lastOutcomeDetail,
    ...
    canControlledSend:
      Boolean(wechat.wechatReplyContact) && !wechatAgentRunning,
    canAutoSend:
      Boolean(wechat.wechatReplyContact && wechat.wechatReplyDraft) &&
      !wechatAgentRunning,
    canSkipCurrent: effectiveWechatBatchState.active,
    sendBlockReason:
      wechat.wechatReplyContact && wechat.wechatReplyDraft
        ? undefined
        : "可以直接处理当前微信聊天；指定联系人发送时再填写联系人和正式回复",
  };
  void wechatCardProps;
  return (
        ...
        <CardBody>
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              <Chip
                color={agentS.agentSStatus?.connected ? "success" : "default"}
              >
