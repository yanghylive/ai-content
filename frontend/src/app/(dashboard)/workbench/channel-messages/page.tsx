...
export default function ChannelMessagesPage() {
  ...
  const visibleOutcome = React.useMemo(() => {
    ...
    return {
      ...
      roundStatusLabel: "正在启动真实视频号私信回复",
      roundStatusDetail:
      ...
      liveEvents: [
        {
          message:
            startingFeedback || "已点击开始，正在启动真实视频号私信任务。",
          level: "info" as const,
  ...
  const handleStartCommentReply = useCallback(async () => {
    ...
    try {
      setTaskBusy(true);
      setStartingFeedback("正在创建任务，马上打开视频号后台读取真实私信。");
      const task = await localEngineApi.createBusinessTask("channel-messages", {
      ...
      addToast({ title: "视频号私信回复任务已启动", color: "success" });
    } catch (error) {
      setStartingFeedback(null);
      addToast({
        title: "启动失败",
  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        <h1 className="text-2xl font-bold">视频号私信回复</h1>
