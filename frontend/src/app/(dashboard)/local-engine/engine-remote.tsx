          ...
          {loading ? (
            ...
            <div className="mx-card" style={{ padding: 8 }}>
              ...
              {screenshot?.capturedAt && (
                <p style={{ fontSize: 10.5, color: "var(--kaypal-v3-muted)", marginTop: 7 }}>
            ...
            <div className="mx-card mx-empty" style={{ padding: 26, textAlign: "center" }}>
              <MonitorPlay width={28} height={28} style={{ color: "var(--kaypal-v3-muted)", margin: "0 auto" }} />
              <p style={{ fontSize: 12.5, color: "var(--kaypal-v3-muted)", marginTop: 9 }}>暂时获取不到桌面画面</p>
              <p style={{ fontSize: 11, color: "var(--kaypal-v3-muted)", marginTop: 4 }}>请确认桌面助手已启动，且已授予屏幕录制权限</p>
            </div>
          )}
                ...
                {Object.entries(status.window as unknown as Record<string, unknown>)
                  ...
                  .map(([key, value], i) => (
                    <div key={key} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 0", borderTop: i > 0 ? "1px solid rgba(142,165,190,.15)" : "none", gap: 10 }}>
                      <span style={{ fontSize: 11.5, color: "var(--kaypal-v3-muted)", flexShrink: 0 }}>{key}</span>
                      <span style={{ fontSize: 12, fontWeight: 600, color: "var(--kaypal-v3-ink)", textAlign: "right", minWidth: 0, overflow: "hidden", textOverflow: "ellipsis" }}>
                        {String(value)}
                      </span>
                    </div>
                  ))}
