...
export default function WechatError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("Wechat route failed:", error);
  }, [error]);
  ...
  return (
    <main className="mx-auto flex min-h-[55vh] w-full max-w-2xl flex-col items-center justify-center gap-5 px-6 text-center">
      ...
      <div>
        <h1 className="text-foreground">微信管理页暂时无法打开</h1>
        <p className="mt-2 text-sm leading-6 text-default-500">
          可以重新加载本页；仍失败时请返回互动中心。
        </p>
      </div>
      <div className="flex flex-wrap justify-center gap-2">
        ...
        <Button as={Link} href="/engagement" startContent={<RefreshCw aria-hidden="true" size={16} />} variant="flat">
          返回互动中心
        </Button>
      </div>
    </main>
