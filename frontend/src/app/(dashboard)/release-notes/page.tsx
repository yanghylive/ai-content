...
const highlights = [
  {
    title: "统一线索对象 + 事件流",
    text: "评论/私信/导入三套线索表收敛为统一 leads 表，线索从评论到客户的完整链路可追溯；新增 lead 创建/转化事件流。",
    icon: <RefreshCw className="h-5 w-5" strokeWidth={1.8} />,
  },
  {
    title: "AI 质量观测落地",
    text: "每次 AI 调用的 prompt/回复快照、耗时、成败、失败原因自动落库，AI 回复质量可量化评估，失败原因不再被吞。",
    icon: <BadgeCheck className="h-5 w-5" strokeWidth={1.8} />,
  },
  {
    title: "多租户双维度 + 成员管理",
    text: "CRM 与增长任务补「操作者维度」actorUserId，成员邀请/移除闭环，为团队协作铺好数据地基。",
...
const improvements = [
  "平台互动适配器统一契约：小红书/抖音/视频号评论读写走统一契约，消除平台分支。",
  "登录状态滑动续期：7 天内用过一次自动续满 14 天，不再定期被强制登出。",
  "获客任务平台与执行账号联动，多账号不再同时选中。",
  "回复截图/回读证据自动落库，真实送达有存证。",
  "增长控制台获客策略/工作流/账号健康/复盘四个入口直达。",
  "素材库补「本地上传」入口，支持直接上传本地素材。",
  "版本号、更新说明统一更新到 v1.1.83。",
];

const fixedIssues = [
  "修复 Windows 任务栏点图标无法唤起主窗口（补单实例锁 + 二次唤起）。",
  "修复新建获客任务平台与账号无关联、前几个账号同时选中无法单选。",
  "修复线索池只能新增不能删除（补单条删除 + 批量删除）。",
  "修复自动评分无验证入口（线索卡片展示评分依据）。",
  "修复生视频走 kaypal 网关必失败（网关通道补全提交+轮询+入库）。",
  "修复 FLAGSHIP 订阅用户误报「缺少有效商用授权」（登录同步订阅授权）。",
  "修复打包版缺 RedFox/好单库/聚推客/模型同步等配置导致第三方能力全断。",
  "修复 session 固定 14 天无滑动续期导致的定期强制登出。",
];
