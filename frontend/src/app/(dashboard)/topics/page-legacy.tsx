...
export default function TopicsPage() {
  ...
  const handleCreateContent = useCallback(
    async (id: string, type: "article" | "xiaohongshu", title?: string) => {
      setGeneratingIds((prev) => new Set(prev).add(id));
      try {
  ...
  const hasVisibleTopics =
    discoveredTopics.length > 0 || displayTopics.length > 0;
  return (
    <div className="flex flex-col gap-4 max-w-[1400px] mx-auto w-full pb-10">
      <Card className="border-small border-primary/15 bg-background shadow-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-small text-default-500">
                输入一个关键词、事件或一段描述，系统会先主动检索相关信息，再整理出
                3-5 个不同切入点的候选选题。
              </p>
            </div>
          </div>
        ...
        <CardBody className="px-5 pb-5 pt-3">
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_auto]">
            ...
            <div className="flex items-end">
              <Button
                ...
                startContent={
                  <Icon icon="solar:bolt-bold-duotone" width={18} />
                }
          ...
          {discoverySummary ? (
            <div className="mt-4 rounded-[8px] border border-default-200 bg-default-50 p-4">
              <div className="flex flex-wrap items-center gap-2 text-small text-default-600">
                <Chip size="sm" color="primary" variant="flat">
                  归一主题：{discoverySummary.normalizedSeed}
                </Chip>
                <Chip size="sm" variant="flat">
                  目标读者：{discoverySummary.audience}
                </Chip>
              </div>
              <p className="mt-3 text-small text-default-700">
                {discoverySummary.intent}
              </p>
              <p className="mt-3 text-small text-default-500">
                本次已主动检索 {discoverySummary.retrieval.scannedSources}
                个渠道，抓取 {discoverySummary.retrieval.fetchedCount}
                条最新信息， 临时池候选
                {discoverySummary.retrieval.candidateCount} 条，过滤掉
                {discoverySummary.retrieval.rejectedCount}
                条低质量或低相关素材， 最终保留
                {discoverySummary.retrieval.matchedCount} 条，其中新增入库
                {discoverySummary.retrieval.savedCount} 条。
              </p>
      ...
      <header className="rounded-[8px] border-small border-divider flex flex-col md:flex-row items-start md:items-center justify-between gap-4 p-5 bg-background shadow-sm">
        <div className="flex flex-col">
          <h2 className="text-[17px] font-bold leading-6 text-[var(--kaypal-v3-ink)]">
            精选选题库
          </h2>
          <span className="text-small text-default-500 mt-1">
            AI 基于您的采集源深度挖掘出具有爆款潜质的内容方向。
          </span>
        </div>
        <div className="flex w-full md:w-auto items-center gap-3">
          ...
          <Button
            color="primary"
            size="sm"
            startContent={<Icon icon="solar:magic-stick-3-linear" width={18} />}
            isLoading={isMining}
            onClick={handleMine}
          >
            一键挖掘新选题
      ...
      {loading ? (
            ...
            <section className="flex flex-col gap-4">
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-small text-default-500">
                    这批选题围绕你的输入自动挖出，切入点彼此不同，可以直接选择创作文章或小红书。
                  </p>
                </div>
                <Button
                  variant="light"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {discoveredTopics.map((topic) => (
                  <TopicCard
                    ...
                    onCreateContent={(id, type) =>
                      handleCreateContent(id, type, topic.title)
                    }
                  />
                ))}
              </div>
        ...
        <FunctionalEmptyState
          actions={[
            ...
            { href: "/intelligence/search", label: "情报找选题" },
            { href: "/content/articles", label: "文章库" },
          ]}
          description="当前筛选下没有匹配的选题记录。可以清空筛选，或从情报中心寻找新素材后再沉淀为选题。"
          examples={["清空筛选", "情报找选题", "AI 评估", "生成内容"]}
          surface="plain"
          title="当前没有匹配选题"
        />
      )}
...
const TopicCard = ({
  ...
  const chartData = [
    ...
    {
      subject: "降维科普",
      A: topic.details.simplificationPotential,
      fullMark: 100,
    },
  ...
  return (
    <Card className="bg-background dark:bg-content1 shadow-sm border-small border-divider flex flex-col h-full min-w-0 hover:border-primary/50 transition-colors">
      <CardHeader className="flex flex-col items-start gap-2 p-5 pb-3">
        <div className="flex w-full justify-between items-start gap-3">
          <div className="flex items-start gap-2 flex-1 min-w-0">
            {isPublished && (
              <Chip
                ...
                startContent={
                  <Icon icon="solar:check-circle-bold" className="ml-1" />
                }
              >
                已创作
              </Chip>
          ...
          <Chip
            ...
            color={
              isCompleted ? "primary" : isGenerating ? "secondary" : "default"
            }
            className="shrink-0"
          >
            {isCompleted
        ...
        {isGenerating ? (
          <div className="flex flex-col flex-1 items-center justify-center gap-4 py-8">
            <Progress
              ...
              color="secondary"
            />
            <span className="text-small text-default-500 animate-pulse">
              AI 正在进行推演与创作...请稍候
            </span>
          </div>
        ) : !isCompleted ? (
          ...
          <>
            ...
            {isCompleted && topic.reasoning && (
              <div className="bg-warning/10 dark:bg-warning/5 border-l-3 border-warning p-3 rounded-r-medium">
                <p className="text-tiny text-warning-700 dark:text-warning-500 font-medium">
                  ✨ 商业转化分析
                </p>
            ...
            {isCompleted && (
              <div className="w-full min-w-[220px] h-[180px] relative mt-2 -mb-2">
                <ResponsiveContainer width="100%" height={180} minWidth={220} minHeight={160}>
                  <RadarChart
                    cx="50%"
                    cy="50%"
                    outerRadius="75%"
              ...
              <div className="flex flex-wrap gap-1 mt-auto pt-2">
                {topic.keywords.slice(0, 3).map((kw, i) => (
                  <Chip
                    ...
                    size="sm"
                    variant="flat"
                    className="text-tiny h-6 bg-default-100 text-default-600"
                  >
                    {kw}
                  </Chip>
                ))}
      ...
      <Divider className="opacity-50" />
      <div className="p-4 flex justify-between items-center bg-default-50 mt-auto rounded-b-medium">
        <Button
          size="sm"
          variant="light"
          color="danger"
