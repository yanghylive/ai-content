export type AgentSOutcomeStatus = "idle" | "running" | "completed" | "failed" | "waiting_approval" | "cancelled";
...
export function buildAgentSOutcome(
  ...
  const status = String(event?.status || session.status || "running") as AgentSOutcomeStatus;
  const isRunning = status === "running" || status === "waiting_approval";
  const target =
    text(payload.target) ||
    text(payload.contact) ||
    text(payload.assetPath);
  const message =
  ...
  const rawDetail = text(event?.message) || (isRunning ? fallbackRunningDetail : "本机执行已返回状态。");
  const detailParts = [
    rawDetail,
