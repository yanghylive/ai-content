...
export type DouyinBrowserMode = 'comment-reply' | 'direct-message-reply' | 'open-only';
...
export function detectDouyinLoginRequired(source: string): boolean {
  ...
  return (
    ...
    /(抖音创作者中心|抖音后台|创作者后台|creator\.douyin\.com)/i.test(normalized)
  );
}
...
export function extractWechatCandidateContacts(listTexts: string[], exclude: string[] = []) {
  const excludeSet = new Set(exclude.map((item) => item.trim()).filter(Boolean));
...
export function evaluateWechatSendability(
  ...
  if (entityType === 'search-result') {
    return {
      canSend: false,
      reason: '当前落在搜一搜/公众号结果页，这类对象只支持只读分析，不支持自动发送。',
    };
  }
...
export function parseDouyinStructuredStatus(source: string): DouyinStructuredStatus | null {
  const normalized = source.trim();
...
export function buildDouyinOutcomeState(
  ...
  if (kind === 'sent') {
    return {
      ...
      lastOutcomeTitle: mode === 'direct-message-reply' ? '上一条私信已真实发出' : '上一条评论已真实发出',
  ...
  if (kind === 'skipped') {
    return {
      ...
      lastStopTarget: target,
      lastStopReason: detail,
      lastOutcomeTitle: mode === 'direct-message-reply' ? '已跳过当前私信对象' : '已跳过当前评论对象',
      lastOutcomeDetail: detail,
      recentOutcomes: [
        {
  ...
  return {
    ...
    completionSummary: undefined,
    lastOutcomeTitle: mode === 'direct-message-reply' ? '这一条私信处理失败' : '这一条评论处理失败',
    lastOutcomeDetail: detail,
    recentOutcomes: [
      {
...
export function buildDouyinModeStartingState(
  ...
  return {
    ...
    lastOutcomeDetail:
      mode === 'comment-reply'
        ? `正在围绕"${targetHint || '目标留言用户'}"进入评论管理页并准备处理当前这一轮。`
        : mode === 'direct-message-reply'
...
export function buildWechatGuardBlockedState(
  ...
  return {
    ...
    lastOutcomeDetail: reason || '当前微信现场联系人还没完全对齐，这一轮不会继续发送。',
  };
}

export function buildWechatMissingInputState(current: WechatBatchState): WechatBatchState {
  return {
    ...current,
    ...
    lastOutcomeDetail:
      mode === 'read-only-analyze'
        ...
        : mode === 'draft'
          ...
          : mode === 'controlled-send'
            ? `正在围绕"${contact}"准备现场正式回复，稍后会停在发送前确认。`
...
export function buildWechatDraftReadyState(
  ...
  return {
    ...current,
    lastOutcomeTitle: '现场草稿已就位',
    lastOutcomeDetail: `目标联系人"${contact}"的正式回复已经真实写入输入框，当前停在发送前确认。`,
  };
}
...
export function buildWechatSendFailedState(
  ...
  return {
    ...current,
    lastOutcomeTitle: mode === 'controlled-send' ? '发送没有成功收口' : '自动发送没有成功收口',
    lastOutcomeDetail: detail || `目标联系人"${contact}"这一条没有完成真实发送。`,
  };
}

export function buildWechatDraftQueueState(current: WechatBatchState): WechatBatchState {
  return {
    ...current,
    active: true,
    paused: false,
...
export function buildWechatCompletedState(current: WechatBatchState): WechatBatchState {
  return {
    ...current,
    active: false,
    paused: false,
...
export function buildDouyinPausedState(current: DouyinBatchState): DouyinBatchState {
...
export function buildDouyinAdvancingState(
  ...
  return {
    ...
    lastStopReason: undefined,
    lastOutcomeTitle:
      mode === 'direct-message-reply' ? '正在继续下一条私信对象' : '正在继续下一条评论对象',
    lastOutcomeDetail: current.nextTargetHint
...
export function buildDouyinSkippingState(
  ...
  return {
    ...
    lastStopReason: undefined,
    lastOutcomeTitle:
      mode === 'direct-message-reply' ? '正在跳过当前私信对象' : '正在跳过当前评论对象',
...
export function buildDouyinResumingState(current: DouyinBatchState): DouyinBatchState {
  return {
    ...current,
    active: true,
    paused: false,
...
export function buildDouyinRunningState(
  ...
  return {
    ...
    paused: false,
    completed: false,
    consecutiveSkippedOutcomes: shouldReset ? 0 : current.consecutiveSkippedOutcomes,
    repeatTargetOutcomeStreak: shouldReset ? 0 : current.repeatTargetOutcomeStreak,
...
export function buildWechatPausedState(current: WechatBatchState): WechatBatchState {
