...
function platformConnectionStatusLabel(
  ...
  if (entryLoggedIn === false) return "等待登录核对";
  return "待核对";
}
...
function evidenceSavedLabel(value?: string | null) {
  ...
  const text = commercialDisplayText(value);
...
export function WorkbenchPageShell({
  ...
  const processingSummary = processingSummaryTemplate.replace(
    ...
    String(
      Math.max(
        douyin.douyinBatchState?.processedCount ?? 0,
        wb.processedCount ?? 0,
      ),
    ),
  );
  ...
  const browserEvidence = [
    ...
    entry?.browser
      ? "本机浏览器已接管平台后台"
      : null,
    session?.profileDir || entry?.profileDir ? "独立账号环境已准备" : null,
    session?.currentUrl || entry?.url ? "平台页面已打开" : null,
  ...
  const pageProbeEvidence = [
    ...
    entry?.pageTextSample ? "页面内容已完成探测" : null,
  ].filter(Boolean) as string[];
  return (
        ...
        <CardBody className="gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              {wb.accounts.length > 1 ? (
                <Select
                  ...
                  className="w-56"
                  listboxProps={{
                    "aria-label": `${platformName}账号列表`,
                  }}
                  size="sm"
                  selectedKeys={
                    wb.selectedAccount?.id
                      ? [String(wb.selectedAccount.id)]
                      : []
                  }
          ...
          {agentS.agentSError && (
            <Chip color="danger" size="sm">
              {commercialDisplayText(agentS.agentSError)}
            </Chip>
          )}
          <OpsWorkbenchDouyinCard
            ...
            roundStatusLabel={
              overrideOutcome?.roundStatusLabel ??
              wb.visibleOutcome?.roundStatusLabel
            }
            roundStatusDetail={
              overrideOutcome?.roundStatusDetail ??
              wb.visibleOutcome?.roundStatusDetail
            ...
            stageLabel={
              overrideOutcome?.stageLabel ?? wb.visibleOutcome?.stageLabel
            }
            lastOutcomeTitle={
              overrideOutcome?.lastOutcomeTitle ??
              wb.visibleOutcome?.lastOutcomeTitle
            }
            lastOutcomeDetail={
              overrideOutcome?.lastOutcomeDetail ??
              wb.visibleOutcome?.lastOutcomeDetail
            }
            progressLabel={`已处理 ${processedCount} 条`}
            progressHint={
              processedCount > 0
                ? "最近完成的真实任务已同步到本页。"
                : "尚未开始。"
            ...
            liveSteps={
              overrideOutcome?.liveSteps ?? wb.visibleOutcome?.liveSteps
            }
            liveEvents={
              overrideOutcome?.liveEvents ?? wb.visibleOutcome?.liveEvents
            }
          ...
          {browserEvidence.length || pageProbeEvidence.length ? (
            <div className="grid gap-2 rounded-[8px] border-small border-divider bg-default-50 p-3 text-tiny text-default-600 md:grid-cols-2">
