...
export type OpsWorkbenchWechatDraftReadyResult =
  OpsWorkbenchWechatConversationSnapshotResult & {
    ok: boolean;
    stage:
      | "contact_not_ready"
...
export type BuildDouyinDirectiveContinuePromptParams =
  BuildDouyinContinuePromptParams & {
    directive: string;
  };
  ...
  return [
    ...
    mode === "comment-reply"
      ? sendMode === "auto-send"
        ...
        : "进入页面后，优先进入评论管理页；如果当前没有可处理评论，要明确回报 no_target_available，不要假装正在处理。只有发现真实目标留言时，才继续分析上下文，生成一条适合当前对象的 AI 正式回复，并把这条回复真实填入页面；确认输入框、发送按钮和目标对象真实可见后直接发送，现场条件不完整时停止并回报 waiting_for_send_confirmation。"
      : mode === "direct-message-reply"
        ? sendMode === "auto-send"
          ...
          : "进入页面后，优先进入私信管理页；定位一个真实会话，读取最近上下文，分析对方当前意图和沟通阶段，生成一条适合当前对象的 AI 正式回复；如果已经给了参考回复，也只能把它当约束，不要机械照抄。最终要把你分析后的正式回复真实填入当前输入区；确认真实输入框、真实发送按钮和真实现场草稿都已就位后直接发送，现场条件不完整时停止并回报 waiting_for_send_confirmation。如果当前列表没有合适会话，要明确回报 no_target_available，不要假装已经发出。"
    ...
    sendMode === "approval-send"
      ? "发送策略：受控发送。目标对象、输入框、发送按钮和回复内容都在现场回读一致后直接发送；任何一项不完整就停止并说明原因，不要跨目标扩散。"
    ...
    `[目标对象] ${params.creatorName || "目标账号"} / ${
      params.commentUser ||
      (mode === "direct-message-reply" ? "目标会话用户" : "目标留言用户")
    }`,
    ...
    mode === "read-only-analyze"
      ...
      : mode === "auto-send"
        ...
        : mode === "controlled-send"
          ? "这轮是受控发送模式。先锁定目标联系人并准备正式回复，目标和内容回读一致后继续执行；现场条件不完整就停止并说明原因。"
...
export function shouldWechatAllowDesktopActionExecution(
  mode: WechatExecutionMode,
) {
...
export function resolveWechatSendPolicy(
  ...
  if (taskType === "wechat.reply.read_only_analysis")
    return "read-only-analyze";
...
export function getDouyinSessionTitle(mode: DouyinBrowserMode) {
  ...
  if (mode === "direct-message-reply")
    return "私域运营作战台：抖音后台私信自动回复";
...
export function getDouyinResumeDirective(mode: DouyinBrowserMode) {
  return mode === "direct-message-reply"
    ...
    : "请从刚才暂停的位置继续这一轮抖音自动评论。如果已有下一条目标提示，优先回到那条真实评论；否则继续在当前评论列表里定位下一条需要处理的对象，并按结构化阶段回报。";
}
...
export function buildDouyinContinuePrompt(
  ...
  const liveHints = [
    ...
    params.liveTargetLabel
      ? `当前页面对象提示：${params.liveTargetLabel}`
      : null,
    params.liveEditorPlaceholder
      ? `当前回复框占位提示：${params.liveEditorPlaceholder}`
      : null,
  ]
    .filter(Boolean)
    .join("\n");
...
export function resolveWechatRecommendedAction(
  ...
  const effectiveTarget =
    params.expectedContact || params.sentTargetContact || "当前联系人";
  ...
  if (
    params.preferredLiveTarget &&
    params.preferredLiveTarget !== params.expectedContact
  ) {
    return {
  ...
  if (params.canSend) {
    ...
    return {
      ...
      label: "继续受控发送",
      hint: `目标联系人"${params.expectedContact}"已对齐，当前可以按受控发送推进。`,
      mode: "controlled-send",
    };
  }
...
export function resolveWechatAdvanceDecision(
  ...
  if (!params.aligned) {
    return {
      ...
      detail:
        params.note ||
        `候选联系人"${trimmedContact}"还没完成真机对齐，这一轮不会直接放行发送。`,
    };
  }

  const mode = resolveWechatNextMode(params.sendPolicy);
  const shouldUseGuardOverride =
    mode === "controlled-send" || mode === "auto-send";
  ...
  if (shouldUseGuardOverride && !params.canSend) {
    return {
      ...
      detail:
        params.sendReason ||
        `候选联系人"${trimmedContact}"虽然已经对齐，但执行保护还没通过，这一轮会停止并留下证据。`,
    };
  }
...
export function resolveWechatLiveSnapshotDecision(
  ...
  if (
    snapshot?.entityType === "contact" ||
    snapshot?.entityType === "search-result"
  ) {
    entityType = snapshot.entityType;
  } else {
    const headerTexts = Array.isArray(snapshot?.headerTexts)
      ? snapshot.headerTexts
      : [];
    const listTexts = Array.isArray(snapshot?.listTexts)
      ? snapshot.listTexts
      : [];
    const looksLikeSearchResult =
      headerTexts.some(
        (text) => typeof text === "string" && text.includes("搜一搜"),
      ) ||
      listTexts.some(
        (text) =>
          typeof text === "string" &&
...
export function resolveWechatPreparedDraftDecision(
  ...
  if (!params.draftReady.draftInserted || !params.draftReady.inputReady) {
    return {
      kind: "draft-not-ready",
      detail:
        params.draftReady.note ||
        "微信输入框还没准备完成，这一轮先停在发送前。",
    };
  }
  ...
  if (params.mode === "controlled-send") {
    const wasAlreadyConfirmedReady =
      ...
      params.previousDraftReadyState?.stage ===
        "draft_ready_for_send_confirmation";
...
export function resolveDouyinLiveSendDecision(
  ...
  const shouldAutoAllowSend = Boolean(
    hasPermissionRequest &&
    isSendLikeTool &&
    sendMode === "auto-send" &&
    canAllowLiveSend,
