...
function getDouyinModeTitle(
  mode: DouyinBrowserMode | null,
  completed: boolean,
) {
  if (completed) {
...
export function buildOpsWorkbenchTodayMetrics(
  ...
  const pendingConfirmCount =
    args.hasApproval || args.douyinBrowserProgressSummary?.status === "review"
      ? 1
      : 0;
  const estimatedSavedMinutes =
    processedCount > 0 ? Math.max(5, processedCount * 4) : 15;
...
export function buildOpsWorkbenchPendingApprovals(
  ...
  if (args.approvalRequest) {
    items.push({
      ...
      statusLabel: "待审批",
      subtitle:
        args.approvalRequest.description || "当前有一项人工确认请求待处理。",
...
export function buildOpsWorkbenchPendingQueue(
  ...
  if (args.activeDouyinBrowserSessionId && args.douyinBrowserProgressSummary) {
    items.push({
      ...
      subtitle: args.douyinBatchState.paused
        ? args.douyinBatchState.pauseReason ||
          "暂停后会从当前后台列表继续推进。"
        : args.douyinBatchState.completed
          ? args.douyinBatchState.completionSummary ||
  ...
  if (args.browserCapabilityStatus !== "ready") {
    items.push({
      ...
      subtitle:
        args.browserSetupHint ||
        "需要先连接浏览器后台，才能直接进抖音后台自动处理私信。",
      ownerLabel: "浏览器后台处理",
      badges: ["后台前置"],
      tone: "warning",
    });
  ...
  if (
    args.showWechatPanel ||
    args.wechatExpectedContact ||
    args.wechatLiveEntityType !== "unknown"
  ) {
    items.push({
      id: "wechat-queue",
      title:
...
export function buildOpsWorkbenchSummaryCards(
  ...
  return [
    ...
    },
    {
      ...
      tone:
        args.localControllerPermissionMode === "full" ? "warning" : "neutral",
    },
  ];
}
...
export function buildOpsWorkbenchRecentReport(
  ...
  if (args.wechatBatchState.completed && args.wechatSendState?.sent) {
    ...
    return {
      ...
      stats: [
        ...
        {
          id: "wechat-saved-actions",
          label: "预估省动作",
          value: `${savedActions} 步`,
        },
        {
          id: "wechat-skipped",
          label: "已跳过",
  ...
  if (args.douyinBatchState.completed || args.douyinBrowserProgressSummary) {
    ...
    const savedActions =
      Math.max(processed + (args.douyinBatchState.skippedCount || 0), 1) * 4;
    const completedTarget =
    ...
    return {
      ...
      stats: [
        ...
        },
        {
          ...
          label:
            args.douyinBatchState.completed || args.douyinBatchState.paused
              ? "最后对象"
              : "已记录步骤",
          value: args.douyinBatchState.completed
            ? completedTarget
          ...
          ? [
              {
                ...
                label: args.douyinBatchState.completed
                  ? "暂停原因"
                  : "当前停因",
  ...
  return {
    ...
    stats: [
      ...
      },
      {
        id: "status",
        label: "当前进度",
        value: mapAgentSessionStatus(args.agentSession.status),
      },
    ],
  };
}
...
export function buildOpsWorkbenchRecentReports(
    ...
    return [
      {
        ...
        title: args.wechatBatchState.completed
          ? "微信这一轮处理结果"
          : "最近一次微信发送结果",
        ...
        stats: [
          ...
          ...(args.wechatBatchState.lastProcessedContact
            ? [
                {
                  label: "最后处理",
                  value: args.wechatBatchState.lastProcessedContact,
                },
              ]
            : []),
          ...
          ...(args.wechatBatchState.lastPauseReason
            ? [
                {
                  label: "暂停原因",
                  value: args.wechatBatchState.lastPauseReason,
                },
              ]
            : []),
          ...(args.wechatBatchState.completed
  ...
  if (
    args.douyinBatchState.completed ||
    (args.activeDouyinBrowserSession && args.douyinBrowserProgressSummary)
  ) {
    const processed = Math.max(
      args.douyinBatchState.processedCount,
      args.douyinStructuredStage === "sent" ? 1 : 0,
    ...
    return [
      {
        ...
        title: getDouyinModeTitle(
          args.douyinBrowserMode,
          args.douyinBatchState.completed,
        ),
  ...
  const isWechatReplyDraft =
    args.agentSession.metadata?.agent_s_business_scenario ===
    "wechat-reply-draft";
  return [
    {
      id: "latest-run",
      ...
      stats: [
        ...
        {
          label: "最近任务",
          value: isWechatReplyDraft
            ? args.wechatSendPolicyLabel
            : mapAgentTaskType(args.agentSession.task_type),
        },
      ],
    },
  ];
...
export function buildOpsWorkbenchRecentTasks(
  ...
  if (args.agentSession) {
    ...
    items.push({
      ...
      subtitle: isWechatTask
        ? wechatTaskSubtitle
        : args.mappedSessionStatusDetail,
      ownerLabel: "私域运营作战台",
